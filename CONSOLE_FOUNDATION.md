...
- Хром модалки (счётчик/панель действий) скрыт классом `--flight` и
  проявляется ТОЛЬКО после приземления карты; на открытии полёт идёт над ЖИВОЙ
  сценой, а СТАТИЧЕСКИЙ `::backdrop` появляется в ОДИН шаг на showModal
  (совпадает с прибытием карты). НЕ добавлять отдельную dim-вуаль (стекалась с
  backdrop и размонтировалась спустя beat → «весь фон мигал») и НЕ анимировать
  `::backdrop` на открытии (Graphite-инвариант). Backdrop гаснет ПОД
  close-полётом (стол уже виден при посадке).
