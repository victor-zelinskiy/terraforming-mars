...
- `.con-zoom-hold` (opacity 0) вешается на слот-ОБЁРТКУ (`sourceSlotEl` =
  `origin.resolve(index)`), НЕ на внутреннюю `.card-container`/`.pcard` —
  иначе гаснет только карта, а рамка/подсветка/выделение обёртки остаются
  «силуэтом» на месте вытянутой карты (особенно на модальных слотах). Геометрию
  полёта меряем от карты (`sourceCardEl`; opacity на layout не влияет). Так же
  deal держит слот через `.con-deal-hold` на обёртке.
- Хром модалки (счётчик/панель действий) скрыт классом `--flight` и
  проявляется ТОЛЬКО после приземления карты; на открытии сцену диммит
  ВУАЛЬ (`.con-zoom-veil` — те же пиксели, что `::backdrop`, который
  появляется под ней мгновенно на showModal; entry-анимации у backdrop НЕТ —
  см. Graphite-инвариант); backdrop гаснет ПОД close-полётом (стол уже виден
  при посадке).
