...
- `physical` — открыто из видимого card tile. FLIP-лифт ПРОКСИ: прокси-карта
  ...
  Слот карты «в руках» держится пустым (`.con-zoom-hold`, ретаргет на каждый
  browse). Закрытие — обратный полёт РЕАЛЬНОГО стейджа в слот ТЕКУЩЕЙ карты
  (`resolve(index)` — живой re-query; поверхность к этому моменту давно
  отпейнчена — close инвариант не нарушает). Невидимый/несуществующий слот →
  graceful dive/rise-fallback.
