...
1. Штатное репро ×N: новая партия → deal → X сразу / после 10с простоя / после Alt+Tab.
2. Регресс: LB/RB, B-возврат в слот, зум из руки/журнала, «Получены карты» (mandatory + L3),
   handoff «разыграть» в композер, reduced motion, десктопный мышиный зум.
3. Probe-ручка (временная): `localStorage.tm_zoom_vanilla='1'` — чисто ванильное открытие без
   полёта. Если баг вдруг воспроизведётся С НЕЙ — триггер не в хореографии; следующий уровень:
   A/B `--disable-direct-composition`, `TM_ELECTRON_GPU=low` (F7), Graphite off (F3) — и
   репорт в Electron/Chromium.
4. После подтверждения — снять `[TM-DIAG zoom]` диагностику по DIAGNOSTIC_CLEANUP.md.
