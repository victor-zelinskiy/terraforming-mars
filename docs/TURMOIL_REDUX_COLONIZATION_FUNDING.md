...
**Сдано 2026-09-25 (RX21)** — с одной поправкой к рецепту: слово `'marsCity'` завёл RX20 для счёта НАЗНАЧЕНИЙ яруса
(`marsCities`, клетка раз), а Migration Funding считает ВЕЛИЧИНУ (сумму ярусов) — потому board-счёт получил
явную меру `measure: 'cells' | 'tiers'`, свой id `marsCityTiers` и столбец высот `tiers` рядом со `spaces`.
См. `docs/TURMOIL_REDUX_MIGRATION_FUNDING.md`.
