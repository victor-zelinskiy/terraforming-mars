...
- [LESS_DEBT_CUT.md](LESS_DEBT_CUT.md) — the style layer catches up with the deletion waves: a live-class corpus, per-selector verdicts, deep-grep before every cut — `cards.less`/`cards_scifi.less`/`player_home.less` deleted with live styles re-homed (`card_render_dsl.less`, `app_chrome.less`), `cards_v2`/`journal` trimmed; −10.5k lines, styles.css −9 %, visual A/B (incl. a same-css control run) + DSL computed-style probe byte-identical.
...
- [TURMOIL_REDUX_ANALYSIS.md](TURMOIL_REDUX_ANALYSIS.md) — Turmoil → Turmoil Redux, stage-1 analysis (official rules vs the upstream engine in this fork, mechanics breakdown, "mechanics on / cards off" switch, the console-native UI plan, integration risks, effort table, open questions for the Redux rules). Analysis only — no code.
- [TURMOIL_REDUX_ITERATION0_PLAN.md](TURMOIL_REDUX_ITERATION0_PLAN.md) — iteration 0 plan (approved scope, server architecture: `parliament` module + `game.politics` facade + per-card readiness, the resumable political phase, the six party effects, the «Парламент» workspace concept and resolution card family, MarsBot bystander mode, staged implementation with verifiable results, readiness criteria, deferred TODO).
- [TURMOIL_REDUX_SPEC.md](TURMOIL_REDUX_SPEC.md) — stage 2: the unified Standard-Variant rule spec (Turmoil vs Redux vs code, per mechanic), revised reuse map (new `parliament` module + `turmoilRedux` expansion key), the 48-resolution catalog with mechanism/status per card and the starting pool, cross-cutting changes (greenery revision, solar-phase driver, chairman quests), the console UI concept around the voting area, the staged implementation plan with estimates, open rules questions. Analysis only — no code.
- [TURMOIL_REDUX_ITERATION0_STATUS.md](TURMOIL_REDUX_ITERATION0_STATUS.md) — iteration 0 STATUS (2026-09-14): what works on the server (`parliament` module, `Phase.PARLIAMENT`, six party effects, quests/chairman, save/resume) and in the console UI (resolution card family, the «Парламент» workspace, inspector, InfoPanel strip, forecast), how it was verified (server/client specs, the `console-parliament` e2e + screenshots), the honest limitations and the agreed TODO.
- [TURMOIL_REDUX_PARLIAMENT_UI_REWORK.md](TURMOIL_REDUX_PARLIAMENT_UI_REWORK.md) — the «Парламент» UI rework (2026-09-14): the composition (government · voting area with tallies + focus rail · parties 3×2 + one detail · Agenda as levels), the three profile layouts, the player routes (vote, party actions moved into «Действия карт» with a contextual Parliament door, Unity locked trade, Reds, chairman seat collapse/restore, results once per generation across reloads), the contracts (server-authoritative party tiles, the party trade door, honest forecast, fits that never read their own output, the production floor), verification (unit + dense/seat e2e on 1080/TV/Deck with `expectFits`) and honest limitations.
- [TURMOIL_REDUX_PARLIAMENT_V3.md](TURMOIL_REDUX_PARLIAMENT_V3.md) — the «Парламент» v3 rework (2026-09-15): the VOTE STEP as a phase descent of the frame (the pressed card carried into the hero column, the decision surface beside it, the same step from the fullscreen viewer via the zoom handoff, B folds the phrase back), strict card physicality (physical zoom origins, one object), delegates as one token system with sockets (lobby 0/1, threshold places, popular support) and the SEATS ledger, the rebuilt overview hierarchy (enacted card as the main object, ruler badge, no V-labels), the plaque anatomy + new Unity/Scientists emblems, the resolution face template with dev examples, the paid vote's bill inside the step (server marker, collapse/restore/reload), the Agenda marker glide + card reward lift, the text fixes, verification and honest limits.
- [TURMOIL_REDUX_PARLIAMENT_V2.md](TURMOIL_REDUX_PARLIAMENT_V2.md) — the «Парламент» v2 rework (2026-09-15): the game-screen / help boundary (objects and states on the overview, rule prose only in the X inspector), the free composition (ruling plaque + chairman quest block · voting cards with stacked tallies + focus rail · six party plaques in one row, 3×2 on the Deck · Agenda glyphs), the party visual family (`ConsolePartyPlaque` tile/hero/full), ONE execution point with two doors (action menu / Parliament plaque → the same nested action workspace), the Reds as one flow inside it (embedded reveal → hand step → payout beat; collapse/restore/reload resume), the rewritten inspector texts, verification (unit + the `console-parliament-v2` e2e on 1080/TV/Deck) and honest limits.
...
- [PROMPT_PREMIUM_AUDIT.md](PROMPT_PREMIUM_AUDIT.md) — prompt SHAPE parity: which prompts still reach the player as the generic two-step list / nested wizard instead of the premium one-press decision (the Flooding reference fix + the wave plan).
