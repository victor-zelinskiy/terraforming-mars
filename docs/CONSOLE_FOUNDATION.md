...
**Lifecycle-контракты:** `spawnProxies(..., holdSource=true)` — источник
(его `.card-container`) прячется классом `con-deal-hold` (`opacity:0
!important`, БЕЗ transition) в ТОМ ЖЕ синхронном блоке, где позиционируется
прокси (после nextTick, ДО пейнта) — прокси появляется и реальная карта
исчезает одним кадром, НИКОГДА оба разом (баг «карта улетает и одновременно
остаётся в слоте»). НЕ полагаться на `onLift`/table-beat-фейд для скрытия
источника (onLift часто лишь асинхронно сабмитит; 240ms-фейд table-beat
показал бы источник УХОДЯЩИМ под уже летящей проксей). Источник —
УХОДЯЩИЙ элемент (следующий ре-рендер его удаляет, ключи разные → нет
reuse-утечки класса), поэтому hold НЕ снимается (единственное исключение —
recovery-путь драфт-бита, он un-hold'ит). Применено ко ВСЕМ exit-потокам:
take/collect/hero/depart/draftPickToTray; `runCardTransfer` держит источник
своим `holdFrom`/Vue-managed стейджингом (рука↔композер). `onLift` — хост
коммитит game/UI state ВНУТРИ него
...
`beginZoomOpen`/`playZoomOpenFlight`/`cancelZoomOpen`/`playZoomClose`/
`retargetZoomHold`/`releaseZoomMotion`), `CardZoomModal.vue` (opt-in
`consoleMotion` prop + `measureLanding()`), `ConsoleShell.runZoomOpen()`, стили
в `console_card_deal.less` (`.con-zoom-hold`, `.con-zoom-veil`,
`.con-zoom-flight-layer`/`-proxy`, `--flight`, `--closing`).

**⚠️ GRAPHITE-ИНВАРИАНТ ОТКРЫТИЯ (load-bearing — не нарушать; история:
`ZOOM_BUG_HANDOFF.md`, фикс 2026-07-14).** `showModal()` вызывается ТОЛЬКО в
момент касания полёта: первый top-layer кадр диалога = финальный, статичный,
полностью видимый контент. Премиум-лифт летит ПРОКСИ (`CardZoomCard` на
`.con-zoom-flight-layer`, обычный fixed-слой — паттерн deal/exit полётов);
посадочная геометрия меряется на ещё ЗАКРЫТОМ диалоге
(`CardZoomModal.measureLanding()` — инлайн `display:flex; visibility:hidden`,
layout идентичен открытому, ничего не пейнтится); вуаль `.con-zoom-veil`
пре-диммит сцену теми же пикселями, что будущий `::backdrop`. Причина: слой,
созданный в первые кадры свежей top-layer поверхности скрытым /
transform-анимируемым (или keyframe-анимируемый `::backdrop`), на
Windows+Skia Graphite мог НИКОГДА не презентоваться (DOM идеален — пиксели
мёртвые). Поэтому НИКОГДА: не скрывать/не transform-анимировать реальный
`.card-zoom-stage` на первых кадрах диалога и не возвращать entry-анимацию на
`::backdrop`.
...
- `physical` — открыто из видимого card tile. FLIP-лифт ПРОКСИ: прокси-карта
  стартует из rect слота и раскрывается на измеренную посадку; на касании —
  `showModal()` (top layer накрывает прокси, шелл снимает его с задержкой).
  `zoomOpening` ставится СИНХРОННО в watcher'е (не внутри async runZoomOpen) —
  окно «диалог отрендерен, но закрыт + идёт измерение посадки» не должно
  ловить B нормальной close-веткой (playZoomClose по display:none диалогу =
  rect 0×0 → dive → zoom.close() no-op → нет 'close'-события → `zoomClosing`
  застревает true → «B закрывает со второго нажатия»). Слот карты «в руках»
  держится пустым (`.con-zoom-hold`, ретаргет на каждый browse). Закрытие —
  обратный полёт РЕАЛЬНОГО стейджа в слот ТЕКУЩЕЙ карты (`resolve(index)` —
  живой re-query; поверхность давно отпейнчена — close инвариант не нарушает):
  быстрый старт `power2.out` (мгновенный отклик на B, не медленный inOut) +
  РАСТВОРЕНИЕ в слот (reveal реальной карты `holdSlot(null)` за ~60мс до
  касания + кросс-фейд стейджа над ней, как в deal/handoff) — карта
  «возвращается на место», НЕ «падает и щёлкает». Невидимый/несуществующий
  слот → graceful dive/rise-fallback.
- `textual` — открыто из чипа/ссылки/имени (журнал, обзор хода бота):
  честный «инспекторный» вход — подъём прокси из глубины (scale+fade) с тем
  же showModal-на-касании, закрытие — погружение стейджа. НИКОГДА не
  фальшивый коллапс в несуществующий слот.
...
- `.con-zoom-hold` (opacity 0) вешается на ВНУТРЕННЮЮ `.card-container`/`.pcard`
  (`sourceCardEl`), НЕ на слот-обёртку: у обёртки Vue-`:class` (напр. hand
  `--selected` по `index`), и Vue перезаписывает её class при листании LB/RB →
  JS-класс на обёртке смывается → карта возвращается на доску = ДВЕ копии
  (нарушение «1 физическая карта»). Внутренняя `<Card :key>` не перепатчивается
  при листании. «Силуэт» обёртки (рамка/подсветка на месте вытянутой карты,
  заметнее на модалках) гасится CSS `:is([data-zoom-slot],
  .con-composer__playcard):has(.con-zoom-hold){opacity:0}` — от дескрипторного
  класса на карте, устойчиво к перерисовке обёртки. Геометрию полёта меряем от
  карты (`sourceCardEl`; opacity на layout не влияет).
- Хром модалки (счётчик/панель действий) скрыт классом `--flight` и
  проявляется ТОЛЬКО после приземления карты. **Дим = ОДИН источник — вуаль
  `.con-zoom-veil`** (обычный слой под top layer): плавно входит с ПЕРВОГО
  кадра открытия (Vue enter-transition), живёт всё время fullscreen, гаснет
  ПОД close-полётом (`--lifted` по `zoomClosing`; leave-transition страхует
  резкий unmount). `dialog.con-zoom[open]::backdrop` НЕ РИСУЕТ НИЧЕГО
  (background: none) — statический пустой backdrop сохраняет
  Graphite-инвариант бесплатно. НИКОГДА: не возвращать дим/анимацию на
  `::backdrop` и не заводить ВТОРОЙ источник дима (два стека = «фон мигает»;
  один шаг на showModal = «резко и поздно»).
- **Перф-заморозка сцены:** `body.con-zoom-open .con-root { *, & {
  animation-play-state: paused !important } }` (+ revert для
  `dialog.con-zoom` и `.con-zoom-flight-layer` поддеревьев) — под
  полупрозрачным димом доска/попапы с infinite-пульсами иначе растеризуются
  каждый кадр всё время инспекции (console-двойник десктопного
  `body:has(dialog[open]) #player-home`-правила). CSS-анимации only — GSAP /
  WAAPI / transitions не задеты.
- LB/RB листание (`consoleMotion` в CardZoomModal): ПРЕРЫВАЕМОЕ (rapid
  ...
  байт-в-байт прежний.
- Open-полёт едет на ПРОКСИ (GSAP; CSS `zoom` — на внутренней обёртке
  `__zoom`, НИКОГДА на самом трансформируемом элементе: zoom рескейлит
  translate-координаты), close-полёт — на stage-РОДИТЕЛЕ (GSAP), слайд
  листания — на card-РЕБЁНКЕ (WAAPI) — вложенные трансформы, конфликтов нет.
