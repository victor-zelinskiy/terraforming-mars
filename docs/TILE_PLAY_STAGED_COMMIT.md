...
1. **Staging — карта становится заданием.** По «Разыграть» карта перелетает из
   композера в **слот источника досье размещения** (правая рельса; там уже живёт
   `placementSourceView` / `L3 Источник`) — не «в никуда» и не в невидимую таблицу.
   Карта видима весь выбор клетки, X/L3-инспекция непрерывна. **B = обратный перелёт
   той же карты в композер** (один объект, flip-anchor = объект, `onInterrupt`
   восстанавливает — уроки [[flip-anchor-is-the-object]] / wheel-handoff echo).
...
- полная хореография §8-bis: staged-полёт карты В СЛОТ ДОСЬЕ (сейчас — обычный
  leave композера; источник виден в досье текстом), card-seal ПЛАШКА на тайле +
  импульс ACTION COMMIT + «колода отвечает» от плашки; реверс-полёт по B;
