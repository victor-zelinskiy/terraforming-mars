...
## Волна 4 (2026-07-18) — 7 точечных фиксов по фидбеку
1. **Corp-карты «при розыгрыше»**: play-rail (мятный) теперь показывается и на корпорациях —
   над стартовыми ресурсами (начните с X M€ = on-play). Новый `playZoneStartLeading` (ведущий
   run стартовых ресурсов вместо трейлингового); PremiumMechanicsPanel получил проп `corporation`;
   CSS `display:none` для corp-rail снят.
2. **Лидер-линии убраны**: SVG-линии/узлы из ConsoleCardRulesPanel удалены (цветовая связь
   чип↔элемент их заменила); измерение порядка блоков сохранено, geom-код по линиям вычищен.
3+4. **Старт-сцена**: убран мой ошибочный `height:84vh` (ломал full-height stretch-раму);
   **hand-dock «КАРТЫ n/m» скрывается под любой центральной сценой** (новый `handDockVisible` +
   `v-show`, бар теряет bay → хинты центрируются) — это и был компонент, обрезавший карты снизу;
   рама шире + top-headroom под плашку «ВЫБРАНА»; карты заполняют освободившееся место.
5. **Кнопка РАЗЫГРАТЬ**: `.con-start__slot-a` компенсирует zoom родительской карты
   (corp 0.92·s / prelude 0.86·s / candidate 0.78·s) → пилюля рендерится одним спокойным
   root-размером, не ×50px.
6. **Модалка покупки**: `.con-task--wide` расширена до `min(92rem,95vw)`; single-row fit теперь
   учитывает и высоту (потолок 1.6·s) — несколько карт заполняют сцену, не висят мелкими.
7. **Рука**: reason-chip перенесён на центр карты (top:42%) — закрывает арт, не механику;
   grayscale-фильтр недоступных карт EASES IN (keyframe `con-hand-dim-in`, 300ms) — нет резкого
   снапа при влёте из дока; reduced-motion → мгновенно.

### Волна 4 доп.: buy-модалка — root-cause мелких карт
Диагноз: buy и draft — один `ConsoleTaskHost` cardSelect, но **между-поколенный BUY всегда идёт
ПОСЛЕ драфта → включается research-RISE сцена** (карты влетают из трея). `fitCardStrip`, отработавший
во время cinematic (переходная раскладка / до применения `--wide`), оставлял `--con-cards-zoom` на
fallback 1 → карты 320px raw (мелко на 4K). Draft компонуется, потому что ре-фитится каждый раунд.
Фикс: (а) ре-фит по завершению deal/rise cinematic (`watch deal.state.active → false`);
(б) settle-таймер 380ms в resetKey (страховка от исчерпания retry-фреймов, когда pcard-лицо
не разложилось к моменту первого fit). Оба идемпотентны, draft не задет.
Требует живой проверки (e2e до research-buy — длинный путь).

### Волна 4 доп.2: стандартизация цвета рамок эффект/действие
Несоответствие: рамка эффекта `.pcard-effect` брала border из `--pcard-rim-soft` — а он ЗАВИСИТ ОТ
ТЕМЫ карты (azure→синий, corporation→ЗОЛОТО, emerald→зелёный, crimson→красный). Поэтому один и тот
же «эффект» читался разным цветом на разных типах карт (на корпорации — золотой, mismatch с синим
чипом «ЭФФЕКТ»). Фикс: новые СЕМАНТИЧЕСКИЕ токены `--pcard-effect-rim` (#5aa9e6 = @con-effect-blue)
и `--pcard-action-rim` (#f2d795 = @con-action-gold) в корне `.pcard`; `.pcard-effect` → effect-rim,
`.pcard-effect--action` → action-rim. Теперь эффект СИНИЙ, действие ЗОЛОТОЕ на ЛЮБОЙ карте, точно
под цвета чипов правил. Проверены все категории: требование=коппер (корневой токен, ok), ПО=золото
(ok), при розыгрыше=мята (ok) — единственный theme-tied цвет был у эффекта. Shared premium-лицо
(desktop тоже, осознанно — единый язык). PremiumCard спек 12/12.

### Волна 4 доп.3: клип плашки «ВЫБРАНА» на фокусе (buy-модалка)
Плашка-ribbon `.con-cards__pickband` свисает над картой (top:-.6rem); на фокусе слот масштабируется
1.08+lift, и на КРУПНЫХ 4K-картах (после buy-фикса) абсолютный сдвиг вверх перерос headroom полосы
(2rem) → верх плашки уходил за `overflow`-клип полосы (НЕ z-index — клип по overflow контейнера).
Фикс: щедрый top-padding обеих карточных полос на TV (`.con-cards__strip` 3.4rem, grid 2.6rem) +
hand-грид (`.con-hand__pad` 1.6rem). Центрированный масштаб симметричен → низ (46px) в пределах
bottom-padding, верх (свес ~122px) в пределах 136px — не режется ни сверху, ни снизу. Прологи
(`.con-start__prelude-grid`) не были затронуты (там headroom хватало). Альтернатива «плашка внутрь
карты (top:0)» отвергнута — налезала бы на nameplate с названием.
