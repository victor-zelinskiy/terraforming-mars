...
## Итерация 2b (2026-07-19, поздний вечер) — ОТКЛОНЁННЫЕ decision-экраны + реальная 4K-верификация

Итерация 2 отклонена: часть правок была масштабированием desktop-раскладки, а не
переразметкой; критично — скриншот-матрица НЕ покрывала decision-экраны (застревала в
ФАЗЕ ПРОЛОГОВ, снимая старт-сцену вместо реального хода), поэтому «зелёные тесты» не поймали
overflow/пересечения/микро-данные. Исправлено:

- **Инфраструктура верификации:** новая `tests/e2e/tv-decision-screens.spec.ts` — no-prelude
  игра доводится до РЕАЛЬНОГО action-хода (двухфазный драйвер: мастер KeyE-advance + Enter,
  покупка карт хореографией, оплата burst-Enter), затем доводит до каждого decision-экрана и
  снимает при 3840×2160. КАЖДЫЙ фикс проверен на реальном кадре (не по фиксированной матрице).
- **card-actions (worst):** корень — `zoom×3` применялся и к TEXT-override действию (Arcadian
  Communities рисуется как ПРЕДЛОЖЕНИЕ, не иконки) → гигантский текст/overflow/дубль. Фикс:
  `:has(> .con-cardactions__graphic-text)` СНИМАЕТ zoom с текстовой графики (база + TV
  `:not(:has())`); текст — контролируемый rem (мастер secondary /clamp 2 строки, деталь body,
  wrap); деталь получила лейбл «ДЕЙСТВИЕ» (одна полная копия). Иконочная графика сохраняет zoom.
  Верифицировано: пустое состояние gen1 — хром/фильтры чистые.
- **play-confirm:** РЕЗУЛЬТАТ обёрнут в framed **hero-блок** (`__resulthero`, мятная рамка);
  result/variant-чипы hero-размера (токен+.15/+.3), rescat hit-min+body, оплата крупная,
  карта-источник reined до 0.86×. Верифицировано: production-чипы «0→1 / 0→3» крупные,
  данные доминируют над CTA.
- **quick wheel:** лейблы `white-space: normal` (wrap вместо overflow в центр — корень
  пересечения «КОНВЕРТАЦИЯ РАСТЕНИЙ/ТЕПЛА»); TV колонки 14→18rem + gap; backdrop через
  `.con-backdrop-dim()` (TV dim-strong — фон больше не читаем). Верифицировано на обоих колёсах.
- **std projects:** иконка 3rem/плита 4.2rem, cost/gain 1.5rem+coin 1.7rem, кнопка secondary,
  wallet-preview. Верифицировано.
- **colonies:** новый **summary-бар фокус-колонии** (`__summary`: ВЫ ПЛАТИТЕ 🚀 → ВЫ ПОЛУЧИТЕ
