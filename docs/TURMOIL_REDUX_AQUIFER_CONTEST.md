...
- Код **рисуется на общей грани** (`.pcard__code` — гравировка рядом со штампом дополнения; скрыт на thumb-тире)
  и теперь появляется на **всех** премиальных гранях (`metadata.cardNumber` проектов → `vm.code`).
...
- **Стадия принятия** — постоянно смонтированный слой `.con-parl__enact` в Парламенте (никогда не `v-if`):
  ...
  `[data-embed-slot="parliament-enact"]`. Открывается watcher-ом `enactStanding` (промпт-карта с источником-резолюцией
  при `phase.step === 'effects'`); оболочка держит общий `ConsoleTaskHost` (`parliamentEnactStanding`, hold до
  `consoleParliamentUi.enactStanding`) и телепортирует **тот же экземпляр** в зону; крошка
  `ПАРЛАМЕНТ › ПРИНЯТИЕ › ВЫПЛАТА`.
  ...
  - **Океан победителя** — штатное размещение доски; досье показывает источник-резолюцию, и `L3 Источник` открывает её
    инспектор (`ConsoleShell.placementSourceResolution` → `inspectPlacementSource`).
...
6. Клиент **не требует** отдельных правок: грань, голосование, fullscreen, пикер (фиксация на месте + полёт выплаты
   для любого `cardResource`-исхода резолюции), L3-источник, стадия принятия, recap и Полигон читают манифест и
   `scaled`. Проверить в «Полигоне» (каталог подхватывает новую карту из реестра; сценарии работают на любой
   `scaled`-декларации).
