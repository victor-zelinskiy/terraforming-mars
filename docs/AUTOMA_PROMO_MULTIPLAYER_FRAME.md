...
следующий человек после бота по порядку хода`. Бот никогда не «атакует в щит», когда есть
валидная цель; выбор жертвы МЕЖДУ игроками всегда детерминирован и принадлежит боту,
никогда игроку.
- **B01 Meteor Shower:** жертва = человек с наибольшим числом растений среди НЕзащищённых
  (Protected Habitats / ADS / deflection исключают игрока из кандидатов); tie → порядок
  хода. Все растениевладельцы защищены → outcome 'protected' → destroy (как соло-FAQ);
  ни у кого нет растений → nothing-to-lose → discard; removed ≥3 → destroy.
- **B02 Invasive Species:** ГЛОБАЛЬНЫЙ максимум `cubeVpRate` по всем незащищённым
  ...
  исключают); tie между игроками → порядок хода; tie ВНУТРИ выбранной жертвы (несколько
  равных по ставке карт) → confirm-промпт жертвы (соло-совместимо: при равной ставке выбор
  скоринг-эквивалентен; no-auto-select правило форка). Награда бота — независимо от успеха
  удаления. `cubeVpRate` = (each??1)/(per??1) декларативного `{resourcesHere}`-VP,
  special/bespoke → 0 — ДОКУМЕНТИРОВАННАЯ аппроксимация «ценности кубика» (не marginal-VP
  с floor-эффектами); соответствует официальной формулировке «highest-scoring cube».
...
- **Канон жертвы (Q9):** `AutomaUtil.humansInTieOrder` (порядок поколений от кресла после
  бота) + `pickVictim` (строгий max, tie → первый в tie-порядке). B01/B02/B08/MA-селекторы
  переведены; `humanOf` удалён; соло = вырожденный случай ТОГО ЖЕ пути (нет режимных веток).
  B02: жертва bot-owned детерминированно; промпт — только между её равными по ставке картами.
