...
- **Окно ожидания ack (2026-08-14, багфикс «бот иногда думает ~5 с»):** ack — это сигнал
  «человек ДОЧИТАЛ компактную карточку хода», а у неё ограниченная жизнь (`BOT_TURN_TTL`
  = 5 с на клиенте). Раньше `unacked` не имел часов и хранил до 12 ходов, поэтому ОДИН
  неотвеченный ход (уведомления выключены, зритель посреди своего промпта — его клиент
  вообще пропускает refresh, закрытая вкладка, потерянный POST) навсегда держал гейт
  открытым, и КАЖДЫЙ следующий ход бота молча выбирал весь бюджет расширений (1.7 с соло /
  3.2 с при 2+ подключённых). Стенд не выглядел зависшим — бот просто был постоянно
  медленным. Теперь: (1) отслеживается ТОЛЬКО последний разрешённый ход (карточки
  показываются последовательно, значит подтвердивший ход N заведомо видел N-1), (2) запись
  перестаёт держать гейт спустя `UNACKED_MAX_AGE_MS` = 10 с — за этим окном ack уже не
  придёт и ждать нечего. Регрессии закреплены в `tests/automa/BotTurnScheduler.spec.ts`.
