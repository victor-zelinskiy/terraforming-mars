...
- v-show pick-мост НЕ анимируется: leave при активном мосте ставит
  ...
  пустой рамкой.
  ⚠⚠ **Re-show моста восстанавливает РОВНО то shade-владение, что было на
  момент скрытия** — leave записывает его В ЭЛЕМЕНТ (`data-motion-shade-held`,
  прочитано ДО `removeShadeOwner`), enter при `pick-hidden` возвращает
  владельца только по этой метке (per-element, так что два живых экземпляра
  одной поверхности не могут обменяться клеймами). Безусловный `addShadeOwner`
  в начале enter'а — как было — вешал ВЕЧНЫЙ дим на вложенной цепочке
  «Действия карт → Гидросеть (шаг 7) → repeat-селектор → назад»: origin
  card-actions к тому моменту ЗАКОННО отдал тень своему hydro-шагу
  (`yieldedToStep`), v-show-раунд возвращал её ему, а вотчер yield'а — true с
  обеих сторон — больше никогда не срабатывал, чтобы снять. Гварды:
  `tests/e2e/console-delta-card-advance.spec.ts` § nested repeat selector
  (не-владелец не приобретает) + `console-repeat-pick.spec.ts` (владелец
  восстанавливает).
