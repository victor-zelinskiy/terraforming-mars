...
## Почему визуалы расходились (корни, не симптомы)
1. **РАЗМЕР КАРТЫ — два независимых источника.** Покупка: fit-движок
   taskhost'а (заполняет вертикаль band'а, до 1.6×ui → сингл ≈1.1–1.3×).
   Добор: ручная лестница `stripZoom` (embedded n≤2 = ровно 1.0×). Отсюда
   «карта добора меньше». Фикс: ОДНА формула `fitRowZoom`
   (`src/client/console/cardStripFit.ts`, юнит-спека) — taskhost переведён
   на неё (чистый рефактор, покупка не сдвинулась), reveal embedded получил
   свой fit (`fitEmbeddedStrip`: та же дисциплина — зона минус ИЗМЕРЕННЫЙ
   хром head/namebar, probe слота на zoom 1 прямой записью в один JS-turn,
   потом прямая запись результата + реактивное зеркало `embedFitZoom`, чтобы
   inline-проба не воевала с Vue-байндингом). Размер равен ПО ПОСТРОЕНИЮ.
2. **ШАПКА — два локальных font-правила.** Оба embedded-титула были
   «supporting voice» .95rem/600 (ит.18), но эталон юзера — полноценный
   заголовок сцены. Теперь ОДНА роль `.con-ws-stage-heading()` (1.3rem/700,
   @con-text) — обе шапки читают миксин; e2e-пробы в ОБОИХ спеках
   ассертят одинаковые computed (fhd: 26px/700) — дрейф ловится.
3. **КОМАНДНАЯ СТРОКА — nav-хинт.** `drawnRevealCommandRun` получил
   `multi` → ведёт `◄► Навигация` В ТОМ ЖЕ слоте, что бар покупки; сингл
   лишь ОПУСКАЕТ хинт (грид не меняется). Спека билдера дополнена.
4. Подпись под картой уже была общей грамматикой (verdict-name + чип) —
   добавлена taken-грамматика (см. ниже).
