...
### Блок B — второй аудит свежим взглядом (реестр P-01…P-38)

Аудит без правок по галерее приёмочного прогона A (33 кадра × 3 профиля × 3 режима), стадии заседания под лупой,
чтение локали целиком; реестр и статусы — журнал § «Блок B». Сделано кодом: 25 позиций (стол «как проголосовали»
без «принимается» до обновления; карта, вернувшаяся из перетасованного сброса, не исчезает под плитой пропуска;
одна грамматика доступа; свои имена партий с Ё; кикеры вердикта/правительства/закрытия по глоссарию; тихий пустой
слот; одна форма бейджа на профиль; лобби объектами; бар board home больше не режет подписи — планировщик мерит
текст; вложенный пикер называет стадию; TV-пол в подвале осмотра; неразрывная строка награды задания). Доказано,
не дефект: 8 (X на зоне голосования, плашка партии, высота панели, подписи A, B интейка, B оплаты, кандидаты
кресла, HUD на пять кресел). В блок C: стенд (R-08/R-22).

### Блок C — «вне скоупа» пересмотрены

Сделано в общем слое с паритетом: стенд — позиция осмотра в подвале, без плиты-счётчика и пейджера «ЛИСТАТЬ», строка
подсказок стенда в семейном регистре и скрыта под осмотром (R-08, R-22, P-26, P-27); полоса статуса при 4+ креслах —
...
Репетиция на dev-примерах кадром (четыре фикстуры `parliament-dev{passive,action}-{vote,assembly}`, тест галереи
«the FAMILIES rehearsal», кадры 31–33b на трёх профилях × трёх режимах): четыре находки закрыты кодом — тихая поза
НАГРАДЫ для пассива/действия («Эффект, пока принята» / «Действие, пока принята · Доступно в «Действиях карт»»,
`quietRewardPose.ts`), тот же кикер в панели голосования вместо «При принятии» над графикой, которая не выплата
(`voteReadingOf`), префикс «Эффект:/Действие:» срезан с переведённой декларации под кикером и первая буква заглавная
(`actionRuleText`),
плакетка правительства называет ЧАСТЬ принятой карты («Действие резолюции» над действием, `enactedOwnPart`).
`quietResolutionOf` через dev-заместителей (Красные, Учёные), эскизы семи будущих видов наград в `rewardAddress.ts`
+ чек-лист, контракт § 9 · THE SEAM. Открыто: Союз без dev-заместителя (в каталоге нет карты Союза даже среди
dev-примеров).

Приёмка B: e2e-набор парламента + external-draw + playground — **245 / 245** (1.6 ч); фокусные партии (заседание ×3
профиля, галерея пропуска 18/18, vote-fit 21/21, v2/анонс/стабильность/климат) — зелёные; юнит-гейты — журнал
§ «Приёмка блока B».
...
8.5rem (карта сцены резолюции на ~9 % ниже на 1080), осмотр обычных карт — прежняя полоса и прежний размер. Блок B: командный бар ВСЕХ экранов планирует по измеренной
ширине подписей (общий `ConsoleCommandBar`) — там, где оценка недосчитывала, теперь честно отпадает младшая
подсказка вместо среза слов; токен партии в журнале печатает имена шести партий Redux через ключи парламента.
