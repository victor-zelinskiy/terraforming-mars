...
| 2, 3 | **standard step** — still deal N, pick exactly 1 (min:1/max:1 untouched!). The dealt pool excludes all lineage corps + Merger + bot's corp original (§6.5). The console corp step additionally shows the lineage as a «Текущие корпорации» shelf (LT = fullscreen read) | **STAGED deployment chain** (`runCampaignDeploymentChain`, amended 2026-09-03/04, CANONICAL order): press 1 «КОРПОРАЦИЯ» plays the BASE only (lineage[0]; mission 1: the pick); press 2 «БОНУС КАМПАНИИ» (marker `campaignBonus`, `bonus:{megaCredits}`) — the comeback 0/5/10/15 M€ as its own press with the resource-transfer flight + delta chip (serialized `campaignBonusGranted`; no longer granted inside the scoped corp play); press 3+ «СЛИЯНИЕ» ×N — ONE press per ADDITIONAL corporation (lineage[1..] then the pick; missions 3–4 merge and pay for corp №3 AGAIN) (marker `corporationMerge`) — each plays its corp ON TOP (starting M€/effects at THAT press), **then the Merger prelude's own fee is charged right there, AFTER the corp's resources** (D3 REVISED: «Then pay 42 M€» — `Merger.mergerCost` through the same `SelectPaymentDeferred`, so the new corp's own capital can pay its merge; auto-pays when only M€ can pay, raises the real payment panel for alt-payment lineages; serialized `Player.campaignMergeFeesPaid` COUNTER lets recovery re-raise exactly the unpaid fees; `campaignStartingBudget` subtracts fee×(corps−1); `CAMPAIGN_MERGE_COST` in common mirrors it for the client wizard budget — bonus + ALL starting M€ − ALL fees — guard-spec-pinned); then «ОПЛАТА» (when cards were bought) — deliberately AFTER every merge via `cardPaymentPriority: Priority.DISCARD_AND_DRAW`: the hand is budgeted against the WHOLE merged stack, so the bill only arrives once every corporation's capital has assembled and every fee is paid; last «НАСЛЕДИЕ» (marker `campaignLegacy`, `Priority.BACK_OF_THE_LINE`) grants the carried cards — NO reveal batch: the owner sees them FACE UP in the deployment queue from the first frame (self-model `campaignCarriedCards`) and the press flies them into the hand dock with the SAME delivery flight as the bought projects. One research release drains the chain; reload recovery = `campaignSetupResumeInput` (base → bonus → merge → fees → legacy from the tableau + serialized flags). The console queue PRE-LAYS every element in activation order from the first frame (base · bonus plate · every merge corp dimmed with its fee badge · purchase row · legacy row · preludes) |
| 4 | **corp step omitted** (new `SelectInitialCards` option; preludes/CEOs/projects steps untouched — prologues are never cut) | the SAME canonical chain: «Штаб» base press (lineage[0]) → bonus → «СЛИЯНИЕ» ×2 for lineage[1], lineage[2] — each charging the 42 M€ fee AGAIN → payment → «НАСЛЕДИЕ» (carried from mission 3) |
