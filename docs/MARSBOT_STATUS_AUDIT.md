...
### ⭐ ПЕЙСИНГ ХОДА: сервер отвечает за ~1 мс, ждал КЛИЕНТ (2026-08-17)

Жалоба «MarsBot иногда думает ~5 секунд даже на локальном сервере» — целиком клиентская.
Измерения: `AutomaController.takeTurn` = 0.2–17 мс (медиана 0.72 мс на 40 ходов);
серверный `BotTurnScheduler` = 200 мс + ограниченные расширения.

Причина: **staging включался на КАЖДЫЙ ход**. Окно буферизует авторитетный view (вместе с
СОБСТВЕННЫМ промптом игрока), пока карточка хода не будет ДОСТАВЛЕНА, а доставка ждёт
глушащих условий ленты (`SILENCING_REASONS`: живая кинематика, полноэкранный reveal, театр,
церемония) — то есть ровно ту анимацию, которую запустило действие самого игрока. Одиночному
ходу секвенировать нечего: его footprint И ЕСТЬ авторитетный view.

Правило теперь структурное (`presentFreshBotTurns`, критерий — `waitingFor`, никогда не текст):
- **`isBotStagingActive() || fresh.length > 1 || !handsControlBack` → секвенируем.**
  Окно уже открыто (порядок нельзя рвать), несколько ходов в ОДНОМ ответе (пас → сервер
  разрешил весь круг), или бот ещё играет (промпта у игрока нет — буфер ничего не стоит).
- иначе — **коммит немедленно**, карточка едет обычной лентой.

Плюс **B по карточке хода схлопывает ВЕСЬ бэклог** (`skipBotTurnPresentation`): ack + доставка
визуала каждого очередного хода + коммит. Раньше консольный B звал голый `dismiss` — он даже
ack не слал, так что сервер продолжал пейсить следующий ход на клиенте, который уже всё увидел.

### ⭐ РАУНД 2 (2026-08-18): «лагает 5–10 с, ОСОБЕННО когда я в workspace»

Воспроизведено e2e (`tests/e2e/console-bot-turn-latency.spec.ts`, сценарий «пас →
бот играет круг → игрок всё это время в workspace»): **92 секунды** до возврата
управления. Зонд `window.__foregroundDiag()` назвал держателей поимённо. Три
независимые причины, все клиентские:

1. **Серия ходов дренировалась ПО КАРТОЧКАМ.** Окно коммитило на доставку
   ПОСЛЕДНЕЙ карточки, карточки показываются по одной и каждая живёт свой
   `BOT_TURN_TTL`. Замер: карточки становились видимы на 4.4 / 7.7 / 31.2 /
   42.5 с, коммит — на 42.5 с. Теперь у серии СВОЙ темп (`RUN_STEP_MS = 900`,
   `marsBotStagedCommits`): доска проходит ход за ходом, а коммит не ждёт
   чтения. И пейсинг существует, чтобы на него СМОТРЕЛИ — пока стоит workspace,
   доска закрыта, поэтому окно вообще не открывается (`workspaceStackActive`).
2. **Бэклог тостов = бэклог на игроке.** Видимая flow-holding карточка поднимает
   `presentation` (блокирует ВСЕ prompt-поверхности) и держит
   `notificationsSettled()` ложным, из-за чего собственное решение игрока даже не
   АНОНСИРУЕТСЯ. Очередь бот-карточек теперь ограничена
   (`MAX_QUEUED_BOT_CARDS`): лишние сливаются (визуал применён, ack отправлен,
   журнал хранит всё), и вместе с ними снимается серверная пауза «ждём, пока
   прочитают».
3. **⚠️ Приземление тайла держало передний план, ПОКА ЖДАЛО доску.**
   `isRemotePlacementActive` считал ход в очереди живой анимацией, а
   `awaitWatchableBoard` ждёт до `BOARD_WAIT_MAX_MS` = **20 с**, пока игрок
   стоит в workspace. То есть ход бота с тайлом + открытый workspace = блокирующий
   animation-hold на всё ожидание: ни одна поверхность не монтируется, лента
   заглушена. Это и есть «особенно когда я в workspace». Хвататься надо за ПОЛЁТ,
   а не за ОЖИДАНИЕ — добавлен `remotePlacementState.waitingForBoard`.

Результат того же замера: **92 000 мс → ~9.8 с**, из которых держателей нет вовсе
(timeline пуст) — это реальные 5 ходов бота + производство + смена поколения.
Коммит каждого хода: 0–3 мс. Одиночный ход из workspace: 1355 мс против 1220 мс с
доски (разницы нет).

### ⭐ РАУНД 3 (2026-08-19): «после розыгрыша карты — снова до 5 секунд»

Воспроизведено e2e (сценарий: 2-я карта хода — её confirm заканчивает ход, ход бота
приезжает на фоне кинематики розыгрыша): **21.4 секунды** до возврата управления,
при свободном экране уже с 3.8 с. Пас/пропуск чисты, потому что у них нет кинематики.
Два пролома, оба — «отказ от refresh = потеря», и оба закрыты как «отказ = ДОЛГ»:

1. **Poll-путь выбрасывал полученную модель.** `App.update` отказывается коммитить
   view, пока жива одна из восьми сцен (`isPlayedHeroActive` и др.) — правильно, но
   модель ВЫБРАСЫВАЛАСЬ («следующий poll закоммитит»), а при здоровом WS следующий
   fallback-poll — через ~20 с, и единственный `GAME_STATE_INVALIDATED`-wake уже
   потрачен на эту неудачную попытку. Теперь отказ паркует retry
   (`deferredViewRefresh.ts`): перепроверка каждые 200 мс, `update()` перезапускается
   на грани освобождения сцен; ограничено 15 с с warn (застрявший флаг сцены — уже
   дефект со своим потолком, немой отказ хуже устаревшего view). Нормальный коммит
   гасит припаркованный retry.
2. **GO съедался устаревшим `viewerHasPrompt`.** Submit-ответ розыгрыша держится всю
   hero-сцену (`await runPlayedHero`), и wake, прилетевший в это окно, видел СТАРЫЙ
   промпт → «viewer mid-input» → refresh пропущен, wake потрачен. Теперь `.finally`
   каждого `fetchPlayerInput` вызывает `waitForUpdate(true)`: конец обработки
   submit'а — единственный момент, когда staleness гарантированно кончилась; на
   обычном submit'е перепроверка стоит один WAIT-раундтрип.

Итог того же замера: **21 153 мс → 3 095 мс** (кинематика + серверный пейсинг; таймлайн
держателей пуст, commit каждого хода ≤1 мс). Гард: `console-bot-turn-latency.spec.ts`
§ «a play that ends the turn» (острая проверка — per-turn commit < 1.5 с; потолок 12 с
против бага в 21.4 с). Юнит: `tests/client/deferredViewRefresh.spec.ts`.

**Диагностика:** `window.__foregroundDiag()` — что именно держит экран
(reason / claims / animation holds / raw admission signals / длина очереди).
`marsBotTurnTiming.ts` — стадии `response → commit → visible → released`
на каждый ход; печатает при `?botTiming=1` (или `tm_bot_timing=1`), сам предупреждает при
commit > 1.5 с, и отдаёт `window.__botTurnDiag()` для e2e. Измерено после фикса
(`tests/e2e/console-bot-turn-latency.spec.ts`): одиночный ход `commit: 1 ms` при
`released: 3969 ms` — те самые 5 секунд, к которым коммит был привязан; серия после паса
по-прежнему `sequenced`; B даёт `commit: 433 ms · "with the batch"`.
