# AUTOMA_DATA_AUDIT — источники данных MarsBot и статусы верификации

Обновлено: 2026-07-07. Официальные источники: **RB-A** = TM-Automa rulebook A (08-15-2023, база, 12 стр.),
**RB-C** = TM-Automa rulebook C (11-14-2023, Adding Expansions, 16 стр.), **SG** = Setup Guide v1.3 (10-10-2024).
Извлечение текста: `pdftotext` (Git mingw64). Извлечение изображений компонентов: `pypdfium2` рендер
RB-A p.2 (страница компонентов) в масштабе 22–30, покомпонентные кропы.

## 1. Нумерация бонус-карт — КОНФЛИКТ ИСТОЧНИКОВ (решено: canonical identity = имя карты)

RB-C (11-2023) и SG v1.3 (10-2024) расходятся в B-номерах Colonies/Turmoil-карт, причём RB-C
противоречит сам себе (Government Intervention = B16 в Venus-секции И Colonies Expedited
Construction = B16 в Colonies-секции):

| Карта | RB-C | SG v1.3 | RB-A (компоненты) | **Canonical (наш enum)** |
|---|---|---|---|---|
| Lobbyists (Venus) | B15 | B15 | — | `B15_LOBBYISTS_VENUS` |
| Government Intervention | B16 | B16 | **B16** (p.6, анатомия карты) | `B16_GOVERNMENT_INTERVENTION` |
| Expedited Construction (Colonies) | ~~B16~~ | B17 | — | `B17_EXPEDITED_CONSTRUCTION_COLONIES` |
| Outer System Foothold | ~~B17~~ | B18 | — | `B18_OUTER_SYSTEM_FOOTHOLD` |
| Shipping Lines | ~~B18~~ | B19 | — | `B19_SHIPPING_LINES` |
| Extended Shipping Lines | ~~B19~~ | B20 | — | `B20_EXTENDED_SHIPPING_LINES` |
| Party Politics | ~~B20~~ | B21 | — | `B21_PARTY_POLITICS` |

RB-A p.6 печатает Government Intervention с номером **B16** → нумерация SG v1.3 согласована с
физическими компонентами; RB-C содержит опечатку сдвига. **Поведение карт привязано к имени
(enum-член), номер — только отображение.** `BonusCardId` в `src/common/automa/AutomaTypes.ts`
следует SG v1.3.

## 2. Tharsis MarsBot board (`src/server/automa/boards/TharsisMarsBot.ts`)

Статус: **принят как верный baseline** (решение владельца проекта, 2026-07-07). Повторная
физическая сверка НЕ блокирует POC. Функциональное поведение покрывается тестами против RB-A.

## 2a. Hellas MarsBot board (`src/server/automa/boards/HellasMarsBot.ts`)

Статус: **транскрибирован покадрово** с официального компонента «Hellas MarsBot board»
(late PnP / low-ink лист, «compatible with rules v1.16+», предоставлен владельцем 2026-08-26).
Каждая из 7 × 19 клеток закреплена отдельным ассертом в
`tests/automa/HellasMarsBotBoard.spec.ts` — сдвиг ОДНОЙ иконки падает с её координатами.

**Раскладка треков (сверху вниз, как напечатано):** Building (сталь) · Space (титан) ·
Event (M€) · **Jovian+Science** (без производства) · Power (энергия) · City+Earth (тепло) ·
Microbe/Animal/Plant (растения).

**Единственное отличие тегов от Tharsis — официальное** (RB-C p.8): *«Hellas and Elysium: the
[Jovian] tag is paired with [Science], not with [Power] (as it is on the Tharsis board)»*.
Производственные регрессии при этом НЕ меняются — они идут по печатному значку производства
на клетке 0 каждого трека (`productions` в определении), а не по паре тегов.

**Полный diff клеток против Tharsis** (закреплён тестом):

| Трек:клетка | Tharsis | Hellas |
|---|---|---|
| building:11 | `tag_1` (продвинуть Space) | пусто |
| event:6 | `venus2` | `floater2` |
| science:2 | пусто | `floater` |
| science:5 | пусто | `floater2` |
| power:2 | `venus` | `advance` |
| power:4 | `venus2` | пусто |

**Значки расширений на клетках.** Клетка Space 7 несёт ТОЛЬКО синий бейдж «V» (дуга шкалы
Венеры) → `venus`, игнорируется без Venus Next. Клетки-флоатеры (science 2/5, event 6) несут
«V» **и** тёмный треугольник Colonies → работают при Venus **или** Colonies («Gain Floater: if
playing without Venus Next, place a resource token in the Titan storage area», RB-C p.4), и
игнорируются без обоих (RB-A p.7, значок неиспользуемого расширения). Клетка Power 9
дополнительно печатает жетон второго Trade Fleet — это и есть «9th space of the [power] track»
из RB-C p.4/p.6.

⚠️ Venus-ряд с предоставленного low-ink листа НЕ импортирован: трек Венеры остаётся отдельной
подсистемой (`VenusMarsBot.ts`), приклеиваемой 8-м треком к любой карте.

## 2b. Elysium MarsBot board (`src/server/automa/boards/ElysiumMarsBot.ts`)

Статус: **транскрибирован покадрово** с официального компонента «Elysium MarsBot board»
(«compatible with rules v1.16+»); семь основных треков сверены клетка-в-клетку с retail-компонентой
(владелец, 2026-08-29). Каждая из 7 × 19 клеток закреплена отдельным ассертом в
`tests/automa/ElysiumMarsBotBoard.spec.ts` — сдвиг ОДНОЙ иконки падает с её координатами.

**Раскладка треков (сверху вниз, как напечатано):** Building (сталь) · Space (титан) ·
Event (M€) · **Jovian+Science** (без производства) · Power (энергия) · City+Earth (тепло) ·
Microbe/Animal/Plant (растения). Тот же порядок строк, что у Tharsis и Hellas, — это и держит
клетку 9 трека Венеры (`tag_6`) нацеленной на bio-строку на любой карте (пин в спеке).

**Пара тегов — как у Hellas, официально** (RB-C p.8): *«Hellas and Elysium: the [Jovian] tag is
paired with [Science], not with [Power]»*. Производственные регрессии НЕ меняются: они идут по
печатному значку производства на клетке 0 (`productions`), поэтому энергия по-прежнему откатывает
Power, а не Science.

**Полный diff клеток против Hellas** (закреплён тестом):

| Трек:клетка | Hellas | Elysium |
|---|---|---|
| building:4 | пусто | `tag_4` (продвинуть Power) |
| building:5 | `tr2` | пусто |
| building:13 | пусто | `tag_4` (продвинуть Power) |
| space:11 | `tr3` | `tr4` |
| space:16 | `tr4` | `tr5` |
| event:12 | `tr4` | `tr5` |
| science:7 | пусто | `tag_4` (продвинуть Power) |
| science:8 | `tr2` | `tr3` |
| science:12 | `tr3` | `tr4` |
| science:14 | `tr4` | `tr5` |
| power:2 | `advance` | пусто |
| earth:4 | `tr3` | пусто |
| bio:12 | `tr3` | `tr4` |

**Круглая тег-иконка = «продвинуть трек этого тега»** (та же нотация, что на Tharsis building:11 и
на клетке 9 доски Венеры). Elysium печатает ТРИ такие клетки, все три — значок POWER
(building 4/13, science 7); спек отдельно пинит и их количество, и цель.

**Значки расширений на клетках.** Space 7 несёт только синий бейдж «V» → `venus`. Клетки-флоатеры
(science 2/5, event 6) несут «V» **и** треугольник Colonies → работают при Venus **или** Colonies,
игнорируются без обоих. Power 9 дополнительно печатает жетон второго Trade Fleet — «9th space of
the [power] track» (RB-C p.4/p.6).

⚠️ **Что с листа НЕ переносится** (печатные напоминания старше текущего рулбука, и всё это уже
централизовано): встроенный 8-й ряд Венеры (остаётся `VenusMarsBot.ts`, приклеиваемый 8-м треком),
статичная рамка «FAILED ACTIONS = 5» (решает сложность: 3 M€ на Easy, 5 иначе), «REMOVE ANY
RESOURCES» и таблица финального пересчёта M€ (`AutomaScoring`). Сверка Venus-ряда с листа с
`VENUS_TRACK` выполнена и совпала — это подтверждение существующих данных, а не новый источник.

## 2c. Utopia Planitia MarsBot board (`src/server/automa/boards/UtopiaMarsBot.ts`)

Статус: **транскрибирован покадрово** с официального компонента «Utopia Planitia MarsBot board»
(«compatible with rules v1.16+»); семь основных треков сверены клетка-в-клетку с retail-фотографией
(владелец, 2026-08-29). Каждая из 7 × 19 клеток закреплена отдельным ассертом в
`tests/automa/UtopiaMarsBotBoard.spec.ts`.

**Раскладка треков (сверху вниз, как напечатано):** Building (сталь) · Space (титан) ·
Event (M€) · **Science** (без производства) · **Jovian+Energy** (энергия) · Earth+City (тепло) ·
Plant/Animal/Microbe (растения). Тот же порядок строк, что у Tharsis/Hellas/Elysium.

⚠️ **Пара тегов — как у THARSIS, а не как у Hellas/Elysium.** RB-C p.8 называет ровно две карты:
*«Hellas and Elysium: the [Jovian] tag is paired with [Science]»* — Утопии в списке нет, поэтому
здесь Jovian едет на треке Power, а Science стоит отдельно. Производственные регрессии, как всегда,
идут по печатному значку на клетке 0: энергия откатывает **Jovian+Energy**, тепло — **Earth+City**.

**Полный diff клеток против Tharsis** (закреплён тестом):

| Трек:клетка | Tharsis | Utopia |
|---|---|---|
| building:11 | `tag_1` (продвинуть Space) | `tag_7` (продвинуть **Венеру**) |
| space:1 | `advance` | пусто |
| space:4 | пусто | `advance` |
| space:5 | `ocean` | **`colony`** (Place a Colony) |
| space:10 | пусто | `ocean` |
| space:11 | `tr3` | `tr2` |
| event:6 | `venus2` | `floater2` |
| science:3 | `advance` | пусто |
| science:5 | пусто | `greenery` |
| science:6 | `greenery` | `milestone` |
| science:9 | `milestone` | `advance` |
| power:2 | `venus` | `floater` |
| power:4 | `venus2` | `floater2` |
| bio:7 | `advance` | пусто |

**Две новые для движка печатные иконки.**

1. **`colony` (space 5) — «Place a Colony».** Резолвится обычным ботовым конвейером колоний
   (`AutomaColonies.botBuildColony`, тот же, что у B17/B18): случайный выбор плитки методом
   флипа, 2 ресурса в складскую зону, замена Европы океаном, триггеры «any player built a colony»
   и хук корпорации. Нет подходящей плитки → Failed Action (`no-colony-tile`). Без модуля Colonies —
   иконка неиспользуемого расширения: игнорируется, без Failed Action (RB-A p.7).
2. **`tag_7` (building 11) — круглая тег-иконка ВЕНЕРЫ.** Та же нотация «продвинуть трек этого
   тега», но цель — приклеиваемый 8-м трек Венеры. Без Venus Next такого трека нет: `performTrackAction`
   проверяет диапазон и игнорирует клетку (тоже RB-A p.7). Тест пинит и номер строки, и оба режима.

**Значки расширений на клетках.** Space 7 — синий бейдж «V» → `venus`. Клетки-флоатеры
(power 2/4, event 6) несут «V» **и** треугольник Colonies → работают при Venus **или** Colonies.
Power 9 печатает жетон второго Trade Fleet — «9th space of the [power] track» (RB-C p.4/p.6).

⚠️ **Что с листа НЕ переносится:** встроенный ряд Венеры, «FAILED ACTIONS = 5», «REMOVE ANY
RESOURCES», таблица финального пересчёта M€. Плюс **устаревшая панель вех самого листа**: она печатает
Metallurgist как «tracks' spots (combined) at **5**», тогда как текущий рулбук говорит **7**.
Реализована семёрка — приоритет у современного официального текста, а не у напоминания на компоненте.

## 2d. Terra Cimmeria MarsBot board (`src/server/automa/boards/TerraCimmeriaMarsBot.ts`)

⚠️ **КАКАЯ ИМЕННО «Terra Cimmeria».** В форке ДВЕ карты с этим именем, и официальная
Automa-карта — это **`BoardName.TERRA_CIMMERIA_NOVA`**: именно она печатает гекс MSL Curiosity
(`SpaceBonus.COLONY`) и несёт ряд вех/наград карточки B12 (Electrician · Founder · Mogul ·
Zoologist · Forecaster). Старая `TERRA_CIMMERIA` — другая карта: другой ряд M&A, нет колониального
гекса; она НЕ поддерживается (пин в `AutomaCompatibility.spec.ts`).

Статус: **транскрибирован покадрово** с официального компонента «Terra Cimmeria MarsBot board»
(«compatible with rules v1.16+», предоставлен владельцем 2026-08-29). Каждая из 7 × 19 клеток
закреплена отдельным ассертом в `tests/automa/TerraCimmeriaMarsBotBoard.spec.ts`.

**Раскладка треков (сверху вниз, как напечатано):** Building (сталь) · Space (титан) · Event (M€) ·
**City+Science** (без производства) · **Energy** (энергия, в одиночку) · **Jovian+Earth** (тепло) ·
Microbe/Animal/Plant (растения). Порядок строк тот же, что у всех карт — это и держит клетку 9
трека Венеры нацеленной на bio-строку, и это же официальный порядок «сверху» для тайбрейка Mogul.

⚠️ **Топология тегов уникальна для этой карты** — сдвинуты сразу три пары: City уезжает к Science,
Jovian к Earth, Energy остаётся один. Производственные регрессии не меняются: они идут по печатному
значку на клетке 0, поэтому энергия по-прежнему откатывает **Energy**, а тепло — **Jovian/Earth**.

**Полный diff клеток против Tharsis** (закреплён тестом):

| Трек:клетка | Tharsis | Terra Cimmeria |
|---|---|---|
| building:4 | пусто | `tag_4` (продвинуть Energy) |
| building:5 | `tr2` | пусто |
| building:11 | `tag_1` (Space) | пусто |
| building:13 | пусто | `tag_4` (продвинуть Energy) |
| space:11 | `tr3` | `tr4` |
| space:16 | `tr4` | `tr5` |
| event:6 | `venus2` | `floater2` |
| science:3 | `advance` | `city` |
| science:4 | `city` | пусто |
| science:5 | пусто | `city` |
| science:7 | пусто | `tag_2` (продвинуть Event) |
| science:8 | `tr2` | `milestone` |
| science:9 | `milestone` | пусто |
| science:10 | `temperature` | `city` |
| power:2 | `venus` | пусто |
| power:4 | `venus2` | пусто |
| power:7 | `milestone` | `award` |
| earth:2 | пусто | `floater` |
| earth:4 | `tr3` | `temperature` |
| earth:5 | `city` | `floater2` |
| earth:8 | `city` | пусто |
| bio:10 | `award` | `milestone` |
| bio:12 | `tr3` | `tr4` |

**Три круглые тег-иконки:** building 4/13 — значок ENERGY (`tag_4`), science 7 — значок EVENT
(`tag_2`). Клеток `colony` на этой карте НЕТ: колония здесь — это ГЕКС доски (MSL Curiosity), а не
действие трека, как на Утопии. Спек пинит и это.

**Значки расширений.** Space 7 — `venus`; клетки-флоатеры (earth 2/5, event 6) работают при Venus
**или** Colonies; power 9 печатает жетон второго Trade Fleet.

⚠️ **Что с листа НЕ переносится:** встроенный ряд Венеры, «FAILED ACTIONS = 5», «REMOVE ANY
RESOURCES», таблица финального пересчёта M€.

## 3. Venus Next MarsBot board (`src/server/automa/boards/VenusMarsBot.ts`)

Статус: **транскрибирован** с RB-A p.2 (кроп «1 Venus Next MarsBot board», масштаб 26–30).
Один трек VENUS, позиции 0–12 (`maxPosition: 12`):

| Поз. | Иконка на компоненте | Действие |
|---|---|---|
| 0 | — (старт, клетка с маркером V) | — |
| 1 | одиночное жёлтое облако | Gain 1 floater |
| 2 | сдвоенное жёлтое облако | Gain 2 floaters |
| 3 | дуга Venus-шкалы | Raise Venus 1 step |
| 4 | сдвоенное облако | Gain 2 floaters |
| 5 | дуга Venus | Raise Venus 1 |
| 6 | — | — |
| 7 | сдвоенное облако | Gain 2 floaters |
| 8 | дуга Venus | Raise Venus 1 |
| 9 | **круглая тег-иконка: бактерия (microbe)** | Advance another track → Bio (верифицировано) |
| 10 | дуга Venus | Raise Venus 1 |
| 11 | сдвоенное облако | Gain 2 floaters |
| 12 | красный квадрат «5» (иконка Gain TR из легенды RB-A p.8) | Gain 5 TR |

Проверка согласованности: суммарные флоатеры за полный трек = 1+2+2+2+2 = 9 ≥ 7 (Hoverlord). ✓

- **ВЕРИФИЦИРОВАНО (2026-07-07, владелец, физический компонент):** поз. 9 — метка
  **бактерии (Microbe)** → Bio-трек; альтернативное прочтение Jovian → Energy исключено.
  Кодировка `VENUS_CELL9_TARGET_TRACK = 6` (Bio) в `VenusMarsBot.ts` подтверждена, TODO снят.
- **Note:** иконка floater-storage зоны справа от трека на компонент-миниатюре читается как
  «5 [зелёный квадрат с лапой] → +[карта]» — похоже на арт ранней ревизии (animal-тег вместо
  floater-иконки). Поведение реализуется ПО ТЕКСТУ RB-C p.2 (однозначен): 5 floaters →
  дополнительная карта action deck в Research Phase. На геймплей иконка не влияет.

## 4. Colonies Shipping Board (`src/common/automa/ShippingBoardData.ts`)

Статус: **транскрибирован полностью** (RB-A p.2, кропы построчно, масштаб 22; текстовые правила
RB-C pp.4–5 совпадают с печатной сводкой на самом планшете — жёлтая сноска «COLONY INTERACTION
EFFECTS» прочитана и совпадает дословно).

| Область | Хранимый ресурс | «5 → тег» (обмен → трек) |
|---|---|---|
| Ceres | сталь | Building |
| Luna | M€ | Event |
| Io | тепло | Earth |
| Enceladus | микробы | Microbe (Bio) |
| Ganymede | растения | Plant (Bio) |
| Callisto | энергия | Power (Energy) |
| Miranda | животные | Animal (Bio) |
| Triton | титан | Space |
| Pluto | **научные ресурсы** (science) | Science |
| Titan | флоатеры | — (обмена нет; floaters по Venus-правилам) |
| Europa | — (не хранит) | — (build→океан+TR / trade→+1 TR / colony bonus→+1 M€) |

**ИСПРАВЛЕНИЕ (2026-09-12, повторная сверка с RB-C p.5 + кропом планшета RB-A p.2):**
Pluto хранит НАУЧНЫЕ ресурсы, не «карты»-суррогат (прежняя транскрипция была ошибочной).
Дословно RB-C p.5: «MarsBot does not gain cards for **Pluto**. Instead it gains [science]
resources into the corresponding storage area» — в предложении напечатана иконка научного
ресурса (атом), и сама область на планшете печатает «5 [science] → [science tag]» (карты
печатает только Titan: «5 [floater] → +[card]»). При этом Pluto НАМЕРЕННО отсутствует в
steal/remove-списке той же страницы («Resources on Ceres, Luna, Io, Enceladus, Ganymede,
Callisto, Miranda, and Triton are considered to be resources of the indicated type for the
purposes of your cards. You may steal/remove from them as usual») — науку Плутона игрок
таргетить НЕ может; движок (`AutomaTargeting`) это уже реализует отсутствием маппинга.

Нотация подтверждена: круглая тег-иконка = «advance track, соответствующий тегу» (та же
нотация, что у Advance Another Track). Второй trade fleet: SG «space 9 of the power track» —
для Tharsis это Energy track, клетка 9 (RB-C p.6: «in addition to resolving the space's
effect» — эффект клетки 9 Energy-трека Tharsis (`temperature`) резолвится, флот разблокируется,
Extended Shipping Lines подмешивается со следующего поколения).

## 5. Полные тексты бонус-карт

| Карта | Полный текст | Источник |
|---|---|---|
| B01–B08 (base) | ✓ | RB-A pp.6–7 |
| B15 Lobbyists (Venus) | ✓ (4 ветки: temp / oxygen / **venus 2 steps без destroy** / furthest-Martian; ocean-ветки base-версии НЕТ) | RB-C p.3 |
| B16 Government Intervention | ✓ (чёт/Venus-complete → furthest MARTIAN param [tie: O₂→ocean→temp]; иначе Venus+1; без TR/MC вкл. каскадные бонус-подъёмы) | RB-C p.3 + RB-A p.6 |
| B17 Expedited Construction (Colonies) | ✓ (city≥2 greenery/ocean → destroy; иначе если колоний ≤1 → построить колонию flip-методом + 2 ресурса, БЕЗ destroy; иначе ничего) | RB-C p.4 |
| B18 Outer System Foothold | ✓ (колония flip-методом + 2 ресурса; затем draw из BONUS deck и discard без резолва — reshuffle без самого OSF) | RB-C p.5 |
| B19/B20 Shipping / Extended Shipping Lines | ✓ (тайл с самым продвинутым треком → tie: где колония бота → tie: flip-метод; −1 MC и trade) | RB-C p.5 |
| B21 Party Politics | ✓, но Turmoil вне POC | RB-C p.6 |
| B09–B14, B22–B32 | вне POC (карты/Awards&Milestones/корпорации) | — |

## 6. Venus-правила, закрытые RB-C (бывшие блокеры B1/B4)

- Setup: Venus board + clear cube на 0; B06→B15; B16 set aside; **B16 подмешивается в action deck
  каждое поколение, ВКЛЮЧАЯ ПЕРВОЕ** (RB-C p.2 «including on the first round» — уточнение к SG).
- **Floater-spend в конце Research Phase** (если Hoverlord недоступен И ≥5 флоатеров): драфт —
  оставить 4-ю карту вместо дискарда; не-драфт — получить 4-ю карту из project deck; Brutal — 5-ю.
- Venus tag → Venus track; трек ведёт себя идентично прочим (включая Failed при maxed).
- **Solar Phase Step 2 (WGT) НЕ выполняется** — его роль играет B16; MarsBot+Venus без B16 нельзя.
- Planner (Tharsis): все треки ≥4 **кроме Venus** (подтверждено RB-C p.2).
- Hoverlord: 7 флоатеров; в leftmost-тайбрейке считается ПОСЛЕДНИМ. Venuphile: позиция Venus-трека;
  в leftmost-тайбрейке ПОСЛЕДНЯЯ.
- Corporate Competition получает helper «Venuphile: advance Venus track» во всех версиях.
- **Открытый вопрос OQ-7:** бонус-шаги шкалы Венеры 8% (card draw) для бота RB-C не оговаривает.
  16% (+1 TR) покрыт «TR per the normal rules». Принято: 8%-бонус к боту не применяется (у бота
  нет руки; явной замены не прописано) — пересмотреть при появлении официального разъяснения.

## 7. Colonies-правила, закрытые RB-C (бывший блокер B2)

- Setup: 2p-раскладка, но ВСЕ тайлы (вкл. Titan/Enceladus/Miranda) стартуют трекером на
  выделенной 2-й клетке; B05→B17; B18 в bonus deck; Shipping Board; 2 trade fleet бота
  (второй на Energy-track-9); B19/B20 set aside.
- Research: со 2-го поколения в action deck подмешивается B19 (+B20 после разблокировки флота).
- Storage-переполнение: в ЛЮБОЙ момент хода бота ≥5 ресурсов в области (кроме Titan) → −5,
  advance трека по тегу области.
- Трейды/колонии/Europa/Pluto/Titan — см. §4 и RB-C p.5 дословно.
- Floater-spend действует и без Venus («assuming Hoverlord is no longer available»).
- Human steal/remove МОЖЕТ таргетить storage-ресурсы бота как ресурсы соответствующего типа —
  но ТОЛЬКО области из списка RB-C p.5 (Ceres, Luna, Io, Enceladus, Ganymede, Callisto,
  Miranda, Triton); Pluto (science) в списке отсутствует и не таргетится.

## 8. Печатный порядок тегов (решение)

Canonical = порядок массива `tags[]` карты — ровно то, что продукт рендерит игроку
(`CardTags.vue`, слева-направо). MarsBot резолвит теги в отображаемом порядке. Полный аудит всех
карт НЕ блокирует POC; очевидные расхождения in-scope мульти-теговых карт правятся точечно.
Event-карты: `Tag.EVENT` не хранится в `tags[]` (тип карты) — резолвер добавляет event-тег
согласно печатной позиции (последним). Покрыть тестом.

## 9. Официальный FAQ (RB-A p.11) — card-specific правила

Транскрипция официального FAQ (rulebook A, p.11, скриншот подтверждён владельцем 2026-07-19).
Это ЕДИНСТВЕННЫЙ официальный источник пер-карточных правил; ничего сверх него не выдумывать —
всё остальное = явный хоумрул (см. `AUTOMA_PROMO_MULTIPLAYER_FRAME.md`).

### Project Cards (promo-пакет реализован 2026-07-19; тесты = `tests/automa/AutomaPromoCards.spec.ts` + `AutomaBonusCards.spec.ts`)

| Карта | Модуль | Официальное правило | Статус в коде |
|---|---|---|---|
| **Asteroid Deflection System** | Promo | DOES block MarsBot's bonus card *Meteor Shower* from forcing you to remove the respective resources | ✅ авто: `Player.plantsAreProtected()` включает ADS; B01 проверяет `plantsAreProtected()`. Тест есть |
| **Galilean Waystation** | Colonies | +1 MC production за каждый свой Jovian tag **+ half (rounded down) MarsBot's Jovian track position** | ✅ реализовано: `AutomaTargeting.automaTagCount` (FAQ-ветка `GALILEAN_WAYSTATION`) через `Counter.ts` |
| **Lawsuit** | Promo | You steal 3 resources from MarsBot; card goes into MarsBot's played pile; MarsBot does **not** resolve the icons; MarsBot does **not** lose points from the card | ✅ реализовано: co-located ветка в `LawSuit.bespokePlay` (steal через `Player.attack`-адаптер — supply→Luna; карта в `bot.playedCards`); карточные ПО бота считает ТОЛЬКО `AutomaScoring` — цикл card-VP в `calculateVictoryPoints` для бота отключён, `automa.playedPile` карту не видит. Атрибуция: B01 (`from: {player: bot}`) + B02 (`removingPlayer: bot`) регистрируют бота в `human.removingPlayers` (заодно заработал Crash Site Cleanup) |
| **Protected Habitat** | Base | DOES block MarsBot's bonus cards *Meteor Shower* AND *Invasive Species* | ✅ ДЕФЕКТ ЗАКРЫТ: B01 ✅; B02 теперь фильтрует держателей по Protected Habitats + пер-карточному `protectedResources` (Pets и т.п.), зеркаля `RemoveResourcesFromCard.getAvailableTargetCards` |
| **Sponsored Academies** | Venus | MarsBot gains 1 MC instead of the free card draw | ✅ реализовано co-located (`SponsoredAcademies.ts`, внутри цикла по opponents) |
| **St. Joseph of Cupertino Mission** | Promo | Cathedral на городе бота → бот платит 2 MC **if able**; вместо добора карты — advance its **least-advanced track (topmost if tied)** | ✅ реализовано: co-located ветка в `action()` (никакого промпта боту); тайбрейкер = `board.getLeastAdvancedTrackIndex()` + `AutomaResolver.advanceTrack` (тот же путь, что wild-тег и storage-обмен) |
| **Toll Station** | **Base** (не promo!) | Increase your MC production a number of steps equal to **MarsBot's Space track position** (полная позиция, не половина) | ✅ ВЕРИФИЦИРОВАНО авто: декларативный `behavior.production.megacredits = {tag: SPACE, others: true}` → `Counter` → `automaTagCount`. Тест есть |

### Corporations

| Карта | Модуль | Официальное правило | Статус в коде |
|---|---|---|---|
| **Mons Insurance** | Promo | «You may not use this corporation against MarsBot» | ✅ predicate-ban: `src/server/automa/AutomaBans.ts` (`isCardBannedForAutoma`) → фильтр колоды в `GameCards.filterBannedCards` — сознательно НЕ через `gameOptions.bannedCards` (непустой список читается automa-правилами как customLists и ломал бы validateOptions реванша). Только official-solo (НЕ глобальный бан навсегда — frame-док §4); payout-пути (`resolveInsurance`) гейтятся `monsInsuranceOwner` → при бане dormant |

### Отдельно: Recession (Prelude 2)

Официальное правило: Prelude 2 + MarsBot → **Recession удаляется из колоды**. Сейчас покрыто
косвенно (весь prelude2 — automa-конфликт); при будущей адаптации prelude2 к боту правило
переезжает в точечное исключение карты, не в бан модуля.

### Generic-правила взаимодействий (официальные, RB-A p.4–5 + RB-C p.5)

Уже реализованы централизованно и остаются generic target-is-MarsBot адаптерами (НЕ пер-карточными хаками):
- **Remove resources** от бота → MC-supply бота как ресурс соответствующего типа (Colonies storage реального типа — первым). `Player.attack` → `AutomaTargeting.removeFromBot`.
- **Steal resources** от бота → из MC-supply; человек получает **тип ресурса, названный картой**, не обязательно MC. Тот же путь (`stealing`-ветка).
- **Decrease production** бота → регресс соответствующего трека на 1 за ступень. `Production.add` (amount<0) → `AutomaTargeting.regressForProduction`.
- **Положительное production боту** (официального правила НЕТ) → громкий throw-guard в `Production.add` (frame §5) — никакой молчаливой записи в мёртвые поля.

## 10. Корпорации MarsBot (Rule Book B «Adding Corporations», 08-15-2023)

Источники: RB-B полностью (PDF с fryxgames.se, транскрибирован 2026-08-19), SG v1.3 шаги 17–19,
сканы официальных карт C01–C36 / C38–C44 / C45 / C46 / B22 / B23 / B24 / B25 / B26 / B27 / B28 / B30 / B31 (подтверждены владельцем). Реализация:
`src/common/automa/MarsBotCorpData.ts` (данные) + `src/server/automa/corps/` (поведение).

| Карта | Официальный текст | Статус |
|---|---|---|
| **C01 Credicor** | DRAFT PRIORITY: Most expensive. EFFECT: When resolving a card with a cost of 20 MC or more, MarsBot gains 4 MC. | ✅ `MarsBotCredicor.ts`; драфт-протекция по RB-B p.2 Special Cases (сохраняет ВСЕ самые дорогие; все 4 равны → без сброса) |
| **C02 Ecoline** | BEFORE ACTION PHASE: Add Rapid Sprouting to MarsBot's action deck. | ✅ `MarsBotEcoline.ts`; B23 = recurring-механизм (B16-семейство) |
| **B23 Rapid Sprouting** | If the Ecoline corporation card has a plant resource on it, remove it, MarsBot places a greenery tile, and it raises oxygen 1 step. Otherwise, add a plant resource to the Ecoline corporation card. At the beginning of every generation, shuffle this into MarsBot's action deck. | ✅ там же; озеленение через штатный `AutomaTilePlacer.placeGreenery` (его подъём O₂ = печатный «raises oxygen 1 step»); нет места → Failed Action, растение остаётся |
| **C03 Helion** | SETUP: white cube — building #6, space #9, science #10, power #5 и #9, plant #11; black cube — Earth #3, #6, #9, #12, #13, #14. EFFECT: advancing onto a white cube — instead of raising the temperature, MarsBot draws and resolves a card from the project deck; advancing onto a black cube — MarsBot raises the temperature 1 step. | ✅ `MarsBotHelion.ts` + primitive кубов (`MarsBotCorpInfo.trackCubes` по ТЕГУ трека → `AutomaCorporations.onTrackAdvanced`; `AutomaState.corpCubesTriggered` — spent-once, регресс НЕ перевзводит). Белый куб = `'replaces-action'` (все 6 стоят на печатной temperature — проверено тестом); чёрный = +1 температура ДО и В ДОПОЛНЕНИЕ к печатной иконке (RB-B), maxed → штатный Failed Action. Кубы ВИДНЫ на планшете бота (`MarsBotTracks` + легенда) |
| **C04 Interplanetary Cinematics** | Starting tags: event, event. SETUP: Replace the trackers for the building track and event track with white cubes as a reminder for this corporation's effect. EFFECT: When MarsBot advances the building or event tracks, including the starting tags, MarsBot gains 2 MC. | ✅ `MarsBotInterplanetaryCinematics.ts` + primitive `onTrackAdvance` (хук на КАЖДОЕ успешное продвижение, до печатной иконки). Платит ПОШАГОВО: каскад `advance` оплачивается за каждый шаг, maxed-трек (Failed Action) не платит, регресс не «тратит» эффект. Стартовые метки платят сами собой — корпорация сажается ДО их резолва (на Тарсисе event 0→1→2 + 2→3 = 3 продвижения = 6 M€). SETUP — чистое напоминание: `whiteMarkerTracks` (по ТЕГУ) красит маркеры этих треков белым на планшете + строка легенды |
| **C05 Inventrix** | SETUP: Destroy Lobbyists from the bonus deck. EFFECT: When resolving a card with a requirement, MarsBot gains 2 MC. BEFORE ACTION PHASE: Add Do It Right to MarsBot's action deck. | ✅ `MarsBotInventrix.ts`. SETUP уничтожает ту печать «Лоббистов», что в игре (B06 без Венеры, B15 с ней); если движок УЖЕ раздал её в колоду действий 1-го поколения (наш порядок сжат), слот отдаётся следующей бонусной карте — ровно то, что дал бы настольный порядок, размер колоды не меняется. EFFECT = `card.requirements.length > 0` (любое печатное требование; бот их не проверяет). BAP = recurring-механизм B23 |
| **B25 Do It Right** | a. temperature 1-2 steps from a bonus step or completion → raise temperature 2 steps. b. oxygen 1-2 steps away → place a greenery and raise oxygen 2 steps. c. ocean-reserved space adjacent to 2 ocean tiles → place an ocean. d. No effect. At the beginning of every generation, shuffle this into MarsBot's action deck. | ✅ это ДОСЛОВНО лестница B06 Lobbyists без self-destroy и с мёртвым (d) — поэтому вынесена в общий `AutomaNearBonusPush.pushNearestBonus` (одна реализация на обе карты; «raises oxygen 2 steps» = озеленение (+1) + 1 шаг, как у B06). (d) — печатный исход, а НЕ Failed Action (компенсации 5 M€ нет). Венера ничего не меняет: третий вариант карты — океанный |
| **C06 Mining Guild** | Starting tags: building, building. SETUP: Place 10 MC on this card. EFFECT: If the building track is not yet at #18, when MarsBot gains MC, take it from this card instead. When this card empties, place 10 MC on this card and advance the building track. | ✅ `MarsBotMiningGuild.ts` + primitive «банк M€ на карте» (`MarsBotCorpInfo.mcBank`, диспатч `AutomaCorporations.onBotGainedMegacredits` из ЕДИНСТВЕННОЙ точки — `Stock.add`, где `isMarsBot` и `delta > 0`). Карта — РЕЗЕРВУАР, а не пошлина: бот получает свои M€ как обычно, просто они снимаются с карты; каждые полные 10 M€ дохода = бесплатное продвижение трека строительства. Слив идёт ЦИКЛОМ (доход 25 при полной карте = 2 продвижения и 5 M€ на карте); «#18» трактуется как КОНЕЦ трека (`maxPosition`), поэтому продвижение никогда не превращается в Failed Action, а на конце эффект просто выключается. Реентерабельность (продвижение само может дать M€ — покрытые иконки, компенсация Failed Action) поддержана: перезаправка идёт ДО продвижения, есть счётчик глубины |
| **C07 PhoboLog** | Starting tag: space. SETUP: Reveal cards from the project deck until you've revealed 2 cards with a space tag. Shuffle these cards into the bonus deck. Place a white cube on the space track on spaces #7, #10, #13, and #15. EFFECT: When MarsBot advances onto a white cube, MarsBot draws and resolves a card from the bonus deck. | ✅ `MarsBotPhobolog.ts`. **Трактовка «these cards» = ВСЕ вскрытые карты** (не только две космические): карта не называет иного места для остальных, а физическое вскрытие обязано куда-то их деть — зафиксировано комментарием и тестом. Отсюда СМЕШАННАЯ бонус-колода: `AutomaState.bonusDeck` теперь `Array<AutomaActionCard>` (старые сейвы с голыми id поднимаются при десериализации), и КАЖДАЯ точка розыгрыша бонус-колоды резолвит запись по её виду — бонус-карта по своим правилам, проектная как проект (в т.ч. бонусный слот колоды действий поколения, B18-прореживание и B08-fallback). Кубы — существующий primitive; «instead of» на карте НЕТ, поэтому печатная иконка клетки срабатывает В ДОПОЛНЕНИЕ (тест на Венеру: клетка космоса #7) |
| **C08 Saturn Systems** | Starting tags: jovian, space, space, space. DRAFT PRIORITY: Jovian > space. EFFECT: When either you play a card containing a Jovian tag or MarsBot resolves a Jovian tag, including this, advance the event track. | ✅ `MarsBotSaturnSystems.ts` — ПЕРВАЯ корпорация, следящая за ОБЕИМИ сторонами стола. Два хука: `onTagResolved` (из `AutomaResolver.resolveTag` — единственного места, где бот резолвит метку; ПОСЛЕ того, как метка отработала) и `onHumanCardPlayed` (из `Player.onCardPlayed`; флипы бота туда не попадают). Гранулярность у сторон РАЗНАЯ и ровно печатная: у человека триггер — КАРТА (одно продвижение, сколько бы юпитерианских меток на ней ни было), у бота — МЕТКА (он и обрабатывает карту по меткам). «Including this» без спец-кода: стартовая юпитерианская метка идёт тем же путём `resolveTag`. Рекурсии нет: награда ПРОДВИГАЕТ трек, а продвижение метку не резолвит |
| **C09 Teractor** | DRAFT PRIORITY: Earth. SETUP: MarsBot gains 25 MC. Replace the tracker for the Earth track with a white cube as a reminder for this corporation's effect. EFFECT: When MarsBot advances the Earth track, MarsBot gains 2 MC. | ✅ `MarsBotTeractor.ts` — почти целиком из готовых примитивов: приоритет драфта = обычная tag-chain, белый маркер = примитив C04 (та же строка легенды — обещание то же), выплата = ОБЩИЙ `MarsBotTrackPayout` (вынесен из C04 при добавлении C09, чтобы одинаковое предложение не разошлось в двух картах). Своё у карты — только подарок в 25 M€ на сетапе. Триггер — ТРЕК, а не метка: на Тарсисе трек Земли несёт и метку города, поэтому городская метка бота тоже платит (закреплено тестом) |
| **C10 Tharsis Republic** | DRAFT PRIORITY: City. SETUP: MarsBot places a city tile. EFFECT: When you place a city tile, MarsBot gains 2 MC. When MarsBot places a city tile, advance the event track. These effects also apply to setup. | ✅ `MarsBotTharsisRepublic.ts`. Один триггер, две стороны, две разные награды — и единственная точка `Game.addTile`, через которую проходит ЛЮБОЕ размещение на Марсе (новый хук `MarsBotCorp.onTilePlaced`). «Город» определяется движковым `Board.isCitySpace` (столица тоже считается), а не рукописным списком типов тайлов. «These effects also apply to setup» без спец-кода: корпорация сажается ДО своего setup-бокса, поэтому поставленный там город сам продвигает трек событий. Город человека НЕ двигает трек, город бота НЕ платит 2 M€ — половины закреплены тестами |
| **C11 ThorGate** | Starting tag: power. DRAFT PRIORITY: Power. SETUP: MarsBot gains 10 MC. Place a white cube on the power track on spaces #4, #6, #8, and #10. EFFECT: When MarsBot advances onto a white cube, MarsBot draws a card from the project deck and resolves it, ignoring all except its first tag. Then, MarsBot raises the temperature 1 step. | ✅ `MarsBotThorgate.ts`. Кубы — существующий primitive. НОВОЕ: ограниченный резолв карты — `AutomaResolver.resolveProjectCard(game, card, {tagLimit: 1})` (прокинут через `drawAndResolveProjectCard`): карта разыгрывается ПОЛНОСТЬЮ как событие (журнал, played pile, корп-диспатч, RB-B human-реакторы видят всю карту), но трек двигает только ПЕРВАЯ печатная метка — «ignoring all except its first tag» про то, какие треки бот двигает, а не про то, какая карта вошла в игру. Карта без меток — по-прежнему Failed Action. «Instead of» на карте НЕТ, поэтому печатная иконка клетки срабатывает В ДОПОЛНЕНИЕ; закрывающее +1 температуры идёт через общий `AutomaTerraformer.raiseTemperature` (завершённая температура → штатный Failed Action) |
| **C12 United Nations Mars Initiative** | SETUP: Shuffle Government Subsidy into the bonus deck. Do not add a bonus card to MarsBot's action deck during setup. BEFORE ACTION PHASE: Add an additional card from the bonus deck to MarsBot's action deck each generation, starting from the 2nd generation. | ✅ `MarsBotUnmi.ts`. Размен — вся суть карты: 1-е поколение без бонусной карты, каждое следующее — ДВЕ. Наш движок строит колоду 1-го поколения при создании игры, до появления корпорации, поэтому «do not add» не предотвращается, а ОТМЕНЯЕТСЯ: уже розданная бонусная карта возвращается в бонусную колоду (состояние, которое было бы за столом). B31 — НЕ recurring: замешивается в бонусную колоду и живёт в обычной ротации (розыгрыш → сброс → перетасовка), поэтому корпорация и раздаёт себе лишние бонусные карты |
| **B31 Government Subsidy** | MarsBot raises its TR 1 step. Use when MarsBot is playing the UNMI corporation. | ✅ там же; `corpBonusCards` + `resolveBonusCard`, исход `discard` (карта не уничтожается и не возвращается принудительно) |
| **C13 Cheung Shing Mars** | Starting tag: building. DRAFT PRIORITY: Building. SETUP: Place silver resource cube on every space of the building track starting with space #4. EFFECT: When MarsBot advances onto a silver resource cube, MarsBot gains it as MC. | ✅ `MarsBotCheungShingMars.ts`. Кубы — существующий primitive, тип `'credit'` (тот самый, что был заготовлен под жетоны); данные генерируются от #4 до конца трека (`MARSBOT_MAX_TRACK_POSITION`), то есть 15 кубов. **НОМИНАЛ**: «gains IT as MC» = стоимость самого куба; кубы ресурсов в Terraforming Mars бывают 1 (бронза) / 5 (серебро) / 10 (золото), поэтому серебряный = **5 M€** (`MARSBOT_SILVER_CUBE_MC`, вынесено в common рядом с типом куба; **подтверждено владельцем 2026-08-20**). «Instead of» нет — печатная иконка клетки срабатывает В ДОПОЛНЕНИЕ (тест на клетке #5 = tr2). Куб перекрашен в серебро и назван «Серебряный куб ресурса» в легенде планшета |
| **C14 Point Luna** | Starting tag: space. DRAFT PRIORITY: Earth. SETUP: Place a white cube on the Earth track on spaces #1, #5, #9, #13, #17. Place a black cube on the Earth track on spaces #3, #7, #11, #15. EFFECT: When MarsBot advances onto a white cube, advance the least-advanced track (topmost, if tied). When MarsBot advances onto a black cube, advance the space track. | ✅ `MarsBotPointLuna.ts` — целиком из готовых примитивов. Все кубы стоят на ОДНОМ треке (Земля), а двигают ДРУГИЕ: белый — самый отстающий (переиспользован `board.getLeastAdvancedTrackIndex`, тот же хелпер, что у wild-метки, чтобы правило не разошлось; продвижение идёт через `advanceTrack`, а НЕ через `resolveTag(WILD)` — никакая метка не разыгрывалась, и врать об этом в обзоре хода нельзя), чёрный — трек космоса (по ТЕГУ). «Instead of» нет → печатная иконка клетки Земли срабатывает В ДОПОЛНЕНИЕ (тест на #13 = tr4) |
| **C15 Robinson Industries** | НЕТ стартовой метки и НЕТ draft priority (в правом верхнем углу пусто, плашки приоритета нет — сравни C11, где есть обое). SETUP: MarsBot gains 10 MC. BEFORE ACTION PHASE: Add Diversification to MarsBot's action deck. | ✅ `MarsBotRobinsonIndustries.ts`. Вся карта — одна идея: бот ПОКУПАЕТ прогресс. 10 M€ — военная казна, B28 тратит её по 4 на самый отстающий трек. Цена реальная: остаток M€ бота конвертируется в ПО на финальном подсчёте (`AutomaScoring.mcPerVp`), то есть каждый шаг оплачен собственными очками. B28 — recurring (тот же механизм, что у B23/B25), вставка в 1-м поколении — из `beforeActionPhase` (колода строится до появления корпорации) |
| **B28 Diversification** | Advance the least-advanced track, topmost if tied. MarsBot loses 4 MC, if able. At the beginning of every generation, shuffle this into MarsBot's action deck. Use when MarsBot is playing the Robinson Industries corporation. | ✅ там же. **«IF ABLE» = ВСЁ ИЛИ НИЧЕГО**: в этом наборе частичная оплата пишется «up to X» (B01), а «X, if able» — другая формулировка; так же реализовано FAQ-правило «spends 2 MC if able» (St. Joseph of Cupertino Mission). Меньше 4 M€ — не платит вообще (это печатная ветка, а НЕ Failed Action). Два предложения НЕЗАВИСИМЫ и идут в печатном порядке: сначала продвижение (оно само может дать деньги — на заполненных треках это Failed Action с компенсацией 5 M€), затем оплата. «Least-advanced, topmost if tied» — тот же `board.getLeastAdvancedTrackIndex`, что у wild-метки и C14; продвижение через `advanceTrack`, а не `resolveTag(WILD)` (метка не разыгрывалась) |
| **C16 Valley Trust** | Стартовой метки НЕТ (значок науки на карте — это DRAFT PRIORITY). DRAFT PRIORITY: Science. SETUP: Use this corporation only when playing with Prelude. Give MarsBot 1 extra card from the project deck for its starting hand. Place a white cube on the science track on spaces #8 and #16. EFFECT: When MarsBot advances onto a white cube, it draws a card from the project deck and resolves it. | ✅ `MarsBotValleyTrust.ts`. **НОВЫЙ ПРИМИТИВ — УСЛОВИЕ МОДУЛЯ**: «only when playing with Prelude» стало ДАННЫМИ (`MarsBotCorpInfo.requiresModules`), а не рантайм-проверкой: корпорация просто не попадает в пул выбора (`eligibleCorpIds` через второй чистый предикат `hasRequiredModules`), поэтому и dev-форс её не посадит — нелегитимная корпорация никогда не должна дойти до своих хуков. **«Starting hand» = колода действий 1-го поколения** — единственная стопка, которую боту раздают на сетапе (3 проекта, +3 с Прелюдией, +1 на Brutal, плюс бонусная); колода строится до появления корпорации, поэтому карта ЗАМЕШИВАЕТСЯ в неё (seeded-random позиция) — тот же приём, что у B23/B25/B28. Эффект — готовый `drawAndResolveProjectCard` (белый куб Helion без замещения температуры): «instead of» нет → печатная иконка клетки срабатывает В ДОПОЛНЕНИЕ (тест на науке #8 = tr2) |
| **C17 Vitor** | Нет стартовой метки и нет draft priority. SETUP: Set aside Overachievement from the bonus deck. EFFECT: When resolving a card with a non-negative VP icon, MarsBot gains 3 MC. BEFORE ACTION PHASE: Add Overachievement to MarsBot's action deck, unless it has been destroyed. | ✅ `MarsBotVitor.ts`. Корпорация превращает БАЗОВУЮ бонусную карту в постоянную: B04 выходит из случайной ротации и возвращается каждое поколение, пока не заявит достижение (с 6-го поколения — награду) и не уничтожит себя; каждая неудачная попытка всё равно платит 5 M€. **B04 НЕ становится корп-картой** (`corpBonusCards` пуст, её резолв остаётся в `AutomaBonusCards`) — корпорация только вынимает её из ротации (чистка трёх мест по образцу C05) и кладёт в `recurringBonusCards`. **ФРЕЙМВОРК-ФИКС**: B04 — первая recurring-карта, которая МОЖЕТ быть уничтожена, поэтому `routeBonusCard` теперь выкидывает уничтоженный id из recurring-пула (иначе пересборка колоды воскресила бы её). Эффект переиспользует `AutomaScoring.hasNonNegativeVpIcon` — тот же предикат, что считает ПО стопки бота на Hard/Brutal, чтобы трактовка «non-negative» не разошлась. Эффект СОВПАДАЕТ с человеческим Vitor дословно — это напечатано НА КАРТЕ бота, а не протекло через identity-связь (45 M€ и бесплатная награда первым действием — нет) |
| **C18 Arcadian Communities** | Starting tag: building. SETUP: Resolve Settlers now. EFFECT: Only MarsBot may build on the areas reserved by its player markers, and receives 3 MC when it does so. BEFORE ACTION PHASE: Add Settlers to MarsBot's action deck. | ✅ `MarsBotArcadianCommunities.ts` + `AutomaMarkerPlacer.ts`. **Первая половина эффекта БЕСПЛАТНА**: тайл-лесс клетка с `space.player` уже закрыта для всех остальных в `Board.getAvailableSpacesOnLand` — занять ЗНАЧИТ запретить. **Вторая едет на ВЫПЛАТЕ ЧЕЛОВЕЧЕСКОЙ КАРТЫ**: `Game.addTile` уже считает бонус «своя отмеченная площадка» до посадки тайла; корпорация отвечает на тот же вопрос через новый хук `onBuildOnOwnMarker` (возврат `'pays'`), так что 3 M€ выдаёт ОДНА и та же строка — один скоуп, один порядок, одни и те же исключения (перекрытие тайла, фаза SOLAR). Хук спрашивается ДО `simpleAddTile` — единственный момент, когда ещё можно отличить бронь от собственности на тайл |
| **B22 Settlers** | Place one of MarsBot's player markers on the map on a non-reserved area, using the usual tiebreakers. Before using the final tiebreak (flip a card), MarsBot first looks for the space with the most adjacent spaces that are reserved for oceans. At the beginning of every generation, shuffle this into MarsBot's action deck. Use when MarsBot is playing the Arcadian Communities corporation. | ✅ там же + `AutomaMarkerPlacer`. Кандидаты — ПУСТАЯ незабронированная суша без опасных зон. Тайбрейкеры — ОБЩИЕ (`AutomaTilePlacer.breakTie`), куда добавлен необязательный `beforeFlip`: печатная вставка «больше всего соседних клеток, ЗАБРОНИРОВАННЫХ под океаны» (синие клетки карты, с тайлом или без — ЭТО НЕ шаг 1, который считает УЖЕ РАЗМЕЩЁННЫЕ океаны) идёт РОВНО перед вскрытием карты; без `beforeFlip` поведение и расход rng всех прочих размещений байт-в-байт прежние. Карта не печатает else-ветки → карта кончилась = Failed Action. **АНИМАЦИЯ (требование владельца)**: куб бота падает ТОЙ ЖЕ премиальной анимацией (`pc-place` через `cubeDropState`), что и куб человеческой Arcadian: в `MarsBotTurnVisual` добавлен `markers` (цвет-онли дифф по тайл-лесс клетке, ОТДЕЛЬНО от `tiles`), а `applyTurnVisual` красит цвет ПОСЛЕ `armPlacementAnimations()` — дальше работает общий `observeCube` |
| **C19 Astro Drill** | Стартовой метки нет (значок космоса на карте — это DRAFT PRIORITY). DRAFT PRIORITY: Space. SETUP: Place a white cube on the space track on spaces #2, #4, #7, #10, and #13. Place a black cube on the space track on spaces #5, #11, and #16. EFFECT: When MarsBot advances onto a white cube, advance the least-advanced track, topmost if tied. When MarsBot advances onto a black cube, advance the space track. | ✅ `MarsBotAstroDrill.ts`. ЭФФЕКТ ДОСЛОВНО СОВПАДАЕТ С C14 Point Luna, поэтому вынесен в ОБЩИЙ `corps/MarsBotCubeTrackPush.pushCubeTrack` (тот же приём, что `AutomaNearBonusPush` для B06/B15/B25) — C14 переведён на него без изменения поведения (его 10 тестов зелёные), каждая карта владеет только своей идентичностью, местом кубов и счётчиками. ОТЛИЧИЕ — ДАННЫЕ: у C14 кубы на треке Земли, здесь — на ТОМ ЖЕ треке, который толкают чёрные кубы (космос). Это НЕ спецслучай: куб срабатывает максимум раз за партию, поэтому цепочка саморазгона (белый #4 → космос → чёрный #5 → космос → #6) гарантированно конечна — закреплено тестом. Легенда кубов и текст эффекта — ТЕ ЖЕ строки, что у C14 (одно правило — одна формулировка, гард в спеке) |
| **C20 Factorum** | Starting tag: power. SETUP: Replace the tracker for the building track with a white cube as a reminder for this corporation's effect. EFFECT: When MarsBot advances the building track, place 1 MC on this card. BEFORE ACTION PHASE: Add Supply & Demand to MarsBot's action deck. | ✅ `MarsBotFactorum.ts`. **КАССА, а не банк**: у C06 карта начинает ПОЛНОЙ и ОПУСТОШАЕТСЯ с дохода бота, здесь — начинает ПУСТОЙ и НАПОЛНЯЕТСЯ (1 M€ за каждое продвижение трека строительства), а снимает только своя карта B24. Хранилище — тот же `corpResources` + `resource: 'megacredits'` + капсула `.pcard__res`, что у C06. Триггер — общий хук `onTrackAdvance`, НО **НЕ `MarsBotTrackPayout`**: у того модуля контракт буквально «MarsBot ПОЛУЧАЕТ N MC», а здесь деньги до бота не доходят — тот же триггер, другой адресат. SETUP-бокс — чистая подача (примитив C04: `whiteMarkerTracks` + `markerLegend`), игрового состояния в нём нет |
| **B24 Supply & Demand** | MarsBot gains 3 MC, or as much as possible, from the Factorum corporation card. If it gained 0 MC, advance the power track. At the beginning of every generation, shuffle this into MarsBot's action deck. Use when MarsBot is playing the Factorum corporation. | ✅ там же, recurring (механизм B23/B25/B28). **«OR AS MUCH AS POSSIBLE» = ЧАСТИЧНАЯ выплата** и намеренно НЕ «if able» из C15/B28 (всё-или-ничего): набор различает формулировки, а здесь это ещё и подтверждено собственной else-веткой карты («if it gained 0 MC» существует только потому, что частичный съём возможен). Берёт `min(3, на карте)`; трек энергии двигается ТОЛЬКО при съёме РОВНО 0 — один M€ на карте уже съём, и fallback закрыт (тесты на 2 и на 1) |
| **C21 Pharmacy Union** | Starting tag: science. DRAFT PRIORITY: Science. SETUP: Destroy the card Meteor Shower from the bonus deck. Reveal cards from the project deck until you've revealed a card with a science tag, and shuffle it into the bonus deck. EFFECT: When you play a microbe tag, MarsBot loses 4 MC, or as much as it is able to lose. When MarsBot resolves a card with a science tag, including this, it raises its TR 1 step. | ✅ `MarsBotPharmacyUnion.ts`. Первая корпорация, эффект которой СМОТРИТ В ОБЕ СТОРОНЫ: одна половина облагает бота за действия ЧЕЛОВЕКА, вторая платит ему за свои. **ОБА СЕТАП-ПРИМИТИВА ОБЩИЕ** (`corps/MarsBotBonusDeckOps.ts`): трёхместное уничтожение — код C05 Inventrix, reveal-until — код C07 PhoboLog; обе старые карты переведены на них без изменения поведения (33 теста зелёные). РАЗЛИЧАЕТСЯ ДИСПОЗИЦИЯ: C07 печатает «shuffle THESE cards» (всё вскрытое), C21 — «shuffle IT» (только научная карта, остальные в сброс проектов); режим объявлен ДАННЫМИ (`bonusDeckSeed.shuffle`). **ФОРМУЛИРОВКА И ГРАНУЛЯРНОСТЬ ВЗЯТЫ У ДВИЖКА**: «or as much as it is able to lose» = ЧАСТИЧНО (та же третья форма, что у B24), а «when you play a microbe tag» срабатывает РАЗ НА КАРТУ — именно так человеческая `PharmacyUnion.onCardPlayedByAnyPlayer` читает ту же печатную фразу (`card.tags.includes(MICROBE)`, булево), и две сущности с одним правилом расходиться не должны (карта с ДВУМЯ метками микроба стоит 4, не 8 — тест). «A card WITH a science tag» тоже карточной гранулярности → хук `onProjectCardResolving`, НЕ потеговый. «Including this» = собственная стартовая метка науки (RB-B Setup 4), поднимается раз в setup-боксе |
| **C22 Philares** | Стартовой метки НЕТ, draft priority НЕТ. SETUP: MarsBot places a greenery tile and raises oxygen 1 step. Place a science resource on this card. Resolve Local Neural Instance now, then destroy that card. Shuffle Build, Build, Build into the bonus deck. EFFECT: Each new adjacency between your tiles and MarsBot's tiles, regardless of who just placed the tile, gives MarsBot 1 science resource here. Then, if able, MarsBot spends 4 science resources from here to advance its most-advanced non-maxed track. | ✅ `MarsBotPhilares.ts`. **ЕДИНСТВЕННАЯ КОРПОРАЦИЯ, ЧИТАЮЩАЯ ФОРМУ КАРТЫ**: остальные смотрят на трек, метку или карту, эта — на ГРАНИЦЫ, и симметрично: «regardless of who just placed the tile» — человек кормит её так же, как она кормит себя сама. **СЧЁТ — СОСЕДСТВА НОВОГО ТАЙЛА**, а не пересчёт доски: нов только один тайл, значит новы ровно его границы. **ГРАНУЛЯРНОСТЬ И ЧТЕНИЕ ВЗЯТЫ У ДВИЖКА** (тот же закон, что у C21): человеческий `Philares.onTilePlaced` выходит при `space.player === undefined`, поэтому БЕСХОЗНЫЙ тайл (океан, Martian Nature Wonders, Rey Skywalker) ничей — границы не даёт ни в одну сторону. Маркер (C18) — НЕ тайл: «between your TILES», поэтому заявка не платит. **«THEN, IF ABLE» — ОДНА попытка на триггер**, не дренаж: 9 науки → трата 4, остаётся 5; но купленное продвижение может поставить тайл и войти в тот же хук снова — законный каскад, ограничен счётчиком глубины. Цель — `board.getMostAdvancedNonMaxedTrackIndex` (ничья → верхний, как у соседнего `getLeastAdvancedTrackIndex`); Philares — первый его вызывающий. B07 уничтожается ОБЩИМ `destroyBonusCard` (C05/C21) ДО разыгрывания |
| **B27 Build, Build, Build** | a. MarsBot places a city tile adjacent to one of your greenery tiles. If successful, it loses 5 MC. b. MarsBot places a special tile matching a card in its played pile adjacent to one of your city tiles. If successful, destroy that card and MarsBot loses 3 MC. c. MarsBot gains 3 MC, and shuffle this card back into the bonus deck. Use when MarsBot is playing the Philares corporation. | ✅ там же. **ЛЕСТНИЦА «ПЕРВОЕ ВОЗМОЖНОЕ»** (форма B06/B15/B25): ветки в печатном порядке, первая сработавшая закрывает карту. Обе строительные ветки целятся В ТАЙЛЫ СОПЕРНИКА намеренно: каждая сработавшая даёт корпорации новую границу — карта это её двигатель, а не побочка. **«IF SUCCESSFUL, IT LOSES N MC» — НЕ ЦЕНА, которую надо потянуть**: оплата СЛЕДУЕТ за постановкой и зависит от неё, а не наоборот — нищий бот всё равно строит и платит сколько есть (третья форма C21/B24). **ВЫБОР КАРТЫ ДЛЯ (b)** — первая в упорядоченной стопке, чей особый тайл ставится на сушу Марса; что такое такой тайл — ВЫВОДИТСЯ (`isSpecialTile` + `tileTypeToString` + `OCEAN_TILES`), а не таблица; спек падает со списком имён, если upstream добавит тип тайла. Особый тайл СТАВИТСЯ без проверки собственных требований его карты (RB-B: бот требования не читает) — карта называет РОВНО ОДИН критерий места. **НОВЫЙ ПЕЧАТНЫЙ ИСХОД `return-to-deck`**: ветка (c) шлёт карту обратно в БОНУСНУЮ КОЛОДУ (сразу, с перемешиванием), а не в сброс; в разборе хода это судьба `reshuffled` |
| **C23 Recyclon** | Стартовая метка: microbe (значок в правом верхнем углу); DRAFT PRIORITY: Building. SETUP: Place a white cube on the building track on spaces #3, #6, #9, #12, #15, and #18. EFFECT: When MarsBot advances onto a white cube, advance the plant track. | ✅ `MarsBotRecyclon.ts`. **САМЫЙ ЧИСТЫЙ КОНВЕРТЕР НАБОРА**: один трек кормит другой по фиксированному курсу и ничего между. Шесть кубов ровной лестницей на всю длину трека (#18 — его ПОСЛЕДНЯЯ клетка, тест): каждый третий шаг строительства покупает шаг биологии — обмен микробов на растения человеческого Recyclon, выраженный геометрией треков. **МОДУЛЬ КУБОВ ОБОБЩЁН, НЕ СКОПИРОВАН**: `MarsBotWildCubePush` → `MarsBotCubeTrackPush.pushCubeTrack`, где «что толкает каждый цвет» — ДАННЫЕ (`pushes`), а общими остались scope/атрибуция и ОБЩЕЕ продвижение; C14 и C19 переведены, их 22 теста зелёные без правок. ФОРМУЛИРОВКИ НЕ ДЕЛЯТСЯ: C14/C19 печатают одну фразу и делят ключи, C23 печатает СВОЮ — своя легенда, свой текст эффекта, своя строка журнала (гард в спеке). Кубы НИКОГДА не стоят на том треке, который толкают (в отличие от C19), поэтому саморазгон невозможен по конструкции — бот обязан реально СТРОИТЬ (тест с каскадом на стороне растений). «Instead of» нет → иконка клетки строительства срабатывает ТОЖЕ (тест на #12 = океан) |
| **C24 Splice** | Стартовая метка: plant (зелёный круг с ЛИСТОМ в правом верхнем углу); DRAFT PRIORITY: Microbe. SETUP: MarsBot gains 8 MC. Destroy Research and Development from the bonus deck. Reveal cards from the project deck until you reveal a card with a microbe tag, and shuffle it into the bonus deck. EFFECT: When you play a microbe tag, MarsBot gains 2 MC, and you may either gain 2 MC or add a microbe resource to the played card. When MarsBot resolves a microbe tag, it gains 4 MC. | ✅ `MarsBotSplice.ts`. **SETUP — ЭТО СЕТАП C21 С ДРУГИМИ ДАННЫМИ**: оба примитива `MarsBotBonusDeckOps` (трёхместное уничтожение + reveal-until с диспозицией `matching-only`), нового только обычные 8 M€. **ПЕРВАЯ ВЕТКА ЭФФЕКТА ЕДЕТ НА СТРОКЕ ЧЕЛОВЕЧЕСКОЙ КАРТЫ** (закон C18): это дословно `Splice.onCardPlayedByAnyPlayer` с БОТОМ в роли владельца и человеком в роли разыгравшего — поэтому сама сумма, условие «выбор есть только если карта держит микробов», `choiceContext` промпта и атрибуция выплат расходиться НЕ МОГУТ. **ГРАНУЛЯРНОСТЬ ЗДЕСЬ ПОТЕГОВАЯ — и это НЕ противоречие с C21**: та же английская фраза «when you play a microbe tag» читается двумя человеческими картами по-разному (`PharmacyUnion` — булево `includes`, `Splice` — `cardTagCount * 2`), и каждая бот-карта следует СВОЕМУ близнецу (тест на карте с ДВУМЯ метками). Вторая ветка потеговая по СОБСТВЕННЫМ словам («resolves a microbe TAG») → хук `onTagResolved`, как у C08. **НОВЫЙ ХУК `onMicrobeAdvancement`**: RB-B FAQ говорит, что «microbe advancement» (клетка 9 трека Венеры) решается «as if a card with a microbe was played» — человеческие Pharmacy Union / Splice уже так работают, и бот-Splice не должна быть глуше своего близнеца. Диспатч ТОЛЬКО с той клетки: стартовая метка — настоящий `resolveTag` и приходит через `onTagResolved`, так что ни один микроб не оплачивается дважды (тест). Собственная стартовая метка — РАСТЕНИЕ, поэтому на сетапе корпорация себе не платит |
| **C25 Viron** | Стартовая метка: microbe; DRAFT PRIORITY НЕТ. SETUP: Use this corporation only when playing with Venus Next or Colonies. EFFECT: When MarsBot plays a blue card with a red arrow, it gains a floater. At the end of the game, MarsBot scores 1 VP for each blue card with a red arrow in its played pile. | ✅ `MarsBotViron.ts`. **ПЕРВАЯ КОРПОРАЦИЯ, КОТОРАЯ САМА НАБИРАЕТ ПО В КОНЦЕ**: остальные конвертируются в темп, эта превращает стопку разыгранных в табло, и одни и те же карты платят ДВАЖДЫ (плаватель при розыгрыше + очко при подсчёте); убрать ни то, ни другое человек не может. **НОВЫЙ ХУК `endgameVictoryPoints`** → терм `corpVp` в `AutomaVictoryPoints` (входит в `total`, своя строка в церемонии и в консольном обзоре, своя ветка `OvBotCardSource`). Считается ИЗ СТОПКИ в момент подсчёта — никакого параллельного счётчика, который мог бы разойтись, и тест на чистоту (два вызова — один ответ, состояние не тронуто). **«A BLUE CARD WITH A RED ARROW» — ПРЕДИКАТ ДВИЖКА, не свой**: `CardType.ACTIVE` + `isIActionCard` — ровно то, чем ЧЕЛОВЕЧЕСКИЙ Viron выбирает карты для повтора. Синяя карта только с эффектом не считается (тест на Arctic Algae). ⚠️ Search for Life — С ОСОБЫМ ДЕЙСТВИЕМ: примеры брать пробой по корпусу, а не по памяти. **УСЛОВИЕ МОДУЛЯ — «ИЛИ»**: примитив C16 расширен вторым ПОЛЕМ `requiresAnyModule` (а не флагом на первом): карты печатают две разные вещи, и читатель данных не должен угадывать, какая. Гард условия модулей расширен на ОБЕ формы. Флоатеры идут в ЕДИНСТВЕННЫЙ пул `automa.floaters` (тот же, что у клетки Венеры, Титана и траты в фазе исследований), поэтому корпорация реально может довезти бота до Hoverlord |
| **C26 Celestic** | Стартовая метка: Venus; DRAFT PRIORITY: Venus > Jovian. SETUP: Use this corporation only when playing with Venus Next or Colonies. MarsBot gains a floater. EFFECT: Failed actions give MarsBot 1 floater in addition to the usual MC. ROUND START: MarsBot gains 1 floater before the Research Phase. | ✅ `MarsBotCelestic.ts`. **КОРПОРАЦИЯ, КОТОРАЯ ЗАРАБАТЫВАЕТ НА ТОМ, ЧТО БОТ ЗАСТРЯЛ**: Failed Action — это собственное признание движка, что ход не сложился; остальные корпорации берут только утешительные M€, эта превращает их в ресурс. **ДВА НОВЫХ ХУКА**: `onFailedAction` (диспатч из ЕДИНСТВЕННОГО `failedAction`, ПОСЛЕ выплаты и записи в журнал — «in ADDITION to the usual MC» добавляет к завершённому событию, а не заменяет его; тест прогоняет четыре разные причины) и `roundStart` (диспатч из `Game.gotoResearchPhase`, ДО фазы). **РАЗ ЗА ПОКОЛЕНИЕ** — новый маркер `corpRoundStartGeneration` той же формы, что `corpBapGeneration`: коробка, которая ПЛАТИТ ресурс, не имеет права сработать дважды после релоада/отмены (тесты: тройной вызов = одна выплата; маркер переживает save/load; старый сейв читается как 0). Поколение 1 коробку ROUND START не видит: корпорация выбирается на воротах research → action того же поколения, то есть ПОСЛЕ его фазы исследований — первый раунд покрыт плавателем из SETUP. Отдельный тест проверяет НАСТОЯЩИЙ переход поколения (`forceGenerationEnd`), а не только диспетчер. Условие модуля — готовый примитив C25 (`requiresAnyModule`). Флоатеры — в тот же единственный пул `automa.floaters` |
| **C27 Morning Star Inc.** | Стартовые метки: Venus ×ДВЕ (в углу V V); DRAFT PRIORITY НЕТ. SETUP: Use this corporation only when playing with Venus Next. Destroy Lobbyists from the bonus deck. Shuffle Venusian Lobby into the bonus deck. Place a silver resource cube on the Venus track on spaces #5, #6, #7, #8, #9, #11, and #12. EFFECT: When MarsBot advances onto a silver resource cube, MarsBot gains it as MC. | ✅ `MarsBotMorningStar.ts`. **ЗАМКНУТЫЙ КРУГ НА ОДНОМ ТРЕКЕ**: две стартовые метки ставят бота на #2, кубы выстроены по дальней половине того же трека, а собственная карта B26 двигает именно его — каждое лобби это шаг к следующим 5 M€ (тест на замыкание круга). **ЭФФЕКТ ДОСЛОВНО СОВПАДАЕТ С C13** ⇒ вынесен в ОБЩИЙ `corps/MarsBotSilverCubePayout.takeSilverCube`; C13 переведён, его спек (вместе с C05) прогнан ПЕРВЫМ — 30 тестов зелёные без правок. Общие также легенда куба и текст эффекта (гард в спеке), но НЕ факт инсайта. ⚠️ **ПРОБЕГ КУБОВ — НЕ «С #5 И ДАЛЬШЕ»**: карта перечисляет #5–#9, #11 и #12, то есты #10 ПУСТА (проверено кропом скана). Позиции выписаны в данные поштучно, а не сгенерированы диапазоном — генерация тихо выдумала бы восьмой куб; гард в спеке фиксирует именно ПРОПУСК. «Destroy Lobbyists» — примитив C05 с общей парой вариантов (`LOBBYISTS_VARIANTS`): в колоде либо B06, либо B15 — уничтожаем оба id, это точно |
| **B26 Venusian Lobby** | MarsBot raises Venus 1 step and advances its Venus tag track. Then, MarsBot raises oxygen 1 step, places an ocean, or raises temperature 1 step, whichever is furthest from being complete. Use when MarsBot is playing the Morningstar Inc. corporation. | ✅ там же. **ДВА БЕЗУСЛОВНЫХ ПРЕДЛОЖЕНИЯ В ПЕЧАТНОМ ПОРЯДКЕ**, ни одно не fallback для другого: завершённая Венера НЕ отменяет марсианскую половину (тест на максимуме Венеры) и наоборот. Обе половины — готовые правила движка: подъём Венеры через обычную клетку `venus` (со своим РТ и Failed Action на завершённой) и `advanceFurthestMartianParameter` — та же функция, что у трёх базовых бонусных карт с той же фразой (экспортирована ради корп-карты). «Advances its Venus tag track» — ЭТО ПРОДВИЖЕНИЕ, НЕ РЕЗОЛВ МЕТКИ (чтение C22/C23): метка не разыгрывалась, и запись о метке в разборе хода была бы ложью |
| **C28 Aphrodite** | Стартовая метка: plant; DRAFT PRIORITY: Plant > Animal > Venus (лист > лапа > V, проверено кропом). SETUP: Use this corporation only when playing with Venus Next. EFFECT: When Venus is increased (by you, MarsBot, or the card Government Intervention), MarsBot gains 2 MC. | ✅ `MarsBotAphrodite.ts`. **САМАЯ ЧИСТАЯ ПОШЛИНА НАБОРА**: корпорация ничего не делает и ничего не строит — она просто берёт долю с глобального параметра, кто бы его ни двигал. **НОВЫЙ ХУК `onVenusIncreased`, и его МЕСТО — ЭТО РЕАЛИЗАЦИЯ ПЕЧАТНОЙ СКОБКИ**: диспатч стоит в `Game.increaseVenusScaleLevel` рядом с выплатой ЧЕЛОВЕЧЕСКОЙ Aphrodite — то есть ВНЕ гарда `phase !== SOLAR`. B16 Government Intervention поднимает Венеру, принудительно выставив `game.phase = SOLAR` именно чтобы не дать РТ и бонусы; печатная скобка «or the card Government Intervention» существует ровно затем, чтобы сказать «но на этот эффект подавление не распространяется» — движок уже кодирует это позицией, так что честная реализация — НАСЛЕДОВАТЬ МЕСТО, а не переписывать исключение (тест: B16 поднял Венеру, РТ НЕ вырос, корпорация всё равно получила 2 M€). **ПОШАГОВО, НЕ ПОСОБЫТИЙНО**: бот-карта пишет просто «2 MC», но близнец печатает «whenever Venus is terraformed 1 STEP», и движок платит `2 * steps` — закон C21/C24 (тесты на двушаговый подъём и на обрезание потолком) |
| **C29 Manutech** | Стартовая метка: building (проверено кропом угла); DRAFT PRIORITY НЕТ. SETUP: Place a black cube as a reminder above the #5 and #12 columns. EFFECT: When any track (including Venus) reaches #5 or #12, after resolving the effect, advance it another space, and resolve that one as well. | ✅ `MarsBotManutech.ts`. **ЕДИНСТВЕННАЯ КОРПОРАЦИЯ, ПЛАТЯЩАЯ ТЕМПОМ**: остальные дают боту ресурс, тайл или очко — эта даёт ХОД, а чего он стоит, решает то, что напечатано через две клетки. **НОВЫЙ ХУК `onTrackSpaceResolved`** — «after»-близнец `onTrackAdvance`: тот срабатывает ДО печатной иконки клетки, этот — ПОСЛЕ неё и после всего каскада, который она открыла. **ТРИГГЕР — ПРИЗЕМЛЕНИЕ, ТОЛЧОК — ОТТУДА, ГДЕ МАРКЕР СЕЙЧАС**: резолвер запоминает `landed` ДО того, как иконка сможет унести маркер дальше (трек событий: #5 = «advance»), а сам толчок — обычный `advanceTrack` от текущей позиции: игрок доигрывает клетку, смотрит на напоминание и двигает маркер ещё на одну (тест: #4 → #5 → иконка до #6 → толчок до #7 → её иконка до #8). «Advance it another space, and resolve that one as well» ЕСТЬ `advanceTrack`, поэтому ни одно общее правило здесь НЕ переписано: иконка новой клетки, её каскад, чужие кубы и Failed Action на закончившемся треке (трек Венеры короче: его #12 — ПОСЛЕДНЯЯ клетка, там толчок всегда Failed Action — тест есть). Хук получает `depth` резолвера и передаёт `depth + 1` — иначе общий гард от убегающих цепочек начинал бы счёт заново (тест читает `depth` и `cause` прямо из записи хода). **НАПОМИНАНИЕ — КОЛОНКА, А НЕ ТРЕК**: новая пара данных `reminderColumns` + `columnLegend` — это примитив C04/C20, повёрнутый на 90°: треки лежат строками ОДНОЙ сетки, поэтому колонка пересекает все треки сразу и НЕ требует разрешения по доске (клиент читает её прямо из `marsBotCorpInfo`, без нового поля в модели); на мате она рисуется полосой через все треки плюс сам чёрный куб НАД верхней строкой. **РЕГРЕСС НЕ ГЛУШИТ КОРПОРАЦИЮ**: правило регресса гасит СОБСТВЕННОЕ действие клетки, а не клаузу, триггер которой — само движение (так же читает C04); физическая примета та же — потраченный куб уходит с мата, этот напоминающий не уходит никогда (тест: иконка клетки молчит, корпорация срабатывает) |
| **C30 Aridor** | Стартовых меток НЕТ (в углу — плашка приоритета, проверено кропом); DRAFT PRIORITY: least-advanced. SETUP: Use this corp only when playing with Colonies. Place 5 white cubes: building track #3, space track #3, science #3, power track #3, Jovian track #6. Place 4 black cubes: Earth track #3, city track #6, plant track #3 and #6. Add an additional Colony tile to play. EFFECT: When MarsBot advances onto a black or white cube, advance the event track. | ✅ `MarsBotAridor.ts`. **ШИРИНА, ПРЕВРАЩЁННАЯ В СОБЫТИЯ**: приоритет гонит бота в самый отстающий трек, кубы кольцом стоят на #3 у КАЖДОГО трека кроме одного (и ещё раз на #6 у трёх двухметочных), и все девять платят в ОДИН трек — событий, единственный, который карта оставляет пустым. C14 Point Luna — обратная форма: один трек как часы, вся доска как выплата. **ЦВЕТ КУБА — КОМПОНЕНТ, А НЕ ПРАВИЛО**: «a black OR white cube» — одно предложение с одним исходом, два цвета взяты потому, что девяти одноцветных кубов в коробке нет. Поэтому оба цвета толкают один трек, делят одну строку журнала и один счётчик, а мат рисует ОДНУ строку легенды с двумя образцами (общая правка `legendRows`: цвета с одинаковым текстом группируются). Вычитать здесь различие — значит выдумать правило. **ТРЕКОВ НА МАТЕ МЕНЬШЕ, ЧЕМ ИМЁН НА КАРТЕ**: power и Jovian — ОДИН трек, Earth и city — другой, поэтому «power #3, Jovian #6» сеет одну строку дважды, и «Earth #3, city #6» тоже. Данные говорят то же, что карта (адресация ТЕГОМ), `cubesOf` разрешает — ровно ради этого позиции кубов и были сделаны тег-адресуемыми (гард в спеке). **«ADD AN ADDITIONAL COLONY TILE TO PLAY»** — первый бот-сетап, меняющий ОБЩИЙ стол, а не материал бота: новая колония открыта и человеку. Отличается от идентичной фразы человеческого Aridor только ВЫБОРОМ (бот не получает промптов и берёт тайл seeded-random), поэтому вторая половина вынесена в общий `ColoniesHandler.seatColonyTile` — сортировка, проверка активации, изъятие из `discardedColonies` и строка журнала теперь одни на двоих (спеки Aridor/Maria/Prospecting прогнаны как доказательство). Пустой пул в бот-партии недостижим (валидатор запрещает `customColoniesList`, в игре все 11 базовых тайлов) — гард всё равно есть и покрыт прямым вызовом. **ПРИОРИТЕТ `leastAdvancedTrack`** существовал в фреймворке с самого начала и до C30 не был покрыт ни одним тестом — теперь покрыт: выбор, переоценка при сдвиге треков, «ничего не совпало = один большой тай», защита от пост-драфт сброса |
| **C31 Arklight** | Стартовая метка: animal; DRAFT PRIORITY: Animal > Plant (лапа > лист). SETUP: Replace the tracker for the plant track with a white cube as a reminder for this corporation's effect. EFFECT: When resolving a plant or animal tag (not microbe!) including the starting tag, MarsBot gains 2 MC. | ✅ `MarsBotArklight.ts`. **ЗЕРКАЛО C09 TERACTOR, И «(not microbe!)» — ЭТО ПРИЧИНА**: Teractor красит белый маркер на треке и платит за ЕГО ПРОДВИЖЕНИЕ (поэтому на Тарсисе метка города, идущая по треку Земли, тоже платит — дословно как напечатано); Arklight ставит то же напоминание на трек растений и платит за МЕТКУ, а по этому треку идут ТРИ метки, из которых платят две. Одинаковая пометка, та же строка мата, противоположный триггер — печатное восклицание существует ровно потому, что одно напоминание читалось бы как правило Teractor. Отсюда `onTagResolved` (пер-метковый хук C08/C24), а НЕ `MarsBotTrackPayout`, чей контракт «продвинулся ЭТОТ ТРЕК» тихо оплачивал бы микробов (тест: продвижение трека без метки не платит). **«INCLUDING THE STARTING TAG» БЕСПЛАТНО**, но не зря напечатано: стартовая метка животного идёт через обычный `resolveTag` при выборе, то есть корпорация платит себе 2 M€ на входе. **WILD — НЕ РАСТЕНИЕ**: `resolveTag` передаёт метку как НАПЕЧАТАНО, wild приходит как `Tag.WILD` (и может двинуть как раз этот трек) — карта называет две метки, wild не из них. **МИКРОБНОЕ ПРОДВИЖЕНИЕ (клетка 9 Венеры) тоже не платит**: C24 Splice реализует `onMicrobeAdvancement` именно ради неё, а здесь микроб — единственное, что бокс эффекта исключает, поэтому глухота к той клетке и есть чтение. **ПОМЕТОЧНО, А НЕ ПОКАРТОЧНО** — «when RESOLVING a plant or animal tag», а `resolveProjectCard` разыгрывает печатные метки по одной, так что «Экологическая зона» (единственная базовая карта с обеими) платит дважды. ⚠️ Этот тест НЕЛЬЗЯ проверять чистым балансом: вторая метка ставит трек на клетку озеленения, а озеленение рядом с океаном приносит свои 2 M€ — считать счётчиками |
| **C32 Polyphemos** | Стартовые метки: космос ×3 + событие ×3 (в углу ✳✳✳ ↓↓↓, проверено кропом); DRAFT PRIORITY НЕТ. SETUP: MarsBot gains 25MC. BEFORE ACTION PHASE: MarsBot discards one of the cards with the fewest tags from its action deck. | ✅ `MarsBotPolyphemos.ts`. **САМЫЙ БОЛЬШОЙ СТАРТ В НАБОРЕ** — ШЕСТЬ печатных стартовых меток (прежний рекорд, четыре, был у C08) плюс 25 M€: треки космоса и событий стоят на #3 ещё до первого хода, и каждая клетка по дороге срабатывает. Дальше корпорация не получает ничего — она только выбрасывает худшую карту каждого поколения. **ПОЧЕМУ ЭТО РЕАЛЬНЫЙ ЭФФЕКТ**: ход бота = вскрыть карту действия и продвинуть трек за каждую напечатанную метку, то есть число меток И ЕСТЬ ценность карты, а карта БЕЗ меток — официальный Failed Action (rulebook p.6); сброс самой слабой карты поднимает пол каждого оставшегося хода. **«CARDS WITH THE FEWEST TAGS» = ПРОЕКТНЫЕ КАРТЫ**: в колоде действий лежит ещё и бонусная, но у неё не «мало меток», а нет ряда меток вовсе, и это спец-ход поколения, а не кандидат в корзину. Мера — печатный ряд (`MarsBotDraftResolver.printedTagCount`, тот самый счёт, которым живёт приоритет «most tags» у C45), сравнивать можно только то, у чего этот ряд есть (тест: колода, где «бестеговая» только бонусная карта, сбрасывает проектную; колода из одних бонусных не сбрасывает ничего). Ничьи — seeded rng, промптов бот не получает. ⚠️ **ПЕРВОЕ ПОКОЛЕНИЕ НЕ ИСКЛЮЧЕНИЕ**: RB-B резолвит Before-Action-Phase «также после сетапа, до фазы действий 1-го поколения», общий гейт это уже делает — поэтому все счётчики в спеке меряются ДЕЛЬТОЙ, а не абсолютом (отдельный тест фиксирует, что к первой фазе действий коробка уже отработала) |
| **C33 Poseidon** | Стартовых меток НЕТ, DRAFT PRIORITY НЕТ. SETUP: Use this corp only when playing with Colonies. MarsBot builds a colony (and gains 2 of the corresponding resource to its Shipping Board). EFFECT: When you or MarsBot build a colony, including during setup of this card, MarsBot advances the least-advanced track, topmost if tied. | ✅ `MarsBotPoseidon.ts`. **КОРПОРАЦИЯ, КОТОРАЯ ОБЛАГАЕТ САМО РАСШИРЕНИЕ**: у неё нет ни своего трека, ни куба, ни ресурса — она просто берёт шаг каждый раз, когда за столом основана ЛЮБАЯ колония, и шаг всегда уходит туда, где бот слабее всего. Человек не может не кормить её, кроме как не расселяясь вовсе. **SETUP ЦЕЛИКОМ ДВИЖКОВЫЙ**: «builds a colony (and gains 2 of the corresponding resource)» — это дословно `AutomaColonies.botBuildColony` (случайный подходящий тайл, печатная награда игнорируется в пользу 2 ресурсов на транспортную доску, у Европы вместо этого океан, своя строка журнала); здесь не переписано ничего. Стол без подходящего тайла просто не строит (прецедент B18: невозможный первичный эффект — не Failed Action). **«INCLUDING DURING SETUP OF THIS CARD» БЕСПЛАТНО, и порядок — это и есть причина**: `selectCorporation` сажает корпорацию ДО резолва Setup-бокса, поэтому построенная им колония доходит до собственного хука и оплачивает первый шаг (тест фиксирует). **ПОЗИЦИЯ ДИСПАТЧА ЕСТЬ ПРАВИЛО (закон C28)**: колонию можно основать ровно в двух местах — `Colony.addColony` (человек) и `AutomaColonies.botBuildColony` (бот, который НАМЕРЕННО не идёт через первое, потому что игнорирует печатную награду), и в каждом уже крутится движковый цикл «any player built a colony» для человеческой Poseidon с её `onColonyAddedByAnyPlayer`. Новый хук `onColonyBuilt` диспатчится рядом с этим циклом в ОБОИХ местах — поэтому «you or MarsBot» не требует ни одной ветки и не может разойтись с картой, чью фразу он делит. «Самый отстающий трек, при равенстве верхний» — движковое wild-правило `getLeastAdvancedTrackIndex`; продвижение идёт через `advanceTrack`, а не `resolveTag` (метка не разыгрывалась, запись о ней в разборе хода была бы ложью). Полностью законченный мат берёт обычный Failed Action, и `poseidonSteps` тогда не растёт |
| **C34 Stormcraft Incorporated** | Стартовая метка: Jovian (Юпитер, проверено кропом); DRAFT PRIORITY НЕТ. SETUP: Use this corporation only when playing with Venus Next or Colonies. MarsBot gains a floater. EFFECT: When MarsBot spends floaters to keep an additional card, MarsBot raises the temperature 1 step. ROUND START: MarsBot gains 1 floater before the Research Phase. | ✅ `MarsBotStormcraft.ts`. **КАРТА, ЗАМЫКАЮЩАЯ САМА СЕБЯ**: два бокса из трёх только НАПОЛНЯЮТ пул аэростатов, а третий срабатывает ровно в тот момент, когда пул ТРАТИТСЯ — медленный насос, каждый разряд которого греет Марс. Это идентичность человеческой Stormcraft (аэростаты как топливо, тепло на выходе), прочитанная через единственный расход аэростатов, который у бота есть. **ДВА БОКСА ДОСЛОВНО СОВПАДАЮТ С C26 CELESTIC** (условие модуля, аэростат на сетапе, аэростат перед фазой исследований) ⇒ выплата вынесена в ОБЩИЙ `corps/MarsBotFloaterPayout.gainFloaters`, C26 переведена на него, её спек прогнан ПЕРВЫМ — 14 тестов зелёные без правок; общие также i18n-ключи этих строк (гард в спеке C34 ассертит РАВЕНСТВО секций setup/roundStart с C26 и РАЗЛИЧИЕ секции effect), но НЕ факт инсайта: Celestic зарабатывает на том, что бот ЗАСТРЯЛ, эта — на том, что бот достаточно БОГАТ, чтобы тратить. **«SPENDS FLOATERS TO KEEP AN ADDITIONAL CARD» — ОДНО ТОЧНОЕ СОБЫТИЕ**, и в движке для него ровно одно место: `AutomaResearch.trySpendFloaters` (5 аэростатов в конце фазы исследований, когда веха Hoverlord уже недостижима — RB-C p.4; в недрафтовом варианте это лишняя карта из колоды, в драфтовом — сохранённая 4-я). Новый хук `onFloatersSpent` диспатчится ПОСЛЕ списания и записи в журнал: печатная фраза добавляется к ЗАВЕРШЁННОМУ событию — то же чтение, что у `onFailedAction`. Подъём — общий `AutomaTerraformer.raiseTemperature` (свой РТ, свои бонусные ступени, свой Failed Action на завершённой температуре); шаг засчитывается только если параметр реально сдвинулся. Тест гонит и НАСТОЯЩУЮ сборку колоды (`buildActionDeck`), а не только хук |
| **C35 Lakefront Resorts** | Стартовых меток НЕТ и DRAFT PRIORITY НЕТ (в углу только логотип). SETUP: Place a white cube on this card. EFFECT: When an ocean tile is placed, if there is a white cube on this card, remove it and advance the building track. Otherwise, place a white cube here. MarsBot's bonus for placing adjacent to an ocean tile is 3 MC (instead of 2 MC). | ✅ `MarsBotLakefrontResorts.ts`. **ЕДИНСТВЕННАЯ КОРПОРАЦИЯ С ПАМЯТЬЮ РОВНО В ОДИН БИТ**: все остальные реагируют на событие одинаково каждый раз, эта ЧЕРЕДУЕТ — сколько стоит океан, зависит от предыдущего океана. Куб и есть этот бит; сетап кладёт его именно для того, чтобы ПЕРВЫЙ океан партии был платящим, дальше платят нечётные (1-й, 3-й, 5-й…), чётные перезаряжают карту. Живёт в том же слоте `AutomaState.corpResources`, что и растение Ecoline / наука Philares — на столе это физически то же самое (куб на карте корпорации), поэтому новых полей нет, только новый вид ресурса `'cube-white'`. **«WHEN AN OCEAN TILE IS PLACED» — ОБА МЕСТА ЗА СТОЛОМ И ЧТЕНИЕ ДВИЖКА**: фраза не называет того, кто кладёт, значит человеческий океан переключает карту так же, как собственный (один хук `Game.addTile` видит оба); а поскольку ЧЕЛОВЕЧЕСКАЯ Lakefront Resorts печатает ровно тот же триггер для своего прироста производства, предикат взят у неё — `Board.isUncoveredOceanSpace` (по-настоящему НОВЫЙ океан, а не Ocean City / заповедник поверх уже лежащего). **ВТОРАЯ ФРАЗА — ОДНО ПОЛЕ, И ЭТО ПОЛЕ ЧЕЛОВЕЧЕСКОГО БЛИЗНЕЦА**: `player.oceanBonus` (движок в общем `Game.grantPlacementBonuses` платит «соседние океаны × oceanBonus» — его собственный комментарий раньше отмечал, что бот просто держит дефолтные 2). Установка на сетапе наследует ВСЕ пути сразу: свои тайлы, тайлы с клетки трека, тайлы бонусной карты. Правило не переписывается — файл только ИЗМЕРЯЕТ надбавку, пересчитывая соседей той же board-выборкой. **КАСКАД РЕАЛЕН И ЛЕГАЛЕН**: клетка #2 трека строительства сама выкладывает океан, поэтому оплаченный шаг может положить океан, который переключит карту снова — это печатное правило, а не особый случай (спек пиннит и цепочку, и инвариант чередования; глубина живёт в МОДУЛЕ, `depth` протаскивается в `advanceTrack`) |
| **C46 Tycho Magnetics** | Стартовых меток НЕТ (в углу только плашка приоритета, проверено кропом); DRAFT PRIORITY: Power > Science. SETUP: Place Interface Hyperlink on the bottom of the bonus deck. Ни EFFECT, ни ROUND START не напечатаны. | ✅ `MarsBotTychoMagnetics.ts`. **КОРПОРАЦИЯ, КОТОРАЯ ЦЕЛИКОМ ЕСТЬ ОДНО ОТЛОЖЕННОЕ СОБЫТИЕ**: ни бокса эффекта, ни старта раунда, ни стартовой метки — вся её игра это одна карта, лежащая НА ДНЕ бонусной колоды, плюс приоритет драфта, который всё ожидание копит трек энергии. Когда колода наконец до неё доходит, размер этой стопки и есть то, что корпорация обналичивает. **«ДНО» — ОТДЕЛЬНАЯ, НЕ СУЩЕСТВОВАВШАЯ ДО СИХ ПОР ДИСПОЗИЦИЯ**: любая другая корп-карта либо замешивается в ротацию (C12/B31, C22/B27, C27/B26), либо становится recurring (C02/B23, C15/B28); эта КЛАДЁТСЯ, а не замешивается — розыгрыш бонусной карты берёт `shift()` с начала, поэтому `push()` и есть дно, и карта выходит ПОСЛЕДНЕЙ. Это и задумано: чем дольше она ждёт, тем выше бот на треке энергии к моменту её выхода |
| **B30 Interface Hyperlink** | Draw project cards from the project deck equal to MarsBot's space number on the power track (minimum 2). Select 2 cards from the drawn cards using the following priorities: 1. Science tag. 2. Most expensive. 3. Most tags. 4. Random. Resolve the 2 selected cards, and discard the rest, then destroy this card. Use when MarsBot is playing the Tycho Magnetics corporation. | ✅ там же. **СПИСОК ПРИОРИТЕТОВ — ЭТО ОДНА ЛЕКСИКОГРАФИЧЕСКАЯ ЦЕПОЧКА**, ровно та же форма, что у DRAFT PRIORITY корпорации, поэтому он работает на той же машинерии: `MarsBotDraftResolver.pickByScore` (метод открыт специально — сравнение массивов очков «по позициям, ничьи решает seeded-перемешивание» уже жило там ради драфта, а «4. Random» и есть это перемешивание, а не четвёртая оценка). ⚠️ Новый тип в `MarsBotDraftPriority` НЕ добавлялся: этот union — множество приоритетов, печатаемых на плашке DRAFT PRIORITY, и правило выбора бонусной карты в нём не место. Наука СЧИТАЕТСЯ, а не проверяется булево — так RB-B читает печатный приоритет по метке («карта с первой меткой дважды — ещё лучше»); при отсутствии карт с двумя науками оба чтения совпадают, потому печатный список и пишет это одним словом. **ВЫБИРАЕТСЯ ДВАЖДЫ, А НЕ СОРТИРУЕТСЯ**: «select 2» это лучшая карта, затем лучшая из оставшихся — то же действие, выполненное дважды, что и делает случайную развязку честной при тройном равенстве. Размер добора — номер клетки на треке энергии (адресация ТЕГОМ: на Тарсисе это та самая строка, которую весь матч кормил приоритет корпорации), «minimum 2» это пол ДОБОРА и одновременно ровно столько же оставляется — бот на клетке 0 или 1 берёт две и играет обе, не сбрасывая ничего. Оставленные РАЗЫГРЫВАЮТСЯ через `resolveProjectCardForBot` (метки двигают треки, эффект корпорации и санкционированные human-реакторы срабатывают, карта уходит в стопку разыгранных); остальные — в сброс проектов ДО розыгрыша, чтобы отвергнутую карту не мог вытянуть заново эффект, запущенный самим розыгрышем. Тест «уничтожается» гоняет НАСТОЯЩИЙ путь (`drawAndResolveBonusDeckCard`): резолв карты, всё ещё лежащей в колоде, проверял бы состояние, которого в игре не бывает |
| **C36 Pristar** | Стартовых меток НЕТ, DRAFT PRIORITY НЕТ, SETUP НЕТ. EFFECT: When MarsBot would raise the temperature, raise oxygen, place an ocean, or raise Venus, if there is a white cube on this card, remove it. Then, MarsBot gains 1 TR and 6 MC, and skip increasing the global parameter. BEFORE ACTION PHASE: Place a white cube on this card if there isn't one already. | ✅ `MarsBotPristar.ts`. **ПЕРВАЯ КОРПОРАЦИЯ, КОТОРАЯ ГОВОРИТ «НЕТ»**: все прежние хуки — уведомления о том, что УЖЕ случилось, и умеют только ДОБАВЛЯТЬ; этот спрашивается ДО действия бота и ОТМЕНЯЕТ его. Отсюда новый хук `onWouldRaiseParameter` (диспетчер `AutomaCorporations.replacesParameterRaise`) и ровно поэтому лицом карты взята человеческая Pristar: та платит владельцу 6 M€ за поколение БЕЗ терраформирования, эта — те же 6 M€ за каждый шаг, от которого она бота отговорила. **ОДИН РАЗ ЗА ПОКОЛЕНИЕ ПО КОНСТРУКЦИИ, И SETUP НЕ НУЖЕН**: куб кладёт коробка Before Action Phase, а её RB-B резолвит и сразу после сетапа (общий гейт `corpBapGeneration`), так что 1-е поколение заряжено как любое другое; «if there isn't one already» = неистраченный куб не удваивается. **ТРИГГЕР — ДЕЙСТВИЕ, А НЕ ДВИЖЕНИЕ ПАРАМЕТРА**: четыре печатные фразы это четыре ПАРАМЕТРИЧЕСКИХ ДЕЙСТВИЯ бота; зелень поднимает кислород как следствие ТАЙЛА, и «place a greenery tile» ни в одной из них нет. Поэтому гейт спрашивается на СВОИХ точках действия бота (общий `raiseTemperature`, общий `placeOcean`, venus трека, обе лестницы бонус-карт), а не внутри `Game.increaseOxygenLevel`, где он съел бы кислород уже положенной зелени. **ЗАВЕРШЁННЫЙ ПАРАМЕТР — ВСЁ РАВНО КОНВЕРСИЯ** (гейт спрашивается ДО проверки на maxed): «WOULD raise» — про инструкцию, а не про её выполнимость, поэтому Failed Action тут не возникает; альтернативное чтение зафиксировано в комментарии и тестом |
| **C38 TerraLabs** | Стартовая метка: наука (в угловой рамке, проверено кропом против C11, где рамка держит метку, и C16, где угол ПУСТ, а атом лежит в плашке DRAFT PRIORITY). DRAFT PRIORITY НЕТ. SETUP: MarsBot loses 8 TR. BEFORE ACTION PHASE: If generation 1-8: Draw and shuffle 1 project card into MarsBot's action deck. If generation 9+: Draw and shuffle 2 project cards into MarsBot's action deck. | ✅ `MarsBotTerraLabs.ts`. **СДЕЛКА ЧЕЛОВЕЧЕСКОЙ КАРТЫ В ВАЛЮТЕ БОТА**: TerraLabs Research покупает карты за 1 M€ вместо 3 и платит за это рейтингом; у бота нет ни руки, ни цен, поэтому та же сделка заключается в единственной валюте, которую он тратит, — в КОЛОДЕ ДЕЙСТВИЙ. Он стартует на 8 РТ ниже и весь остаток партии тянет плотнее любой другой корпорации, а с 9-го поколения — вдвое. **ТОЛСТАЯ КОЛОДА — ЭТО БОЛЬШЕ ХОДОВ, А НЕ ЛУЧШИЕ ХОДЫ**: каждая лишняя карта проекта = ещё один флип, а флип = продвижение трека на каждую печатную метку; корпорация покупает ТЕМП, и 8 РТ — ровно его цена. Ничего не выбирается: карта берётся с верха колоды проектов и вставляется на seeded-случайную позицию — это и значит «shuffle into» (идиома C02/C16/C12, и по той же причине: боту нельзя выдать карту, к которой он не мог бы прийти перемешиванием). ⚠️ Коробка BAP уже отработала к первой фазе действий (RB-B + общий гейт), поэтому 1-е поколение получает карту как любое другое, а счётчики в спеке читаются как ДЕЛЬТЫ. Карта НИКОГДА не называется в журнале — она замешана рубашкой вверх и объявит себя при флипе (закон C16 для той же идиомы) |
| **C39 Utopia Invest** | Стартовые метки: строительство + космос (ДВЕ, в угловой рамке, проверено кропом). DRAFT PRIORITY НЕТ, SETUP НЕТ. BEFORE ACTION PHASE: Add Investors to MarsBot's action deck. | ✅ `MarsBotUtopiaInvest.ts`. Единственная коробка — посев своей recurring-карты B32 (механизм B23/B25/B28). Человеческая Utopia Invest печатает ОДНУ метку строительства — метки бот-карты её собственные (закон C04) |
| **B32 Investors** | If it is an even-numbered generation, advance MarsBot's least-advanced track (topmost, if tied), and regress its most-advanced (topmost if tied) track. Otherwise, MarsBot gains MC equal to its least-advanced track space number. At the beginning of every generation, shuffle this into MarsBot's action deck. | ✅ там же. **УПРАВЛЯЮЩИЙ ПОРТФЕЛЕМ, А НЕ ДВИГАТЕЛЬ**: человеческая карта меняет шаг производства на 4 ресурса того же вида — переносит ценность оттуда, где она лежит, туда, где нужна; B32 делает то же самое с единственной «доской» бота: в чётное поколение снимает шаг с того, в чём бот СИЛЬНЕЕ всего, и отдаёт тому, в чём он СЛАБЕЕ, а в нечётное просто обналичивает позицию слабого трека в M€. **⚠️ ОБА ТРЕКА ЧИТАЮТСЯ ДО ТОГО, КАК ЧТО-ЛИБО СДВИНЕТСЯ** — печатная фраза называет их в ОДНОМ предложении: за столом ты смотришь на планшет один раз, а потом двигаешь оба куба. Выбор самого передового ПОСЛЕ продвижения позволил бы толчку самому создать лидера, которого он же и откатит (плоский планшет: трек 0 идёт на 1, становится лидером и тут же съезжает обратно) — то есть карта не делала бы ничего именно тогда, когда должна делать больше всего. На плоском планшете обе клаузы всё равно указывают на ОДИН трек («верхний при равенстве»), и это честно: иконка клетки срабатывает на подъёме, а маркер регрессии не даст ей сработать второй раз. Зафиксировано тестами. **ТРИ ОБЩИХ ЧТЕНИЯ, НИ ОДНО НЕ ПЕРЕПИСАНО**: `getLeastAdvancedTrackIndex` (тот же хелпер, что у wild-метки, C33 и B28), его брат `getMostAdvancedTrackIndex`, и обычный `AutomaResolver.advanceTrack` (иконка клетки, каскады, кубы и Failed Action на завершённом треке приходят даром). Откат — движковый `MarsBotTrack.regress` (им же пользуется последствие Ares-опасности): трек на старте откатить нельзя, и это честный исход, а не Failed Action. Клетка №0 в нечётном поколении платит 0 — тоже не Failed Action |
| **C40 Ecotec** | Стартовая метка: растение. DRAFT PRIORITY: Plant > Microbe > Animal. SETUP: Place 2 plants on this card. Replace the tracker for the plant track with a white cube as a reminder for this corporation's effect. EFFECT: When resolving a plant, microbe, or animal tag, including the starting tag, place a plant on this card. BEFORE ACTION PHASE: If there are 5 or more plants on this card, remove 5 plants from here and MarsBot advances the plant track. | ✅ `MarsBotEcotec.ts`. **ОРАНЖЕРЕЯ, А НЕ ВЫПЛАТА**: карта берёт семейство «bio-метки» человеческой EcoTec (растение, микроб, животное — ровно те три, что печатает её эффект) и превращает каждую в растение В ХРАНИЛИЩЕ; ничего не тратится по дороге, склад обналичивается раз в поколение по пять за один шаг того самого трека, по которому эти метки и едут. То есть корпорация не ускоряет бота — она заставляет его bio-метки считаться ДВАЖДЫ. Склад — обычный слот `corpResources` + `resource: 'plant'` (там же растение Ecoline и наука Philares), поэтому капсула лица, сериализация и правило атаки на растения корпорации (`AutomaTargeting.corpPlantPool`, FAQ RB-B) приходят даром. **МАРКЕР СЕТАПА — ДОСЛОВНО C31 ARKLIGHT** (тот же трекер на том же треке) ⇒ буквально тот же i18n-ключ; а вот `markerLegend` свой: Arklight платит за растение/животное и намеренно НЕ за микроба, эта кормится всеми тремя. **«INCLUDING THE STARTING TAG» БЕСПЛАТНО, И ПОРЯДОК ЭТО ПОКАЗЫВАЕТ**: фреймворк сначала резолвит бокс SETUP, потом стартовые метки, поэтому карта выходит с сетапа с 2 + 1 = 3 растениями (зафиксировано тестом, а не предположением). **MICROBE ADVANCEMENT — ТОЖЕ МИКРОБ** (FAQ RB-B, чтение C24 Splice): клетка 9 венерианской доски двигает bio-трек микробом, не резолвя метку, и карта, кормящаяся микробными метками, иначе была бы глуше к ней, чем та же фраза в тавро человека; `onMicrobeAdvancement` — единственная точка диспатча этой клетки, стартовая метка приходит через `onTagResolved`, так что дважды никто не считается |
| **C41 Kuiper Cooperative** | Стартовая метка: космос И DRAFT PRIORITY: Space (напечатано И то, и другое — угловая рамка + плашка, проверено кропом; форма C11, в отличие от C19, где угол пуст). SETUP: Place a white cube on the space track on spaces #4, #8, and #12. Place a black cube on the space track on spaces #7, #10, and #14. EFFECT: When MarsBot advances onto a white cube, MarsBot raises the temperature 1 step. When MarsBot advances onto a black cube, MarsBot places an ocean tile. | ✅ `MarsBotKuiperCooperative.ts`. **ВСЯ КАРТА — ОДИН ТРЕК**: стартовая метка, приоритет драфта и все шесть кубов сидят на треке КОСМОСА — корпорация драфтит космос, каждая такая карта двигает этот трек, а трек заминирован шестью вехами, которые терраформируют Марс бесплатно. Человеческая Kuiper Cooperative швыряет астероиды и кометы из-за Нептуна — тепло и вода — ровно это кубы и приносят. **НИ ОДНОГО НОВОГО МЕХАНИЗМА**: позиции кубов — данные (`trackCubes`), посев/spent-once/регрессия — фреймворк (`onTrackAdvanced`), а следствие каждого цвета — ОБЩИЙ примитив: `AutomaTerraformer.raiseTemperature` (та же функция, что зовёт чёрный куб C03 Helion, так что два печатных «raises the temperature 1 step» — одна реализация) и `AutomaTilePlacer.placeOcean`. Оба несут свой РТ, свои бонусы клетки, свою строку журнала и свой Failed Action. Отдельный push-модуль НЕ выносился: у C14/C19/C23 общее — ПРАВИЛО (какой трек двигает цвет), а здесь два цвета делят лишь примитивы, каждый из которых уже одна функция. **НИ ОДИН ЦВЕТ НЕ ГОВОРИТ «INSTEAD OF»**, значит работает общее правило RB-B: куб срабатывает ДО и В ДОПОЛНЕНИЕ к печатному значку клетки — на планшете Tharsis это видно на #14, где чёрный куб кладёт океан, а клетка сама поднимает температуру (закреплено тестом). Печатные предложения SETUP дословно совпадают с C19 ⇒ те же i18n-ключи (числа — параметры) |
| **C42 Nirgal Enterprises** | Стартовые метки: строительство + энергия + растение (ТРИ, в угловой рамке; плашки DRAFT PRIORITY нет — проверено кропом против C41, где напечатано и то, и другое). SETUP: Destroy Overachievement from the bonus deck. EFFECT: MarsBot has +2 on all awards (when considering funding, and during scoring). BEFORE ACTION PHASE: If generation 2-5 or 10+: Perform a Milestone action. If generation 6-9: Perform an Award action. Skip Failed Action if unable to perform these. | ✅ `MarsBotNirgalEnterprises.ts`. **КАРЬЕРА В ТРИ АКТА**: человеческая Nirgal платит за награды и достижения 0 M€; у бота цен нет вообще, поэтому его версия покупает то же самое ТЕМПОМ — бесплатная заявка достижения каждое поколение раннего этапа, бесплатное учреждение награды в середине и снова достижения, когда виден финал. +2 — то, что делает средний акт рабочим: награда, где бот отставал на два, становится той, где он ведёт, и та же +2 остаётся при финальном подсчёте. **+2 ВОЗВРАЩАЕТСЯ, А НЕ ПРИМЕНЯЕТСЯ**: сила бота в награде выводится ровно в ОДНОМ месте (`AutomaMAEvaluation.botAwardScore`), и все три потребителя читают её через `AwardScorer` — решение бота об учреждении, оверлей наград для игрока и финальный подсчёт; поэтому корпорация отвечает ЧИСЛОМ (новый хук `awardScoreBonus`), которое вывод прибавляет, а расчёт на месте вызова оставил бы двух других потребителей врать. Складывается с Easy-поправкой (−5), а не заменяет её; достижений правило не касается. **«SKIP FAILED ACTION IF UNABLE» — карта отменяет общее правило**: обычные трековые действия (`claimMilestoneAction` / `fundAwardAction`) в этом случае берут официальный Failed Action, а этот бокс — нет, поэтому он зовёт пару `tryClaimMilestone` / `tryFundAward` (тот же выбор и та же заявка без ветки компенсации); в тестах обе половины сравниваются с трековым действием, которое платит 5 M€ в том же состоянии. **ПОКОЛЕНИЕ 1 НЕ ПОПАДАЕТ НИ В ОДИН НАПЕЧАТАННЫЙ ДИАПАЗОН**, и это важно: RB-B резолвит Before-Action-Phase боксы и сразу после сетапа — бокс ПРЕДЛАГАЕТСЯ в 1-м поколении и корректно отказывается (закреплено тестом, счётчики читаются как ДЕЛЬТЫ). SETUP — общая операция `destroyBonusCard` (прецеденты C05/C21): уничтожается карта, вся суть которой — «заяви достижение / учреди награду», то есть корпорация, делающая это сама каждое поколение, убирает её, а не соревнуется сама с собой; C17 Vitor ту же B04 УБИРАЕТ В СТОРОНУ и возвращает каждое поколение — противоположное прочтение той же карты |
| **C43 Palladin Shipping** | Стартовые метки: космос + событие (ДВЕ, в угловой рамке) И DRAFT PRIORITY: Space > Event (напечатано И то, и другое — проверено кропом; форма C11/C41). SETUP: MarsBot gains 5 MC. Place white cubes on the space track on spaces #3, #4, #6, #8, #10, and #11. Place black cubes on the event track on spaces #3, #4, #6, #8, #10, and #11. EFFECT: When MarsBot advances onto a white or a black cube, move that cube to this card. Once there is one of both colored cubes on this card, remove one of each and MarsBot raises the temperature 1 step. | ✅ `MarsBotPalladinShipping.ts`. **ГРУЗ ИЗ ДВУХ ПОЛОВИН**: человеческая Palladin платит за карту, которая ОДНОВРЕМЕННО космос и событие (космическое событие), и превращает титан в шаг температуры. У бота нет ни руки, ни титана — та же конъюнкция заключается на единственной доске, которая у него есть: белые кубы ждут на треке КОСМОСА, чёрные на треке СОБЫТИЙ, и шаг температуры стоит по одному КАЖДОГО цвета. Обе стартовые метки и приоритет драфта кормят ровно эти два трека, так что вся партия — попытка держать оба в движении: серия из шести космических карт копит шесть кубов и не греет ничего. **КАРТА — НЕ СКЛАД, А СЛОТ СПАРИВАНИЯ**, и печатное правило делает это фактом модели, а не соглашением: пара уходит в тот же миг, когда на карте оказываются оба цвета, значит В ЛЮБОЙ МОМЕНТ ПОКОЯ на карте лежит РОВНО ОДИН цвет. Поэтому фреймворк оставил обычный единственный слот (`corpResources` — растение Ecoline, наука Spire, флаг C35) и добавил только КАКОГО ОН ВИДА (`corpResourceKind` + печатное `resourceAlt`): «два белых и один чёрный» становится НЕВЫРАЗИМЫМ, а не просто неверным, лицо рисует ОДНУ капсулу с тем цветом, что реально ждёт, и старые сейвы читаются как «объявленный вид» без миграции. **«MOVE THAT CUBE TO THIS CARD» ПРОИСХОДИТ ПЕРВЫМ**, и только потом проверяется пара — видно это ровно в счётчиках: куб, замкнувший пару, всё равно ПЕРЕЕХАЛ (`palladinCubesMoved`), он просто не остался лежать. Обратное прочтение недосчитывало бы каждый второй куб. **НИ ОДИН ЦВЕТ НЕ ГОВОРИТ «INSTEAD OF»** ⇒ общее правило RB-B: куб забирается ДО и В ДОПОЛНЕНИЕ к печатному значку клетки (закреплено на клетке #3 трека космоса, которая сама поднимает температуру). Полная температура пару не задерживает: кубы убираются, а подъём берёт общий Failed Action (прочтение C40/C45 — печатная цена тратится независимо от того, есть ли ещё то, что она покупает). Оба цвета делят ОДИН текст легенды (закон C30): что делает куб при попадании, от цвета не зависит, поэтому планшет рисует ОДНУ строку с ДВУМЯ образцами |
| **C44 Sagitta Frontier Services** | Стартовые метки: энергия + событие (ДВЕ, в угловой рамке); плашки DRAFT PRIORITY НЕТ (проверено кропом против C43, где напечатано и то, и другое). SETUP: MarsBot gains 8 MC. EFFECT: When resolving a card with no tags, MarsBot gains 10 MC (instead of 5MC). When resolving a card with exactly 1 tag, MarsBot gains 1 MC. | ✅ `MarsBotSagittaFrontierServices.ts`. **КОМПАНИЯ, КОТОРАЯ ВОЗИТ ТО, ЧТО НЕ БЕРЁТ НИКТО**: человеческая Sagitta платит 4 M€ за карту без меток и 1 M€ за карту ровно с одной — зарабатывает на тонком конце колоды. У бота та же сделка в его собственной экономике: карта без меток — это единственная вскрышка, которая не даёт ему НИЧЕГО (ни метки, ни шага трека — официальное неудачное действие), и корпорация УДВАИВАЕТ то, что эта неудача платит. Худшая карта колоды становится её лучшей. **СЧЁТ МЕТОК — СВОЙ, БОТОВСКИЙ** (`AutomaResolver.printedTags`), а НЕ человеческого близнеца: прочтения действительно расходятся (человек исключает WILD, бот считает — для него дикая метка это шаг трека), и решает печатная скобка «(instead of 5MC)»: 5 M€ — это компенсация за неудачное действие, а её триггер — ровно `printedTags(card).length === 0`. Любое другое прочтение «no tags» платило бы 10 за карту, которая не провалилась, либо оставляло настоящий провал на 5; вторая половина обязана считать ТАК ЖЕ, иначе одна карта была бы «без меток» для одного пункта и «с одной меткой» для другого. **ДВА ХУКА, ПОТОМУ ЧТО ДВА МОМЕНТА**: новый `failedActionCompensation` ЗАМЕНЯЕТ сумму ДО выплаты (одна строка журнала и одна запись хода называют 10 — число с карты, а не «5 и потом поправка»), и он ЧИСТЫЙ: счётчики и атрибуция живут в `onFailedAction` (C26), который фреймворк спрашивает ровно раз на неудачу. Вторая половина — обычный `onProjectCardResolving`. **EASY НЕ УМЕНЬШАЕТ ПЕЧАТНЫЕ 10**: и карта, и правило Easy называют АБСОЛЮТНЫЕ числа (неудача платит 3, а не «5 − 2»), поэтому корпорация ЗАМЕНЯЕТ константу, а не правит её; альтернативное прочтение (применить скидку Easy и к числу корпорации, то есть 8) записано как отвергнутое и закреплено тестом. Все ОСТАЛЬНЫЕ неудачи (максимум трека, нет места под тайл, три достижения) платят как платили |
| **C45 Spire** | Starting tag: Earth. DRAFT PRIORITY: Most tags. EFFECT: When resolving a card with 2 or more tags, place a science resource on this card. BEFORE ACTION PHASE: If there are 10 or more science resources on this card, remove 10 science resources from here, and MarsBot places a city tile and gains 1 TR. | ✅ `MarsBotSpire.ts`; город невозможен → Failed Action, −10 науки и +1 РТ всё равно резолвятся (две отдельные печатные части) |

**Ключевые правила RB-B, закреплённые в коде/тестах:**
- Setup 1: random select после розыгрыша корпорации человека, ДО прелюдий; коллизия → select another (мультиплеер-хоумрул: против ВСЕХ людей; canonical key = CardName оригинала).
- Setup 3–4: Setup box немедленно; starting tags «as if they are shown on a card revealed during play» → продвигают трек (действия клеток срабатывают: Earth[1]='city' у Spire реально ставит город при сетапе); «ongoing effect уже активен при сетапе».
- Setup 5: corp-specific bonus cards невыбранных корпораций не в игре — структурно (в пул попадают только карты активной корпорации).
- Драфт: pick по priority (несколько равных → random, seeded; ноль совпадений → random); дискард «reveal top → unprotected → discard, protected → set aside» = максимум 1 сброс; колода действий может быть из 5 карт.
- «Before Action Phase» боксы резолвятся и после сетапа, до первой фазы действий (гейт research → action, `corpBapGeneration`-гард).
- FAQ Ecoline: атака растений МОЖЕТ целить растение на карте корпорации; excess is lost; «not allowed to additionally destroy/steal from MarsBot's MC supply» (`AutomaTargeting.corpPlantPool/removeCorpPlants`, опции в `RemoveAnyPlants`/`StealResources`).
- «Most tags» = печатный ряд меток карты (`AutomaResolver.printedTags`: event-метка событий считается; wild считается — RB-B исключает wild только для СОВПАДЕНИЯ с tag-chain приоритетами). Зафиксировано тестом `MarsBotSpire.spec.ts`.

**RB-B FAQ — human-реакции на бота (реализовано 2026-08-19, `AutomaHumanTagReactions.ts`):**

| Правило | Статус |
|---|---|
| **Saturn Systems**: «triggered when you or MarsBot play a card with a Jovian tag; an advance tracker effect does NOT trigger it; a Jovian STARTING tag of MarsBot's corporation triggers it» | ✅ санкционированный диспатч из всех трёх точек резолва карты бота (флип хода / B03 / B07-fallback) через СОБСТВЕННЫЙ `onCardPlayedByAnyPlayer` карты; starting tags → существующий `onNonCardTagAddedByAnyPlayer` (тот же хук, что у Гидросети). Исключение tracker-advance — структурное: каскады не проходят через эти точки. Тесты: `AutomaHumanTagReactions.spec.ts` |
| **Pharmacy Union / Splice**: «MarsBot's starting corporation or any track or bonus effect gives it a microbe advancement (not a plant or animal) → resolve as if a card with a microbe was played» | ✅ флип с microbe-меткой → собственные хуки карт (science-половина PU own-plays-only по самому хуку); microbe advancement без карты (starting tag; **Venus-клетка 9** — печатная microbe-метка, `TrackDefinition.microbeTagCells`) → co-located `onMarsBotMicrobeAdvancement` (ICard). Splice: боту промпт запрещён → детерминированная M€-половина (микроб физически некуда класть — played pile хранит имена); владелец +2 M€ |
| **ПРАВИЛО ФОРКА (владелец, 2026-09-06, заменяет прежний allowlist): разрешённая ботом КАРТА ПРОЕКТА = «any player plays a card» для КАЖДОГО человеческого `onCardPlayedByAnyPlayer`-реактора** — обобщение прецедента Saturn Systems («triggered when you or MarsBot play a card»; продвижение трека НЕ триггерит — структурно). Единица срабатывания — карта, раз на каждое разрешение (доп. проекты B03/Helion/ThorGate/Valley Trust/PhoboLog/Tycho идут той же воронкой `resolveProjectCardForBot`); Failed Action замены движения НЕ гасит срабатывание (диспатч стоит ДО резолвера); вскрытие без резолва (tiebreak по стоимости, драфт, seed бонус-колоды) не срабатывает. Solar Logistics при космическом событии бота даёт добор через mandatory external-draw intake (`ExternalDrawIntake`). NON-CARD-пути (стартовые метки, microbe advancement клетки 9 Венеры) ОСТАЮТСЯ FAQ-allowlist'ом {Saturn Systems, Pharmacy Union, Splice}. Тесты: `AutomaHumanTagReactions.spec.ts` (полная таблица позитивов/негативов) | ✅ |

**RB-B, вне поддерживаемой матрицы (задокументировано, не реализовано):**
- Credit-жетоны на треках: тип (`'credit'`) и рендер уже есть в primitive кубов, но ни одна реализованная корпорация их не сеет — первая такая корпорация просто объявит их в своих `trackCubes`.
- Aphrodite/Lakefront Resorts/Pristar/Utopia Investments — Turmoil-часть FAQ; Turmoil вне матрицы. (Human Aphrodite от подъёмов Венеры ботом уже работает штатным engine-путём.)

### Grep-аудит promo-модуля (frame §7, выполнен 2026-07-19)

Ростер: 72 project + 9 prelude + 11 corporation (`PromoCardManifest.ts`). Кросс-игровые взаимодействия:
- **AUTO-COVERED generic-адаптерами:** SmallAsteroid / DeimosDownPromo (`removeAnyPlants` → attack-адаптер); ADS (protection); tile-триггеры с эффектом только на владельца-человека — Philares, NeptunianPowerConsultants, Hospitals, Vermin (тайлы бота легально триггерят; VP-штраф Vermin по городам бота — честный board-based и сохранён при отключении card-VP-цикла для бота); board-`{all}` требования (NewHolland / Supermarkets / OutdoorSports — считают ТАЙЛЫ, не теги).
- **NEVER-TRIGGERS для бота** (картами он не владеет / читает свою руку): PolderTechDutch (only-own-tiles), PublicPlans (своя рука), MarsNomads fan-out. ⚠️ УСТАРЕЛО для `onCardPlayedByAnyPlayer`-реакторов: с 2026-09-06 флипы бота диспатчатся во ВСЕ такие реакторы через `AutomaHumanTagReactions.onBotCardResolved` (см. правило форка в §10 ниже).
- **PER-CARD (закрыто):** LawSuit, StJosephOfCupertinoMission — таблица выше.
- Ни одна promo-карта не использует декларативный opponents-tag `Counter` (единственная такая — base Toll Station); положительного production оппонентам в promo нет — страхует guard в `Production.add`.

---

## 11. Hellas — правила карты (RB-C: Adding Expansions)

Статус: **закрыто по официальным источникам**, реализовано как MAP PROFILE
(`src/server/automa/boards/MarsBotMapProfile.ts`) поверх общего движка. Второй профиль после
Tharsis; Tharsis не менялся ни в одной клетке (регресс-тест — `HellasMarsBotBoard.spec.ts`).

### 11.1. Setup (RB-C p.8 + Setup Guide v1.3 шаг 18)
«Use the MarsBot board that corresponds to the map… Replace Corporate Competition (B08) with the
card of the same name that corresponds to the current map (B09–B13)» → профиль карты отдаёт
`corporateCompetition`, остальной состав бонусной колоды не трогается.

### 11.2. Вехи (RB-C p.9 + p.2 для Venus)
| Веха | Условие для бота |
|---|---|
| Diversifier | space 3 на КАЖДОМ треке; с Venus — «3 on **7 of the eight** tracks» (один трек заменяем, RB-C p.2) |
| Tactician | 35+ M€ |
| Polar Explorer | без изменений (3+ тайла бота в двух нижних рядах) |
| Energizer | трек **Power** ≥ 6 |
| Rim Settler | трек **Jovian/Science** ≥ 6 |

⚠️ Diversifier ≠ Planner: Planner **исключает** трек Венеры из набора, Diversifier **считает** его
и отбрасывает худший из восьми.

⚠️ **Эти числа — ВНУТРЕННИЕ и до UI не доходят.** Прогресс бота нормализуется на шкалу игрока
(`AutomaMAEvaluation.botMilestoneScore`, контракт §11.2a) — за столом бот читается как ещё один
игрок.

### 11.2a. НОРМАЛИЗАЦИЯ прогресса бота на шкалу игрока (правило форка, не RB)

Веха у бота меряется его единицами (M€ у Tactician, позиция трека у Rim Settler), у человека —
своими. Одно поле `MilestoneScore.score` под ОДНИМ печатным порогом смешивало обе шкалы, и список
вех читался бессмыслицей («Вы 2/5 · Бот 41»). Поэтому сервер переводит прогресс бота в единицы
игрока ещё до модели:

```
показать = floor(value * порог_игрока / порог_бота)
```

Tactician (бот 35 M€ ↔ игрок 5 карт с требованиями): 34 M€ → **4**/5, 35 → **5**/5, 40 → **5**/5,
50 → **7**/5. Rim Settler (6 ↔ 3) → половина трека; Diversifier (3 ↔ 8); Planner (4 ↔ 16); у
Builder / Energizer / Hoverlord шкалы совпадают и число не меняется.

`floor` здесь несущий: `floor(v·H/T) ≥ H` истинно ровно при `v ≥ T`, поэтому показанное число
пересекает порог на том же шаге, на котором срабатывает `botMilestoneMet` — клиент не может
нарисовать «может заявить» раньше или спрятать позже, и ему для этого не нужен код про бота.

**Награды НЕ нормализуются** и не должны: оценка награды сравнивается НАПРЯМУЮ с числами живых
игроков при раздаче 5/2 ПО (`AwardScorer` → `giveAwards`), то есть это уже общая валюта правил —
пересчёт заставил бы показанную гонку противоречить финалу.

▶ **Добавляя веху для бота на новой карте/расширении:** правило пишется ТОЛЬКО в
`AutomaMAEvaluation.botMilestoneProgress` как `value` + `target` (никогда как голый boolean) —
нормализация тогда включается сама. Guard-worklist: `tests/automa/AutomaMilestoneNormalization.spec.ts`
падает с точным списком вех, для которых решение не записано.

### 11.3. Награды (RB-C p.9)
| Награда | Оценка бота |
|---|---|
| Cultivator | без изменений (озеленения бота на поле) |
| Magnate | зелёные карты в **played pile** бота (у бота нет tableau — `award.getScore` вернул бы 0) |
| Space Baron | позиция трека **Space** |
| Excentric | `floor(M€ / 5)` |
| Contractor | позиция трека **Building** |
| Venuphile | позиция трека Венеры (только с Venus, RB-C p.3) |

⚠️ **Space Baron = SPACE-трек, не Science.** Источник §17.1 файла `1.txt` (OCR миниатюры
страницы компонентов RB-A) читает его как «Science track space» — это ошибка извлечения.
Печатная reference-страница RB-C p.9 показывает значок КОСМОСА, а сама карта B09 печатает
открытым текстом «Space Baron: Advance the space track».

### 11.4. Тайбрейкеры размещения (RB-C p.10)
Единый нумерованный список для всех карт; для Hellas активны шаги 1, 2 и 4:
1. смежность с максимумом океанов;
2. **Polar Region (два нижних ряда)** — только Hellas;
4. максимум покрытых бонус-иконок;
7. флип карты проекта (общий финальный тайбрейк).

Шаг 2 стоит МЕЖДУ океанами и иконками — не в конце. Tharsis печатает только шаги 1 и 4.

### 11.5. Южный полюс (RB-C p.11 «ADDITIONAL PLACEMENT BONUS RULES»)
Гекс с бонусом-океаном и −6 M€ (`SpaceName.HELLAS_OCEAN_TILE`):
- **есть свободные океаны И у бота ≥6 M€:** приоритет ВЫШЕ прочих 2-бонусных гексов (реализовано
  весом 2.5 — строго выше 2 и ниже 3, как явно оговорено у близнеца Terra Cimmeria); при
  постановке бот НЕ получает M€ за иконки, а ставит океан (+1 TR) и теряет 6 M€;
- **иначе:** «hex without rewards» для тайбрейкеров (вес 0) и «doesn't gain or lose anything»
  при постановке. Остаётся ЛЕГАЛЬНОЙ клеткой даже когда бот не может заплатить — человеческий
  путь легальности её отбрасывает, поэтому бот запрашивает тот же вопрос с зачётом стоимости.

### 11.6. B09 Corporate Competition — helper-действия (RB-C p.12 + скан карты)
Cultivator → озеленение +1 кислород · Magnate → вскрывать колоду до ЗЕЛЁНОЙ карты и разыграть её
(остальные в сброс) · Space Baron → продвинуть трек Space · Excentric → человек теряет самое
ценное животное/бактерию · Contractor → продвинуть трек Building · Venuphile → трек Венеры.
Резолвер ОБЩИЙ с B08 (`corporateCompetition`): различается только список helper'ов, который
живёт в одном `switch` по имени награды (имена уникальны в поддерживаемом наборе карт).

## 12. Elysium — правила карты (RB-C: Adding Expansions + карта B10)

Статус: **закрыто по официальным источникам**, реализовано как третий MAP PROFILE
(`src/server/automa/boards/MarsBotMapProfile.ts`) поверх общего движка. Ни Tharsis, ни Hellas не
менялись ни в одной клетке (регресс-тесты — `HellasMarsBotBoard.spec.ts`,
`ElysiumMarsBotBoard.spec.ts`).

### 12.1. Setup
Профиль отдаёт `corporateCompetition = B10`; остальной состав бонусной колоды не трогается
(Tharsis → B08, Hellas → B09, Elysium → B10).

### 12.2. Вехи (RB-C p.9 + p.2 для Venus)
| Веха | Условие для бота | Семья |
|---|---|---|
| Generalist | space **2 на каждом треке, КРОМЕ Венеры** | своё правило бота |
| Specialist | **любой** трек ≥ 10 (трек Венеры считается) | своё правило бота |
| Ecologist | трек **Bio** ≥ 4 | своё правило бота |
| Tycoon (форк: Tycoon10) | зелёные/синие карты в **played pile** бота | «Unchanged», но своя ветка |
| Legend | красные карты в **played pile** бота | «Unchanged», но своя ветка |

⚠️ **Generalist ≠ Diversifier.** На листе у Generalist напечатано «(excl. Ⓥ)», поэтому это форма
**Planner**: трек Венеры выбывает из набора целиком. Семь базовых треков на 2 при Венере на 0 —
веха ВЫПОЛНЕНА; максимальная Венера НЕ подменяет базовый трек, застрявший на 1. Diversifier
(Hellas) наоборот считает восемь треков и отбрасывает худший. Отдельный регресс-тест.

⚠️ **Specialist считает трек Венеры.** Исключение «(excl. Ⓥ)» напечатано только у Generalist, а
RB-C p.2 говорит «The Venus track and its actions behave identically to every other track» —
поэтому «any track» читается буквально. Решение зафиксировано тестом; если официальный FAQ
когда-нибудь скажет иное, менять надо ОДНУ ветку `botMilestoneProgress`.

⚠️ **Tycoon/Legend: «Unchanged» ≠ ветка `default`.** Правило действительно совпадает с человеческим,
но карты бота лежат в `automa.playedPile`, а не в tableau, поэтому `milestone.getScore(bot)` вечно
вернул бы 0. Ветка нужна ради КОНТЕЙНЕРА, а не ради правила; `target` берётся из собственного
порога вехи (`milestoneThreshold`), поэтому нормализация становится тождественной и строка бота
показывает ровно то число, что показала бы строка человека. Тот же приём, что у награды Magnate.
На Elysium форк ставит **Tycoon10** (10 зелёных/синих), а не классический Tycoon (15) — «unchanged»
поэтому переносит порог формы, а не число 15.

⚠️ **Порог для формы «10 из 10».** `guard`-тест `familyGuard`
(`tests/automa/AutomaMilestoneNormalization.spec.ts`) теперь ДОКАЗЫВАЕТ принадлежность к семье:
`PLAYER_METRIC` обязан совпадать с `getScore`/`canClaim` бота в каждом состоянии свипа, а
`BOT_METRIC` обязан хоть где-то разойтись. Записать веху не в ту семью больше нельзя молча.

### 12.3. Награды (RB-C p.9 + жёлтая сноска листа)
| Награда | Оценка бота |
|---|---|
| Celebrity | карты стоимостью 20+ M€ в **played pile**, **включая события** (в отличие от человека) |
| Industrialist | позиция трека **Power** + 5 |
| Desert Settler | без изменений (реальные тайлы бота в четырёх нижних рядах) |
| Estate Dealer | без изменений (реальные тайлы бота рядом с океанами) |
| Benefactor | **РТ бота − 15**, только для сравнения |
| Venuphile | позиция трека Венеры (только с Venus, RB-C p.3) |

⚠️ **Celebrity: 20 ровно засчитывается, события засчитываются.** Печатная награда у человека
события отбрасывает — это и есть заявленная на карте асимметрия.

⚠️ **Benefactor: −15 только в оценке.** Настоящий РТ бота не трогается: нет фейкового события, нет
изменения финального счёта. Значение живёт только в `botAwardScore`.

⚠️ **Industrialist — сила ЧЕЛОВЕКА тоже переопределена** (жёлтая сноска листа): *«MarsBot counts
your current steel resources, your current steel production, and your current power production.
(Your current power resources do not count since they cannot be carried over.)»* Это правило, а не
подсказка: печатная награда читает «most steel and energy», и после финального производства обычный
оценщик переключается на `steel + energy` — то есть считает ровно те ресурсы, которые сноска
исключает. Реализовано как `AutomaMAEvaluation.humanAwardScore`, который читает ТОЛЬКО `AwardScorer`
и только в automa-играх: одно число обслуживает решение бота о финансировании, оверлей и финальные
ПО, а игра без бота не меняется вовсе. В мультиплеере формула считается для КАЖДОГО человека, дальше
работает общее правило «сравнить с лучшим человеком».

### 12.4. Тайбрейкеры размещения (RB-C p.10)
Для Elysium активны шаги 1, 4 и 5:
1. смежность с максимумом океанов;
4. максимум покрытых бонус-иконок;
5. **Southern Region (четыре нижних ряда)** — только Elysium;
7. флип карты проекта (общий финальный тайбрейк).

⚠️ **Порядок ЗЕРКАЛЕН к Hellas.** У Hellas регион стоит шагом 2, ДО иконок; у Elysium — шагом 5,
ПОСЛЕ них. Северный гекс с двумя иконками выигрывает у пустого южного. Переиспользовать профиль
Hellas с косметической заменой региона нельзя, и спек это проверяет с обеих сторон.

⚠️ **Общие правила размещения остаются первичными.** Профиль карты подключается только внутри
`AutomaTilePlacer.breakTie`, то есть ПОСЛЕ печатных стратегий тайла (озеленение — максимум своих
городов и минимум чужих; город — максимум смежных озеленений) и после Ares-фильтров. Карта решает
ничью, а не заменяет алгоритм.

**Южный регион = четыре нижних ряда (y 5–8), ОДИН предикат** — `Board.isSouthernRegion`. Его
читают награда Desert Settler, тайбрейкер Elysium и helper B10 Desert Settler. Полярный близнец
Hellas (`Board.isPolarRegion`, y 7–8) выделен туда же. Три определения одного печатного региона —
именно тот способ, которым они расходятся.

⚠️ **Никакой протечки Hellas.** У Elysium нет ни шага Polar Region, ни правила Южного полюса
(−6 M€ / бонус-океан): этот гекс существует только на Hellas (`isHellasSouthPole` проверяет имя
карты). Отдельный тест.

### 12.5. B10 Corporate Competition — helper-действия (скан карты)
Резолвер ОБЩИЙ с B08/B09 (`corporateCompetition`): различается только список helper'ов в одном
`switch` по имени награды. Общий каркас не меняется — только оплаченные награды, «leftmost при
равенстве», невозможный helper пропускается, оплата 5 M€ ТОЛЬКО после успешного helper'а, ни одного
успешного → без оплаты и с добором ещё одной бонусной карты как ДОЧЕРНЕГО шага того же потока.

| Награда | Helper | Невозможен, когда |
|---|---|---|
| Celebrity | вскрывать колоду проектов до карты **20+ M€**, разыграть её (остальные в сброс) | в колоде нет карты 20+ |
| Industrialist | продвинуть трек **Power** | трек на 18 |
| Desert Settler | озеленение **в Южном регионе** + кислород 1 | нет легальной клетки В РЕГИОНЕ |
| Estate Dealer | озеленение **рядом с океаном** + кислород 1 | нет легальной клетки у океана |
| Benefactor | **+2 РТ** боту | никогда (всегда возможен) |
| Venuphile | трек Венеры | трек на максимуме / нет Venus |

⚠️ **Оба ограничения на озеленение — ЖЁСТКИЕ, не тайбрейкеры.** Кандидаты = «легальные клетки для
озеленения бота» ∩ ограничение. Пусто ⇒ helper НЕВОЗМОЖЕН: карта переходит к следующей оплаченной
награде, а не ставит озеленение мимо ограничения и не тратит Failed Action. Реализовано через
`AutomaTilePlacer.placeGreenery(game, {restrict, onEmpty: 'impossible'})` — тот же общий конвейер
(владение тайлом, M€ за покрытые иконки, кислород, РТ, бонусы, триггеры «любого игрока», Ares,
тайбрейкеры карты), поэтому Ares/Delta/мультиплеер/анимации/рекордер получают обычное размещение.
Заодно это убрало старую двойную проверку: helper раньше спрашивал легальность у `game.board`
напрямую, а ставил через фильтрованный список плейсера — расхождение (все клетки под hazard'ом
Ares) дало бы Failed Action И оплату 5 M€.

⚠️ **Celebrity переиспользует общий reveal-until** (`revealUntilAndResolve`), появившийся для
Magnate: у Hellas предикат «зелёная», у Elysium «cost ≥ 20». Второго цикла вскрытия нет. Отвергнутые
карты сбрасывает сам поиск колоды, одной публичной строкой «Discarded N cards» — на каждую
отвергнутую карту отдельного foreground-уведомления не бывает.

⚠️ **Benefactor даёт именно РТ**, а не глобальный параметр и не тайл: `increaseTerraformRating(2)`.

## 13. Utopia Planitia — правила карты (RB-C: Adding Expansions + карта B11)

Статус: **закрыто по официальным источникам**, реализовано как ЧЕТВЁРТЫЙ map profile поверх общего
движка. Ни Tharsis, ни Hellas, ни Elysium не менялись ни в одной клетке (регресс-тесты —
`HellasMarsBotBoard.spec.ts`, `ElysiumMarsBotBoard.spec.ts`, `UtopiaMarsBotBoard.spec.ts`).

### 13.1. Setup
Профиль отдаёт `corporateCompetition = B11` (Tharsis → B08, Hellas → B09, Elysium → B10,
Utopia → B11); остальной состав бонусной колоды не трогается.

### 13.2. Имена слотов: форк ≠ печатная карта
Форк схлопнул клоны вех/наград, поэтому слоты этой карты живут под своими каноническими именами.
Это НЕ переименование правил — критерий бота привязан к тому имени, которое реально попадает в игру:

| На reference card | В коде (форк) | Почему |
|---|---|---|
| Specialist | `Land Specialist` | «Specialist» уже занят Элизием |
| Trader | `Tradesman` | `Trader` — deprecated клон `Tradesman` |
| Metallurgist | `Smith` | `Metallurgist` — deprecated клон `Smith` |
| Suburban | `Edgedancer` | одна и та же метрика «тайлы по краю поля» |

Устаревшие имена (`Trader`, `Metallurgist`) оставлены алиасами в `botMilestoneProgress`, чтобы
старое сохранение продолжало считаться.

### 13.3. Вехи (RB-C p.9)
| Веха | Условие для бота | Семья |
|---|---|---|
| Specialist (`Land Specialist`) | **3+ УНИЧТОЖЕННЫХ** бонусных карты | своё правило бота |
| Trader (`Tradesman`) | Jovian/Energy, Earth/City **и** Венера — все на space 2+ | своё правило бота |
| Metallurgist (`Smith`) | Building + Space **суммарно** 7+ | своё правило бота |
| Researcher | Science на space 4+ | своё правило бота |
| Pioneer | *Unchanged* — 3 колонии | **default**: колонии бота настоящие |
| Hoverlord | *Unchanged* — 7 флоатеров | существующая ветка Venus |

⚠️ **Specialist читает пул УНИЧТОЖЕННЫХ карт** (`destroyedBonusCards`), а не сброс поколения.
⚠️ **Trader берёт МИНИМУМ из трёх названных треков** — трек Венеры входит в набор, поэтому без
Venus Next веха честно недостижима (читает 0), а не «почти выполнена».
⚠️ **Pioneer — контрольный случай для семьи `default`:** формулировка «Unchanged» такая же, как у
Tycoon/Legend Элизия, но здесь она действительно ведёт в `default`, потому что колонии бота лежат
ровно там, куда смотрит игроцкий эвалюатор (`getColoniesCount`). У Tycoon/Legend не так — карты бота
лежат в `automa.playedPile`, а не в tableau.

### 13.4. Награды (RB-C p.9)
| Награда | Сила бота |
|---|---|
| Suburban (`Edgedancer`) | *Unchanged* — настоящие тайлы по краю поля (**default**) |
| Investor | клетка трека Earth/City |
| Botanist | клетка трека Bio **минус 2** (только для оценки; сам трек не двигается) |
| Incorporator | карты стоимостью **≤ 10 M€** в стопке разыгранных, **включая события** |
| Metropolist | *Unchanged* — настоящие города (**default**) |
| Venuphile | клетка трека Венеры |

### 13.5. Тайбрейкеры размещения (RB-C p.10)
Для Utopia активны шаги 1 и 4:
1. **Максимум соседних океанов**;
4. **Максимум закрытых наградных иконок**, причём *«On Utopia Planitia, edge spaces are considered
   to have an additional reward icon for purposes of tiebreakers»*;
7. флип карты проекта.

⚠️ **Край — НЕ отдельный шаг.** Это буллет ВНУТРИ шага 4, то есть арифметика:
`effective = printed + (край ? 1 : 0)`. Поэтому клетка с 2 печатными иконками в глубине поля
по-прежнему бьёт пустую клетку на краю (0+1), а 1+1 бьёт просто 1. Отдельный шаг «предпочитать край»
ранжировал бы их наоборот. В коде это `MarsBotMapProfile.tiebreakRewardBonus` — декларативные данные
карты, а не новый шаг.

⚠️ **Только для тайбрейка.** Выплата остаётся `botCoveredIconMegacredits` (1 M€ за ПЕЧАТНУЮ иконку),
поэтому за виртуальную краевую иконку бот денег не получает.

⚠️ **Канонический предикат края — один:** `MarsBoard.isEdge` (множество-близнец уже существовавшего
`getEdges()`). Его читают награда Edgedancer, тайбрейкер Utopia и helper B11 Suburban.

⚠️ **Никакой протечки.** У Utopia нет ни шага Polar Region (Hellas), ни Southern Region (Elysium),
ни пары Jovian+Science.

### 13.6. B11 Corporate Competition
Общая рамка не менялась: 5+ M€ → ближайшая оплаченная награда (leftmost при равенстве), невозможный
helper пропускается, оплата 5 M€ только после успешного helper'а, иначе — добор ещё одной бонусной
карты тем же связанным flow.

| Награда | Helper |
|---|---|
| Suburban (`Edgedancer`) | озеленение **на краю поля** + O₂ (жёсткое ограничение) |
| Investor | продвинуть трек Earth/City |
| Botanist | продвинуть трек Bio |
| Incorporator | вскрывать колоду до карты **≤ 10 M€**, разыграть её |
| Metropolist | поставить город (обычный конвейер) |
| Venuphile | продвинуть трек Венеры |

⚠️ **Suburban — жёсткое ограничение, а не тайбрейк:** кандидаты = легальные клетки озеленения ∩
край поля. Пусто → helper невозможен, карта идёт к следующей награде; «поставить озеленение
где-нибудь ещё» не бывает.

⚠️ **Incorporator использует ОБЩИЙ примитив вскрытия** (`revealUntilAndResolve`) — тот же, что
Magnate (B09, «зелёная») и Celebrity (B10, «≥ 20»). Здесь предикат «≤ 10», ровно 10 подходит.
Второго цикла вскрытия нет.

## 14. Terra Cimmeria — правила карты (RB-C: Adding Expansions + карта B12)

Статус: **закрыто по официальным источникам**, реализовано как ПЯТЫЙ map profile. Ни одна из
предыдущих карт не менялась ни в одной клетке.

### 14.1. Setup
Профиль отдаёт `corporateCompetition = B12` (Tharsis → B08, Hellas → B09, Elysium → B10,
Utopia → B11, Terra Cimmeria → B12).

### 14.2. Имена слотов: форк ≠ печатная карта
| На reference card | В коде (форк) | Метрика человека |
|---|---|---|
| Specialist-слот → Planetologist | `Planetologist` | 2 Earth + 2 Venus + 2 Jovian метки (6) |
| Architect | `Architect` | 3 метки города (3) |
| Coast Guard | `Coastguard` | тайлы рядом с океаном (3) |
| Forester | `C. Forester` | 3 производства растений (3) |
| Financier | `Fundraiser` | 12 производства M€ (12) |
| Suburban-аналог → Founder | `Founder` | тайлы рядом с особыми тайлами |
| Zoologist | `A. Zoologist` | животные/бактерии |

⚠️ Порог Coast Guard на этом борде — **3**, а не 4 как на печатном player aid. Веха «Unchanged»,
поэтому бот берёт ТОТ порог, который реально стоит в игре, каким бы он ни был.

⚠️ **Коллизий имён нет** — но это не закон домена: `Forester`, `Planetologist`, `Architect`,
`Incorporator`, `Forecaster`, `Zoologist` повторяются в ПОЛНОМ списке бордов форка, и лишь
поддерживаемая пятёрка не пересекается. Инвариант, на котором держится switch по имени, закреплён
тестом `noNameServesTwoBoards` — он упадёт с именем-нарушителем в тот день, когда новая карта его
сломает.

### 14.3. Вехи (RB-C p.9)
| Веха | Условие для бота | Семья |
|---|---|---|
| Planetologist | Jovian/Earth **+** Венера, суммарно 5+ | своё правило бота |
| Architect | City/Science 6+ | своё правило бота |
| Forester (`C. Forester`) | Bio 10+ | своё правило бота |
| Financier (`Fundraiser`) | Event 10+ | своё правило бота |
| Coast Guard (`Coastguard`) | *Unchanged* — тайлы рядом с океанами | **default** |
| Hoverlord | *Unchanged* — 7 флоатеров | существующая ветка Venus |

⚠️ Без Venus Next трек Венеры отсутствует и даёт 0 — то же чтение, что у всех остальных
критериев, называющих Венеру. Planetologist при этом остаётся достижимым одной строкой Jovian/Earth.

### 14.4. Награды (RB-C p.9)
| Награда | Сила бота |
|---|---|
| Electrician | клетка отдельного трека Energy |
| Founder | *Unchanged* (**default**) — но Neural Instance считается особым тайлом ТОЛЬКО для бота |
| Mogul | клетка самого продвинутого трека × 2 |
| Zoologist (`A. Zoologist`) | клетка Bio **+ 5** |
| Forecaster | `floor(M€ / 7)` — каждые 7 M€ = 1 карта с требованием |
| Venuphile | клетка трека Венеры |

⚠️ **Асимметрия Founder на стороне ЧЕЛОВЕКА.** «Neural Instance counts as a special tile for
MarsBot but not for you» — у бота обычный канонический классификатор (он и так отвечает TRUE),
а людям тот же подсчёт выполняется по множеству БЕЗ этого тайла
(`AutomaMAEvaluation.humanAwardScore`). Подсчёт один — `Founder.count(player, isSpecial)` принимает
множество аргументом, поэтому два прочтения не могут разойтись. Глобальная семантика
`isSpecialTile` не менялась.

### 14.5. Тайбрейкеры размещения (RB-C p.10)
Для Terra Cimmeria активны шаги 1, 3 и 4:
1. максимум соседних океанов;
3. **соседство с одним или более особым тайлом** (включая Neural Instance);
4. максимум закрытых наградных иконок;
7. флип карты проекта.

⚠️ Шаг 3 — **ДА/НЕТ, а не счёт**: правило говорит «one or more», поэтому два особых соседа не
лучше одного, и такие гексы падают на шаг 4 вместе. Считать количество значило бы придумать
предпочтение, которого в рулбуке нет. Покрыто отдельным тестом.

⚠️ Особый тайл определяется каноническим доменным классификатором (`isSpecialTileSpace` →
`common/TileType.isSpecialTile`), который уже отвечает TRUE для Neural Instance. Один предикат на
тайбрейкер, награду Founder и helper B12 Founder.

### 14.6. MSL Curiosity
Гекс с печатным `SpaceBonus.COLONY` — близнец южного полюса Эллады, и живёт в тех же трёх хуках
(`AutomaPlacementBonus`):

| Состояние | Условие | Вес для тайбрейка | При размещении |
|---|---|---|---|
| ACTIVE | Colonies ON **и** есть плитка без колонии бота **и** у бота ≥5 M€ | 2.5 (выше 2-иконочных, ниже 3-иконочных) | колония через `botBuildColony` (+2 ресурса на склад) и −5 M€; печатные 2 иконки НЕ выдаются |
| INACTIVE | нет плитки **или** <5 M€ | 0 («гекс без наград») | ничего не получает и ничего не платит |
| ABSENT | Colonies OFF | — | борд сам вычищает бонус → «treat this hex as empty for both» |

⚠️ Вес 2.5 меняет **только шаг 4**. Океаны (шаг 1) и особые тайлы (шаг 3) по-прежнему решают
раньше — отдельный тест.

⚠️ Неактивный гекс остаётся **легальным** для бота: человеческий путь отсекает его дважды (по
5 M€ и по «у игрока есть колония, которую он может построить»), а официальная формулировка «if
MarsBot places on here, it doesn't gain or lose anything» предполагает, что поставить туда МОЖНО.
Восстанавливается тем же приёмом, что и южный полюс.

### 14.7. B12 Corporate Competition
| Награда | Helper |
|---|---|
| Electrician | продвинуть трек Energy |
| Founder | город **рядом с особым тайлом** (жёсткое ограничение; LNI считается) |
| Mogul | продвинуть самый продвинутый трек; при равенстве — **верхний** по порядку строк профиля |
| Zoologist | продвинуть Bio (обычное «как от метки Animal» продвижение) |
| Forecaster | вскрывать колоду до карты **с требованиями**, разыграть её, **+5 M€** |
| Venuphile | продвинуть трек Венеры |

⚠️⚠️ **FORECASTER НЕ ПЛАТИТ 5 M€.** Adding Expansions прямо оговаривает, что этот вариант не
стоит боту обычных 5 M€. Реализовано как платёж, которого НЕ ПРОИСХОДИТ (`helperIsFree`), а не как
пара +5/−5: журнал, статистика, «Разбор хода» и реплей читают СОБЫТИЯ, и взаимоуничтожающаяся пара
показала бы игроку −5 M€, которых по правилам не было. Чистый эффект — **+5 M€**.

⚠️ «Карта с требованием» — каноническая метадата `card.requirements`, никогда не отрендеренный
текст. Требование здесь — только критерий ПОИСКА: бот не проверяет, выполнимо ли оно.

⚠️ Reveal-until — тот же общий примитив, что у Magnate (B09), Celebrity (B10) и Incorporator
(B11). Второго цикла вскрытия нет.
