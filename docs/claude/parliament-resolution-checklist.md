...
3. `winnerReward?` — часть победителя как ДАННЫЕ (`winnerReward.ts`: какой тайл, какой параметр двигает; **или ШАГ
   параметра** — `{kind: 'parameter', parameter, steps}`, исполняемый общим `winnerParameterStep`, НАГРАЖДАЕМЫЙ; раздел RX23 ниже).
...
7. `passive?` — с обязательным `forecast` (закон честности прогноза); `action?` — с `preview`. **Живой пассив
   ...
   `forecast` — двойник скидки есть сам разбор (факта не давать). **Пассив-ЦЕННОСТЬ** (RX19) — хук `resourceValueBonus(player, resource)`, спрашиваемый
   аксессорами `getSteelValue` / `getTitaniumValue` (поле игрока НИКОГДА не пишется законом), `forecast` — двойник есть сам
   рейт на бейдже и в строке оплаты (факта не давать); раздел RX19 ниже.
...
### Счётный член по КОЛОНИЯМ — число путём `Counter`, объяснение списком тайлов (RX17, 2026-09-24)

```ts
scaled: [{id: 'titanium', unit: {kind: 'stock', resource: TITANIUM}, perInfluence: 1, recipient: 'each'},        // RX09 дословно
         {id: 'production', unit: {kind: 'production', resource: MEGACREDITS}, perInfluence: 0, count: {id: 'colonies', per: 1}, cap: 5, recipient: 'each'}],
// resolutionCountKind('colonies') === {kind: 'colonies'}; число — ColoniesHandler.coloniesOf(game, player).length
ctx.report({kind: 'production', ..., count, counted: [], countedColonies: [...counted.colonies], uncapped, amount, before, after});
ctx.report({kind: 'skipped', ..., amount: 0, reason: 'No colonies'});   // ноль колоний — штатный исход, не «нет влияния»
```

- **Вид `colonies`** — шестой рядом с `cards` / `tags` / `board` / `threshold` / `production`: считает КУБИКИ игрока на
  тайлах колоний (два кубика на одном тайле — 2, чужой кубик на том же тайле — 0), никогда тайлы. **Число — движка**:
  общий хелпер `ColoniesHandler.coloniesOf(game, player)` (один элемент на кубик, порядок стола) читают `Counter`
  (countable «colonies»), `Player.getColoniesCount` и `resolutionCount` → `countColoniesToward` — три читателя, одно
  правило; свой обход колоний в карте не пишется (инвариант 7). Гейт дополнения — внутри хелпера.
- **Объяснение числа — СПИСОК ТАЙЛОВ в той же модели**: `ResolutionCountModel.colonies` (имя на кубик), в записи и модели —
  `countedColonies`, в чтении — `InfluenceYield.countedColonies`; слова — `countedColonyNames` («Луна ×2 · Титан»: соседние
  одинаковые имена сливаются в ×n). Второй модели счёта нет. **Реестр RX07 (`colonyBonuses`) НЕ переиспользуется**: он про
  бонусы и по ТАЙЛАМ (один ряд на тайл, `includes`), а счёт — по кубикам и не должен зависеть от присутствия RX07 в каталоге.
- **`compatibility` не объявлять**: колонии в Redux обязательны (формулировка RX09); значок колонии в поле скана —
  маркер исходной совместимости, не наш гейт.
- **Потолок — на той части, под которой напечатан**: «(Max. 5)» стоит под производственной формулой → `cap: 5` у
  производства, титан не ограничен (каталог § 4.1 «+1 пр. M€ за колонию (max 5)»). **Влияние в производственную часть НЕ
  входит** (`perInfluence: 0` — на карте его там нет): новый случай для блока чтения — `yieldInfluenceEnters(effect)`
  гейтит член формулы («1 [пр. M€] / [колония] · не более 5», без «+ 0 / [влияние]») и вход чтения («[колония] 4 → +4»,
  без «+ [влияние] 3»); запись влияние несёт, формула его не использует. Не путать с плоской частью (`yieldIsFlat` — ставка 0
  БЕЗ счёта, с базой).
- **Два горизонта без платы**: `productionHorizonOn(effects, effect, levy)` — производственная часть рядом с частью,
  ...
  для блока, бюджета слов панели и осмотра. Карта только с производством (RX02) горизонта не несёт — ей нечего
  различать. Лента заседания горизонт не печатает (решение RX15).
- **Два независимых пропуска**: титан при влиянии 0 — «No influence» (шаг RX09), производство при нуле колоний — «No
  colonies»; карта платит одну часть и пропускает другую, чтения называют каждый своей причиной; «Для вас» при нуле —
  «Сейчас колоний нет — считать нечего», никогда «ни одна карта».
- **Глиф счётного объекта** — `{kind: 'colony'}`: тайл колонии лица (`b.colonies()` → `tiles/colony.png`), без искры и
  числа; **семейство стенда `counted-colonies`**: место несёт `colonies` (имя на кубик), панель «Колонии» — чип на кубик
  (диск планеты спрайтом `<Colony>-background`, кубик места, имя), кубик сверх потолка зачёркнут; сценарии — 0 / 1 / 4 / 5 / 7
  кубиков, влияние 0 при трёх колониях, места, запись, зритель, задание 0/2, живой `parliament-jovian-vote`.
- **Очерёдность**: шаг колоний фазы (`endGenerationForColonies`: треки, флоты, гость) идёт ДО заседания и кубиков не
  двигает — спек закрепляет «счёт до фазы = счёт в записи»; титан в запасе сразу, производство впервые платит в
  следующем поколении.
- Ловушки: имя карты «Jovian Tax Rights» УЖЕ переведено в `turmoil_events.json` (глобальное событие Turmoil) — ключ имени
  в `parliament.json` не добавлять; RU-форма «у вас нет …» запрещена глоссарием (доступ читается «доступен · …») — «Для
  вас» при нуле формулировать без неё; fullscreen-осмотр рисует блок чтения с `formula: false` (формулу печатает сама
...
### Пассив-ЦЕННОСТЬ РЕСУРСА — надбавка на ЧТЕНИИ аксессора, поле не трогать (RX19, 2026-09-25)

```ts
passive: {
  resourceValueBonus: (player, resource) => resource === STEEL || resource === TITANIUM ? 1 : 0,   // ОДНА функция: чистый запрос
  forecast: () => [],   // двойник ценности — сам РЕЙТ там, где решают (бейдж рельсы, «×3» в строке оплаты); факта не давать
},
scaled: [DEVELOPMENT_CRAZE_STEEL /* дословно */, JOVIAN_TAX_RIGHTS_TITANIUM /* дословно */],   // две выплаты = два шага, две записи
```

- **Ценность ресурса в движке — изменяемое СЕРИАЛИЗУЕМОЕ поле игрока** (`Player.steelValue` / `titaniumValue`,
  `increase*` / `decrease*` с полом `> 0`). Закон его НЕ ПИШЕТ: хук `ResolutionPassive.resourceValueBonus(player, resource)`
  спрашивается диспетчером `ParliamentHandler.resourceValueBonus` (принятая карта, гейт `participates`, без области
  события — здесь только читают) из самих аксессоров `getSteelValue()` / `getTitaniumValue()` = поле + запрос. Поэтому:
  сериализация не меняется ни на байт, сохранение из-под закона грузится правильно по построению, аддитивность с
  картами ценности бесплатна (в поле — шаг карты, на аксессоре — шаг карты + закон), закон уходит из ENACTED — надбавка
  уходит с ним в тот же момент. `increase*` на принятии / `decrease*` на смене закона — ЗАПРЕЩЕНО (второе состояние
  «уже начислил», пол снимает чужую единицу, повторное принятие и отмена фазы рассинхронизируют пару, дрейф невидим).
- **Один аксессор — один читатель поля.** Перед сдачей — `grep steelValue|titaniumValue` по всему `src/`: сырое чтение
  поля мимо аксессора тихо теряет закон (`spendableMegacredits` в ветке Luna Trade Federation так и жил — проверка
  «хватит ли денег» расходилась с ценой). `serialize` / `deserialize` читают ПОЛЕ намеренно. Спек-ловушка запрещённой
  реализации: сохранить из-под закона → в поле 2 / 3, на аксессоре 3 / 4; копия читает надбавку заново.
- **Что приходит бесплатно, когда чтение правильное**: `payingAmount` (сталь, титан, floodgate-сталь) → `canAfford`,
  `checkPaymentAndPlayCard`; ветка LTF (множитель − 1 ПОСЛЕ ценности); `PublicPlayerModel.steelValue / titaniumValue`
  → `paymentPlan.rateFor` (панель оплаты, редактор), МС-бейджи рельсы (`railValueModel` — «2 → 3», «3 → 4» в момент
  принятия, без перезагрузки), `effectForecast.paymentValues`; `Merger`, `IndustrialComplex`. Ценность ≠ скидка:
  `getCardCostBreakdown` не трогается, `discounts: []`, цена печатная.
...
### Награда победителю ШАГОМ ПАРАМЕТРА — с РТ и бонусами трека, в отличие от мирового шага (RX23, 2026-09-25)

```ts
winnerReward: {kind: 'parameter', parameter: 'temperature', steps: 2},                       // третий вид: тайл | колония | ШАГ
winnerSteps: [winnerParameterStep(MOHOLE_CONTEST_ID, MOHOLE_CONTEST_TEMPERATURE)],           // ОДИН общий исполнитель, ключ = id параметра
// запись: {kind: 'globalParameter', part: 'winner', amount: 2, parameter: {id, before, after}, tr: 2}   — БЕЗ unrewarded
```

- **Награда победителя бывает ШАГОМ ПАРАМЕТРА — с РТ и бонусами трека, в отличие от мирового шага.** Мировой ход
  (RX12/RX14) делает закон для всех и передаёт `{unrewarded: true}` ЯВНО; шаг победителя делает ИГРОК, и общий
  исполнитель `winnerParameterStep` этого слова не знает: политическая фаза — `PARLIAMENT`, гейт движка платит
  поднимающему +1 РТ за сделанный шаг, бонусы −24/−20 °C (и реакцию правящих Зелёных на производство), его хуки, океан
  на 0 °C. Скопировать `unrewarded` из `GasExport.ts` — отнять у победителя РТ; гард: запись победителя с `unrewarded`
  падает, запись без `tr ≥ шагов` падает. `tr` в записи ИЗМЕРЕН вокруг вызова движка, не выведен из объявления.
- **Объявление — член объединения `WinnerRewardDeclaration`, и из него выводится всё**: ключ шага
  (`winnerParameterStepKey` = id параметра; гард требует шаг под этим ключом и запрещает шаг без объявления), комната
  (`winnerParameterRoom` принимает тайл | шаг и несёт `steps` / `applied`; арифметика — общая `parameterRoom`, у которой
  теперь есть `heatProductionBonus` и `oceanBonus` для подъёма температуры), РТ (`winnerRewardTr` → `applied`), чтения,
  семейство стенда (`winner-tile`). Океанов в виде `parameter` нет: океан — тайл.
- **Потолок называется**: один шаг до максимума — один шаг, запись с фактическим числом; на максимуме — `skipped` с
  причиной параметра (`WINNER_STEP_AT_MAXIMUM_REASON`), тепло всем выдано; часть победителя от влияния не зависит.
- **Хвост мутирующего шага дренируется СРАЗУ** (`stepEffects`: `drainDeferred` после каждого шага места/мира/победителя) —
  до RX23 отложенная работа мутирующего шага шла после обхода всех мест, и «фаза ждёт океан» работало только у
  спрашивающих шагов (RX01/RX03). Модель публикует хвост как ask шага (`pending` из окна реакций `scan`, когда никто не
  спрашивает и промпт не гейт) — другие места читают «синий размещает», само место — шаг заседания.
- **Клиент: хвост-размещение** (`sittingTailPlacementOf`: `space` без источника-резолюции + `pending` сервера на зрителя
  с `input: 'space'`) → `rewardStep = 'placement'`, `position.tail = true`: страница НАГРАДА держится (`owed-step`),
  крошка «РАЗМЕЩЕНИЕ», хвост стоит **за той же дверью, что тайл** (`parliamentPlacementDoorClosed` — допущенный сразу,
  он забирал экран до волны и секция сбрасывала волну неигранной), но дверь открывает ОБХОД заседания, когда страница
  тиха (волна села, ничего не хостится), а не нажатие — глагола «К полю» у хвоста нет. Планетарный
  такт обобщён: `isPlanetRecord` = `globalParameter` мировая без места ИЛИ с местом и `part: 'winner'`;
  `runWorldMoveBeat(…, {boardBusy})` не возвращает стек, пока у зрителя стоит `space`-промпт — один рейс к полю на глайд
  маркера и океан; `mayYieldForWorld` пускает хвост (резолюционный тайл по-прежнему ждёт нажатия).
