...
## ⭐⭐ CONSUMER UPDATE BOUNDARIES (2026-09-03) — the ingest module, and why «the server half is green» proved nothing

**The Research Outpost forensic audit.** Report: two humans; B plays «Исследовательская станция» (Research Outpost, base #020, `city: {on:'isolated'}`); the city lands, A's Tharsis Republic correctly defers +1 M€ production — and A's console never presents the band. The SERVER half was provably intact (event recorded in B's correlation, attributed to Tharsis, `diffRootNotifications` builds the right positive model — the S8 corpus was genuinely green). The loss lived in the one link no audit had ever executed: **the NotificationLayer's multi-pass orchestration**, where the diff-signature skip, the PREPARING gate, the seen-sets and `firstSeed` interact across fetches.

**The consumer's decision core is now a MODULE: `notificationIngest.ts`** (`applyNotificationDiff` — extracted 1:1 from `NotificationLayer.applyDiff`, the component only feeds it the fetched payload + view signals). That is what makes the orchestration testable against a REAL game across REAL update sequences.

**The three boundary rules (each was a shipped defect):**
1. **A signature-equal pass still runs the PREPARING release.** The ingest signature (`undoCount:gen:messages.length:events.length`) sees only the STREAMS — but `openEventCorrelations` rides the playerView, and the layer's poller legitimately fetches CLOSED streams while the transport has not yet applied the fresh view (WS wake → both fire; the view apply can also sit behind a cinematic hold). That race pass stashes a COMPLETE model against the STALE open set; every later pass has equal stream lengths, and the old early-return skipped `takePreparedModels` entirely — the band froze in PREPARING until an unrelated stream growth, and the 90 s ceiling was DEAD CODE behind the same skip. Now only the O(generation) rebuild is skipped; the O(|preparing|) release + ceiling run on every pass, through the same `presentRootModels` funnel (one presentation policy, no bypass, no double).
2. **The first seed stashes OPEN chains instead of consuming them.** «Old news never presents» is right for CLOSED chains only. A chain still open at seed time (A reconnects while B picks the cell) has impacts that HAVE NOT HAPPENED YET — marking it seen swallowed the later payout permanently, and the asymmetry was cruel: a LOSS survived via the standalone hostile fallback diff, a GAIN had no fallback at all. The seed now stashes open-chain models in PREPARING; they release complete when the chain closes.
3. **View and streams may be from different moments — pair them accordingly in tests.** Any consumer test that feeds one atomic (view+streams) snapshot is testing a sequence production never sees.

**Why every previous audit missed it:** they verified at three altitudes — server events (`openEventCorrelations.spec`), one-shot pure diff with an empty seen-set (`crossPlayerDeliveryAudit` — its `deliveredModels` helper bypasses the layer entirely), and browser e2e (whose std-project door has NO open-prompt window: a pay-on-commit chain is rooted AT the commit, born closed, so the PREPARING path never engages — while a card-play chain is born OPEN and lives across the placement prompt). The middle layer was never executed by any test at any point in history (verified against git). «Каждая карта классифицирована» was source coverage; the defect was in an EXECUTION-SEQUENCE topology no card list can see.

**Guards (the worklist):**
- `tests/notifications/consumerDeliverySequences.spec.ts` — R1 (the production race sequence, red→green), R2 (the ceiling, red→green), R3 (reconnect mid-prompt, red→green), R4 (clean sequence + no-dupe), R5 (actor's own screen), on the real game + real `Server.getPlayerModel` snapshots + real route payloads + the real ingest.
- `tests/notifications/cityPlacementTopologies.spec.ts` — one real door per city EXECUTION topology (deferred `PlaceCityTile` continuation / synchronous `addCity` in `bespokePlay` / special city tile via `behavior.tile`→`PlaceTile`→`addTile` (never `addCity`!) / pay-on-commit standard project / negative + no-op), each verified by the INDEPENDENT ORACLE; plus the `CITY_TILES` completeness worklist (a new counts-as-city tile type fails until classified; topology links are asserted from code — `behavior.tile.type` — never by resemblance). Bot city → corpus S10.
- `tests/notifications/stateChangeOracle.ts` — delivery proven by COMPARISON of independent sources (game objects' before/after ledger vs presented recipient bands): silent mutation / phantom band / wrong delta / wrong recipient / duplicate all fail. Scope: stock + production + TR.
- City-source inventory (who places cities, through which choke point): the 2026-09-03 audit table — 4 choke points (`addCity` / `PlaceTile`→`addTile` / `simpleAddTile` (solo setup, no triggers by design) / `PlaceCityTile`), ~35 in-scope sources. Off-Mars `city:{space}` cards are `SpaceType.COLONY`: Tharsis's production branch correctly excludes them.

**The SECOND report (2026-09-04, Rover Construction × Tharsis first action) was the SAME defect through a SECOND door.** B's Tharsis owes its MANDATORY FIRST ACTION (place a city) and performs it on their first turn; A's Rover Construction is owed +2 M€. The door (`Player.takeAction`'s `pendingInitialActions` branch, category `'corporation-action'`) is correctly SCOPED server-side — but its chain is BORN OPEN across the SelectSpace, so the pre-fix consumer lost the band through exactly boundary rules 1–2 (proven: with the two fix hunks temporarily reverted, F2/F3 go red the same way R1/R3 did). Two census lessons got corrected: the door-census proof string had CLAIMED «corp actions in scope are self-only today» (false — the first action places a city), and no scenario had ever driven this door. Now: `consumerDeliverySequences` F1–F3 (clean / race / reconnect), `cityPlacementTopologies` T7 (independent oracle), the census entry names them.

**THE THIRD REPORT (2026-09-04, the same scenario on a FIXED build) — the G-family: the three inputs come from three moments.** The layer consumed `logs` and `events` from two independent HTTP reads and the open-set from a THIRD moment (the transport's last playerView apply). Four proven holes (all red on the pre-fix HEAD, `consumerDeliverySequences` G1–G4):
- **G1 stream skew** — the logs read lands after a server transaction, the events read before it: a journal header with NO chain events built a NEUTRAL model, the stale open-set didn't hold it → premature release, semantics frozen, and the payout that arrived one read later had no channel (`refreshQueuedImpacts` upgrades only the QUEUE; the standalone fallback diff is loss-only — a GAIN died, a loss survived). Fix: **the STREAM-SKEW GUARD** in the ingest — a header whose correlation has no events in this payload is a skewed pair, held in PREPARING like an open chain (`skewHeldIds` also protects it from the same pass's and the signature-equal pass's release; the 90 s ceiling still bounds a genuinely event-less log group).
- **G2 stale open-set** — fresh streams judged against an older view's open-set bypassed the atomic gate. Fix: **the meta journal-events route** (`?meta=1` → `{events, openEventCorrelations}` captured in ONE read); the layer passes the coherent pair, the playerView copy is only the degraded-mode fallback (older server).
- **G3 restart swallow** — seen-sets/PREPARING/queue are in-memory; an app restart (Deck suspend, TV app kill, crash) between «chain closed» and «band presented» made the payout «old news» for the next session's silent seed. Fix: **the persistent delivery ledger** (`notificationDeliveryLedger.ts`, localStorage keyed by the viewer's participant id, in-memory fallback): after every pass it stores the processed-event watermark + the still-undelivered correlation ids; the next seed releases chains newer than the watermark or listed undelivered (personal-sign/highlight only — the plain neutral feed is not replayed) and applies the same rule to the standalone hostile diff.
- **G4 settings asymmetry** — «show ordinary notifications» OFF silenced personal GAINS (kind `normal`) while losses (kind `negative`) stayed exempt. Fix: `settingAllows(kind, sign)` — a personal sign is signal, not noise.

**The UNSCOPED-DOOR TRIPWIRE (`Game.addTile`).** The next door someone forgets to scope must announce itself in the very first game that hits it, not weeks later as «игрок не получил нотификацию»: before the `onTilePlaced` fan-out, a placement with NO live event scope while a FOREIGN reactive hook exists logs a loud `[events] tile placed … with NO live event scope — an unscoped door` warn naming the reacting card (throws under `THROW_STATE_ERRORS`). Test setups that call `addCity`/`addTile` directly may print it — that noise names a real smell (the spec bypasses doors), never fails a run.
