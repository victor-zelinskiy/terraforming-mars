...
| Корпорация набирает ПО в конце игры | `endgameVictoryPoints` + терм `corpVp` (образец C25) | хук ЧИСТЫЙ: считается из состояния в момент подсчёта, никакого параллельного талли. Терм надо провести через ШЕСТЬ точек (`updateTotal`, `AutomaScoring`, `automaCardsTotal`, `finalScoringRevealModel`, `victoryPointsModel`, `consoleOverviewModel`+`ConsoleOvTabCards`) — иначе ломается «сумма сегментов ≡ total» |
