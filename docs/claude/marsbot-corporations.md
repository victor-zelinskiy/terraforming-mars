...
Обе карты печатают ДОСЛОВНО одно: белый куб → самый отстающий трек (при равенстве — верхний), чёрный → трек космоса. Поэтому реализация ОДНА (`corps/MarsBotWildCubePush.pushWildOrNamedTrack`), а карта владеет только тем, что у неё действительно своё: где стоят кубы (данные), её идентичность и её счётчики.
