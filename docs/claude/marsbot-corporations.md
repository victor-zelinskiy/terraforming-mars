...
**Status: production (2026-08-19; C04–C13 добавлены 2026-08-20).** Реализованные корпорации: **C01 Credicor · C02 Ecoline (+B23 Rapid Sprouting) · C03 Helion (track cubes) · C04 Interplanetary Cinematics (per-advance эффект + белые маркеры) · C05 Inventrix (+B25 Do It Right, destroy-at-setup) · C06 Mining Guild (банк M€ на карте) · C07 PhoboLog (смешанная бонус-колода) · C08 Saturn Systems (триггер по обеим сторонам стола) · C09 Teractor (выплата за трек Земли) · C10 Tharsis Republic (триггер на размещение города с обеих сторон) · C11 ThorGate (карта по первой метке) · C12 UNMI (+B31 Government Subsidy, размен бонусных карт) · C13 Cheung Shing Mars (серебряные кубы) · C45 Spire**. Официальные данные и трактовки: `docs/AUTOMA_DATA_AUDIT.md` §10 (RB-B транскрибирован полностью). Дизайн-референс upstream-типов: `docs/AUTOMA_CORP_FRAMEWORK_REFERENCE.md` (историческая записка — реализация НЕ по его фасаду).
