...
- **Блок A (`938be2f7bf`).** `InfluenceScaledEffect.upTo {total}` — величина есть уровень, `total` говорит, что им
  ...
  (RX05), рука — у движка в момент шага. Модель места несёт `hand` по `declaresHandLevel`. Задание: `QuestGoal
  {kind: 'cardsDiscarded'}`, точка репорта — `Player.discardCardFromHand` → `ParliamentHandler.onCardDiscarded`;
  `eligible` решает чьё (продажа патентов считается, розыгрыш — нет, чужой эффект / политическая фаза / резолюция —
  нет). Гард контракта: выдача от `total.before`, `target` = декларация. Спек карты 29.
