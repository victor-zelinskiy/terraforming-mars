...
## RX16 · Joint Research (Учёные) — величина-ПОРОГ: «добрать ДО», а не «выдать N» (2026-09-24)

Каждый добирает карты до 6 + влияние в руке. Ново два: величина-УРОВЕНЬ (объявление `upTo`, выдача = цель − уровень,
ноль — штатный исход) и первый вид задания, считающий РАССТАВАНИЕ (сбросить 2 карты). Первая карта Учёных — базовая
колода стала шестипартийной. Полный документ — `docs/TURMOIL_REDUX_JOINT_RESEARCH.md`, промт —
`docs/claude/prompts/resolution-rx16-joint-research.md`.

- **Блок A (`938be2f7bf`).** `InfluenceScaledEffect.upTo {total}` — величина есть уровень, `total` говорит, что им
  считается (рука = `{kind: 'cards'}`); `scaledAmount` даёт ЦЕЛЬ, ОДНА новая `topUpAmount` — ВЫДАЧУ для сервера, чтений и
  стенда. Запись `cards`/`skipped` несёт `target` (новый член) + `total {before, after}` (член последовательного семейства
  переиспользован) + `amount`/`drawn`; ноль — причина «Already at the target hand size». Добор — `ExternalDrawIntake`
  (RX05), рука — у движка в момент шага. Модель места несёт `hand` по `declaresHandLevel`. Задание (после промт-фикса
  ниже): `{kind: 'cardsPlayed', cardType: 'event'}`. Гард контракта: выдача от `total.before`, `target` = декларация.
- **Блок B (`17c842f54f`).** Чтение одной линией «до 9 [карта] · в руке 5 → +4 [карта]» (`__in--level`), ноль — «добор не
  нужен» в слоте результата без амбера (`levelYieldIsNone`); модель `levelYield` / `fixedLevelYield` в
  `voteYieldsOf` / `enactedYieldsOf`, прогноз победы по поднятой цели; осмотр «Для вас» словами + правило «рука
  считается на заседании»; бюджет панели считает слова порога; стенд — семейство `up-to`, панель «Карты в руке».
- **Блок C.** Ничего своего: такт добора — интейк RX05, лента читает чтение порога вместо голого «+N», ноль зрителя —
  свой момент ленты, чужой ноль — строка итогов.
- **Блок D.** Фикстуры `parliament-research-vote` / `-assembly` / `-enact` / `-full`; ОДИН e2e
  `console-parliament-research.spec.ts`; документы; спек Q-4 закрыт для этой карты.

### Находки

- **Шестая партия в базовой колоде**: `ParliamentPhase.spec` § THE STARTING-RULE RULER утверждал, что Зелёные всегда
  держат карту поколения 1, — это было свойство посева, не колоды; переписан на литеральное правило (представлены →
  не платятся как отсутствующие, иначе — платятся).
- **Генерический спек фазы отвечал `SelectCard` одной картой** — взятие с несколькими картами переиздаёт остаток и съедало
  все раунды охраны; взятие по `externalDrawPrompt` отвечать целиком.
- Строка интейка с именами карт (`LogHelper.logDrawnCards`) — `${0} drew ${1}` с `reservedFor`; публичная строка — только
  число.
- **Фит стадии взятия измерять ПОСЛЕ посадки сдачи**: ряд пересчитывает фит, пока карты летят с колоды, и одномоментный
  `expectParliamentFits` на такте взятия читал дофитовую высоту (`spills-y 838>835`, один раз из трёх) — утверждение
  «поза помещается, когда сдача села» опрашивается до устоявшегося ответа (`expect.poll` над пробником), не сэмплируется.
- Итоги для нулевого добора: «Пропущено · Эффект резолюции» звучало сбоем — `ResultsPayoutPart.none` печатает «добор не
  нужен» той же фразой, что лента и панель; причина сервера остаётся рядом.

### Прогоны (2026-09-24)

| Что | Результат |
| --- | --- |
| `JointResearch.spec` (29) · `ResolutionContract` (100) · `QuestTracker` · `ClimateResearch` · `rewardAddress` · `ParliamentPhase` (20, два спека переписаны) · `ParliamentModel` · `Parliament` · `Renewal` · RX15 · RX07 · `ChairmanQuestGate` · глоссарий · `parliamentResults` (23) · `e2eFixturesLoad` + `e2eDriverGuard` (79) | зелёные |
| клиентские юниты: `levelYieldModel` (11) · `sequelYieldModel` · `voteInfoBudget` (гард каталога с RX16) · `influenceYieldModel` · `parliamentAnnotations` · `voteInfoModel` · `chairmanQuestFlow` · `colonyLedgerModel` · `parliamentRewardBeat` | зелёные |
| `npm run lint` · `build:test` (оба дерева) · `make:cards` · `make:json` · `build:client` | зелёные |
| e2e `console-parliament-research` · standard-1080 · свой сервер из `build/` | ✓ 57 с; после починки замера фита — ✓ 52 с, `--repeat-each=3` зелёный |

Кадры: `screenshots/parliament-research/standard-1080/` — `01-face-quest` (лицо «[влияние] + 6 [карта]*», сноска
задания — два глифа сброса), `02-vote-explained` (панель: «до 8 [карта] · в руке 5 → +3 · +1 если победите · шаг 5»),
`03-fullscreen-words` («Цель сейчас: 8 карт в руке (6 + влияние 2). В руке 5 — добор 3»; плиты «до 8 · в руке 5 → +3 ·
если принять сейчас» и «до 9 · в руке 5 → +4 · если победите · шаг Повестки 5»), `10-enact-take` (лента «ЭТА ВЫПЛАТА · до 8 ·
в руке 5 → +3», три карты внутри стадии ПОЛУЧЕНИЕ), `11-band-none` (лента «ПОЛУЧЕНО · до 8 · в руке 9 → ДОБОР НЕ НУЖЕН»),
`12-results-none` («player1 +3 [карта]», «player2 · добор не нужен · Рука уже не меньше цели»).

### Промт-фикс: задание председателя — 2 карты события, не сброс (2026-09-24)

Жёлтый кружок со стрелкой вниз на скане — МЕТКА СОБЫТИЯ, а не «сброс»: задание Joint Research читается «разыграйте
2 карты события». Реализовано расширением вида `cardsPlayed` третьим типом `event` (ветка `CardType.EVENT` в
`QuestTracker.match`, репорт при розыгрыше уже нёс тип); сноска — `b.tag(Tag.EVENT, 2)` (метка с числом, как на карте;
ветки `active` / `automated` не тронуты). Вид `cardsDiscarded` удалён целиком: `QuestGoal`, `QuestEvent` и ветка
`match`, ветка `questRender`, `ParliamentHandler.onCardDiscarded`, вызов в `Player.discardCardFromHand`, ключ «Discard 2
cards». Одноимённое `EventImpact.cardsDiscarded` (нотификации) — другая сущность, не тронуто. Ловушка, закреплённая
спеком: `{kind: 'tag', tag: Tag.EVENT}` никогда не засчитается — метка события не лежит в `card.tags` (у `Asteroid` её
нет), она следует из типа в `Tags.count`. Фикстуры RX16 перегенерены; один e2e перепроверен на сноске-метке.

## Лор партий в осмотре партии (2026-09-24, промт `docs/claude/prompts/parliament-party-lore.md`)

Полный осмотр ПАРТИИ получил в левом жёлобе тот же литературный блок, что у проектных карт («ЗАПИСЬ ИЗ АРХИВА»), с
первым абзацем рулбука «Meet the parties» (стр. 3–4). Правила и эффект партии в блок не попадают — у них плита и панель
правил. Исключение в `CardZoomModal.loreVisible` снято для партии; у резолюции жёлоб по-прежнему занят колонкой партии.

**Архитектура — блок принимает МОДЕЛЬ, резолверов два.**
- `CardLoreAside` больше не знает имени карты: проп `model: LoreModel` (`source · text · fallback · tier`; тип переименован
  из `CardLoreModel`, потому что это интерфейс блока, а не карты). Вёрстка, марки, тиры, хореография — без изменений; на
  смену модели с ТЕМИ ЖЕ словами (пересобранный объект) блок не мигает (`sameLore`).
- **Второй резолвер — `src/client/cards/partyLore.ts`** (`partyLoreSource` / `buildPartyLoreModel(party, translate)`),
  по правилам чистоты карточного: без Vue/DOM/i18n, переводчик инъектируется, лестница длины — `loreLengthTier` из
  `cardLore.ts` (одна на оба корпуса). Партия НЕ проходит через карточный резолвер и не получает `cardNumber`.
- Вызывающая сторона решает, чей лор: `CardZoomModal.loreModel` по субъекту — карта → `buildCardLoreModel`, корпорация
  бота → оригинальная карта, партия → `buildPartyLoreModel`, бонус Automa / резолюция → `undefined` (жёлоб пуст / занят).
  `loreVisible ⇔ loreModel !== undefined` — резерв `--con-lore-w` в фите включается для партии сам.
- Переводчик лора вынесен в `src/client/cards/loreTranslate.ts` (`translateNonWordText` — прозе нужен обход «неслов»),
  чтобы резолверы остались чистыми, а хосты (просмотрщик, стенд `CardLorePlayground`) делили одну привязку.

**Корпус и перевод — ОДИН конвейер.** Английский — `assets/text/party_lore_texts.json` (ключ — имя партии, `PartyName`),
русский — в `src/locales/ru/lore_texts.json` рядом с карточным (собирает тот же `make:json`). В оригинале исправлены
опечатки `enterpreneurs → entrepreneurs`, `conservatists → conservatives` и дефис → тире у Зелёных (английская строка —
ключ И текст). Имена партий в переводе — канонические ключи `party name: …` (`parliament.json`): **«Союз»** (не
«Единство», как стояло в промте — глоссарий § «Имена партий»), «Марс вперёд», «Учёные», «Зелёные», «Красные».
Заголовок общий (`LORE_HEADING_KEY`), новых ключей UI нет — только шесть строк корпуса.

**Замеры.** Длины: EN 254–333 символа, RU 221–303 — все шесть в тире `extended`, внутри конверта карточного корпуса
(RU‑максимум карт 303, Saturn Systems). Пробник над живым просмотрщиком (`parliament-dense`, все шесть партий LB/RB):

| Профиль | Колонка | Самый длинный (Индустриалисты) | Самый короткий (Учёные) | Плита |
| --- | --- | --- | --- | --- |
| tv-4k 3840×2160 | 1200 px | 10 строк, тело 1022 / рамка 1060 px | 7 строк, 784 px | по центру (dx 0), зазор 120 px |
| tv-fhd 1920×1080 | 600 px | 10 строк, 511 / 530 px | 7 строк, 392 px | dx 0, зазор 60 px |
| standard 1280×720 | 360 px | 14 строк, 524 / 364 px — течёт ниже рамки, как длинные корпорации | 10 строк, 407 px | dx 0, зазор 60 px |

На TV оба крайних случая внутри константной рамки; на 720p (нештатный, ниже handheld‑800) две самые длинные партии
(Индустриалисты, Красные) выходят за рамку вниз ровно так же, как самые длинные карточные записи той же длины: скролла,
обрезки и наезда на плиту нет, командная пилюля стоит правее колонки. Тир не добавлялся — партии не превышают корпус.
Кадры: `screenshots/party-lore/<профиль>/01-party-industrialists · 02-party-scientists · 03-resolution-aside`.

**Проверка.** `partyLore.spec.ts` (резолвер: шесть партий, ровно шесть ключей, пунктуация, локализация через конвейер
лора, канонические имена, тир по локализованной строке, фолбэк + один warn, гард покрытия — рабочий список), расширены
`CardLoreAside.spec.ts` (партия = тот же класс-набор, что карта того же тира; пересобранная модель не прячет блок) и
...
## RX19 · Metal Research (Индустриалисты) — пассив, меняющий ЦЕННОСТЬ ресурса: надбавка на ЧТЕНИИ аксессора, поле не трогать (2026-09-25)

Сталь = влияние и титан = влияние (две выплаты семейства дословно — RX10 и RX17), и **один новый механизм: пока закон
принят, единица стали участника стоит 3 M€ вместо 2, титана — 4 вместо 3**. Ценность ресурса в движке — изменяемое
сериализуемое ПОЛЕ игрока, поэтому закон его не пишет: хук `ResolutionPassive.resourceValueBonus` спрашивается из самих
аксессоров `getSteelValue` / `getTitaniumValue` через чистый запрос `ParliamentHandler.resourceValueBonus`. Полный
документ — `docs/TURMOIL_REDUX_METAL_RESEARCH.md`, промт — `docs/claude/prompts/resolution-rx19-metal-research.md`.

- **Блок A (`15e8063f04`).** Хук + запрос (гейт `participates`, без области события) + надбавка в двух аксессорах; сырой
  читатель `spendableMegacredits` (ветка LTF) переведён на аксессор; карта (два шага `steel` → `titanium`, два названных
  пропуска, лицо «[сталь][титан] / [влияние]» + «[сталь][титан] : +1 M€», задание +1 пр. титана, `forecast: []`), каталог,
  локаль (4 ключа), арт RX19. Спек (26 → 27 в B): сериализация держит 2 / 3 в поле при 3 / 4 на аксессоре; аддитивность с
  «Улучшенными сплавами» 4 / 5; `payingAmount` / `canAfford` / `spendableMegacredits` согласны; LTF 4 − 1 = 3; бот без
  надбавки; модель клиента 3 / 4. `JointResearch.spec`: устаревшее ожидание «одна карта Учёных» (с RX18) → «среди».
- **Блок B (`e79c085e0f`).** Прочёс `src/` на сырые чтения полей — единственный был `spendableMegacredits`; `serialize` /
  `deserialize` читают ПОЛЕ намеренно; `Merger`, `IndustrialComplex`, `effectForecast` (floodgate-сталь = 3 под законом —
  юнит), `ServerModel` — аксессор; клиентские читатели (`paymentPlan.rateFor`, `railValueModel`, `PaymentWidgetMixin` —
  последний без живого потребителя) читают модель. Клиентские юниты «под законом»: бейдж 3 / 4 и «4/3» с LTF, рейт 3 / 4.
- **Блок C (`c78e34e719`).** `MetalResearchReadings.spec.ts` (6): манифест с двумя частями запаса и пассивом; голосование
  читает ДВЕ строки (сталь, титан — оценка + прогноз «если победите»); блок чтения рисует две группы; правая колонка осмотра
  «При принятии / Эффект резолюции / Задание»; тихая поза НАГРАДЫ читает пассив; лента эффектов ведёт строкой закона. Стенд
  рисует те же две строки через общий `voteYieldsOf` — семейство `influence`, нового не заводится.
- **Блок D.** Фикстуры `parliament-metal-assembly` (синий: Повестка 2, 3 стали / 2 титана) и `parliament-metal-enacted`
  (красный открывает 2-е поколение с «Ядерной энергией» и 4 сталью; сервер 3 / 4, цена 10); ОДИН e2e
  `console-parliament-metal.spec.ts` (standard-1080): бейджи «2» / «3» → осмотр с блоком пассива → ворота → две записи
  `steel:2, titanium:2` → **бейджи «3» / «4» на том же экране, без перезагрузки и без нажатий** → итоги; принятая: сервер
  3 / 4, `calculatedCost` 10 → бейджи → строка закона в списке эффектов → шапка оплаты «10» без пары base/final и без
  «сэкономлено» → строка стали «×3» (3 стали → вклад 9). Документ карты, чеклист (раздел RX19 + строка в §1 п. 7), правило 4h.

### Находки

- **`[data-pay-base]` существует только при СКИДКЕ** — без неё шапка печатает одну цену; проверка «ценность ≠ скидка»
  в e2e читается как отсутствие пары base/final и `[data-pay-saved]`, не как «base = 10».
- **Хередок Bash съедает `\` в шаблонных строках node** — заголовок теста с `\'` превратился в разорванную строку; заголовки
  без апострофов, вставка по маркеру без обратных слэшей.
- **Манифест резолюций живёт в `build/main.js`**, не в ленивом чанке `console-shell.js`: после `build:client` чанк
  консоли законно не переписывается (UI не менялся), а RX19 ищется в `main.js`.
- **`JointResearch.spec` был красным с RX18** (ожидал одну карту Учёных в колоде) — унаследованный провал, поправлен точечно.

### Прогоны (2026-09-25)

| Что | Результат |
| --- | --- |
| `MetalResearch.spec` (27) · `ResolutionContract` · `ResolutionPassive` · `HeatCapture` · вся `tests/parliament/*` (771 с `Player.spec`, `ServerModel`, `effectForecast`, `actionPreviewPayment`, `SelectPayment`) · `AdvancedAlloys` · `Merger` · `IndustrialComplex` · `LunaTradeFederation` · `Helion` · глоссарий · `e2eFixturesLoad` · `e2eDriverGuard` | зелёные |
| клиентские юниты: `MetalResearchReadings` (6, новый) · `railValueModel` (+1) · `paymentPlan` (+1) | зелёные |
| `npm run lint` (eslint + i18n + vue-tsc) · `build:test` (оба дерева) · `make:cards` · `make:json` · `build:server` · `make:css` · `build:client` | зелёные |
| e2e `console-parliament-metal` · standard-1080 · свой сервер из `build/` | 1 passed (1,1 мин) |

Кадры: `screenshots/parliament-metal/standard-1080` — `01-verdict-badges-2-3` (рельса «2» / «3» до принятия),
`03-badges-3-4-under-the-law` (**главный кадр**: закон принят, бейджи «3» / «4» на том же экране заседания),
`06-effects-strip` (строка закона «Принятая резолюция · Исследование металлов» с графикой), `07-play-steel-rate-3`
(композер: цена 10, строка «Сталь ×3», 3 стали → вклад 9, «оплачено 10 / 10»).

## RX20 · Skyscrapers (Марс вперёд) — СТОПКА городов: высота на клетке, ярус = третий случай посадки, тайл ПО ПОРОГУ (2026-09-25)

Первая карта, меняющая МОДЕЛЬ ДОСКИ: победитель и все с влиянием ≥ 2 кладут тайл города **поверх своего города на Марсе**,
каждый ярус приносит ПО за соседние озеленения отдельно, без города — ничего. Четыре новых вещи, все — члены семейств:
стопка как ОДНО поле клетки и ОДНА функция величины; ярус как размещение без клеточных бонусов; ярус как ТРЕТИЙ случай
арма посадки (сцена крана); тайл ПО ПОРОГУ как объявление `tileGrant` с семейством `tile-grant`, счётом назначений и ОДНОЙ
моделью чтения. Полный документ — `docs/TURMOIL_REDUX_SKYSCRAPERS.md`, промт — `docs/claude/prompts/resolution-rx20-skyscrapers.md`.

- **Блок A (`5e91a72639`).** `Space.stackHeight` (отсутствие = 1, сериализуется от 2, старый сейв читается); `MarsBoard.countCities`
  (сумма ярусов) — ОДНА величина, на неё переведены Mayor, Metropolist, Constructor, Landlord, Urbanist, Counter, CitiesRequirement,
  Vermin, Hospitals, StarVegas, NewHolland, StrongSociety, Election, модель игрока; предикаты читают клетку; арифметика ярусов в
  `boards/cityStack.ts` (без зависимостей — цикл `Counter → MarsBoard → Board`). `Game.addTile(…, {stacking})`: клетка не платит
  повторно (печатный бонус, океан, Ares), триггеры «город размещён» и партийный пассив срабатывают, RX10 не повторяет; ПО по
  ярусам + `CityVpDetail` на ярус; тип размещения `city-tier` (кандидаты = свои города на Марсе, `not-your-city`, превью
  «стопка 1 → 2», осмотр «Стопка городов» с рядом на ярус, гипотеза наращивает стопку); объявление `tileGrant`, вид исхода
  `city` + адрес, семейство `tile-grant`, карта (один шаг, два названных пропуска), каталог, локаль, арт. Спеки: карта (16),
  стопка (26).
- **Блок B (`3b96021e50`).** Вид стопки: нижние ярусы под верхним тайлом (шаг 3,5 px, масштаб 0,86 — гекс не растёт), шов с
  подсвеченной кромкой (fx-lite — плоский), счётчик «×N» слева внизу, кубик на верхнем ярусе, `data-stack-height`; осмотр
  клетки «Стопка городов ×N» + ряд ПО на ярус; разбор счёта — строка на ярус «Город · ярус k из N»; досье знает вид `city-tier`.
  Юниты BoardSpace (4), scoreExplorer; фикстуры `parliament-skyscrapers-enact` / `-stacked`.
- **Блок C (`1ff12df8e6`).** Арм с `stacking` — `verifyPlacement` принимает рост стопки ровно на 1 на той же клетке;
  хореография крана: заход с гашением горизонтали → ЗАГРУЗКА основания (`cityStackScene.loading`: контур клетки, счётчик «×1»,
  кубик удержан) → ВЕРТИКАЛЬНЫЙ спуск (x зафиксирован, y монотонен) → КОНТАКТ с пылью и микротолчком, счётчик «×2» в тот же
  синхронный ход, что красится ярус → осадка; бонусы клетки не захватываются и не летят; reduced motion = покраска с тиком;
  abort/откат чистят сцену; `STACK_STEP_PX` / `STACK_SCALE` зеркалят `board.less`. Юниты модели и транзакции; ОДИН e2e
  `console-parliament-skyscrapers` (in-page сэмплы, кадры CDP-скринкаста по моментам пробника). Док
  `tile-replacement.md` § третий случай.
- **Блок D.** Семейство `tile-grant` на клиенте: `tileGrantStepKey` / `tileGrantCountId` / причины пропусков — данные рядом с
  объявлением (карта берёт их оттуда); седьмой вид счёта `marsCities` (`BoardCountedTile 'marsCity'`: клетка ОДИН раз какой бы
  высоты ни была стопка, сервер отдаёт список кандидатов шага, `declaredCountIds` видит грант); `tileGrantModel.ts` (двойник
  `winnerRewardModel`: reference / conditional / pending / applied) + `ConsoleTileGrant.vue` (`.con-tgrant`, варианты
  block / chip / inline); читают: панель голосования (`voteReadingOf().grant`, бюджет считает грант чтением; в inline-варианте
  слова «куда» не печатаются — стрелка на город-основание и графика карты уже сказали), полноэкранный осмотр (чип бара в шелле
  и в меню, «Для вас», правило стопки под эффектом), лента (чип `grant` только для яруса, который должен или лёг), панель
  итогов (часть `city` + «×N» — до этого запись печаталась «0»), стенд (ячейки через общий предикат, `grantOutcomes`,
  контроль «Города на Марсе», семейство `resolving` с `pending: space`). Квитанция: `detectNewViewerTile` видит `city` —
  заседание по возврату читает страницу награды. Глоссарий: две формы тайла победителя с причиной, канон в гарде. Локаль
  (17 ключей). Юниты: `SkyscrapersReadings` (10, новый), `parliamentBand` (+1), `parliamentResults` (+1), `parliamentRewardBeat`
  (+1), `resolutionCounts` (+1), `Skyscrapers.spec` (+3: ключи из объявления, счёт = кандидаты, паритет предиката по корпусу
  досок). e2e дополнен: ожидание конца сцены — СОСТОЯНИЕ сэмплера (12 сэмплов покоя), не сон; после итогов — покупка карт
  нового поколения отвечается по API, L3 → осмотр клетки: «Стопка городов» + ряд на каждый ярус (кадр 10).

### Находки

- **`python3` на этой машине — заглушка Microsoft Store** (печатает «Python», код 49): правки через `python` (3.14). Хередок
  Bash по-прежнему съедает `\n` и `\\` — скрипты правок писать файлом (`Write`) и запускать `python <file>`.
- **Подмена файлов на старый коммит для замера — ТОЛЬКО с бэкапом-копией и восстановлением из него**: `git checkout <sha> -- f`
  пишет в индекс, а список «изменённых» после подмены пуст для файлов, где старая версия совпала с HEAD; восстановление по
  `git diff --name-only` теряет их. Список файлов фиксировать ДО подмены, возвращать копированием, индекс — `git reset HEAD`.
- **Гард `voteInfoBudget` красный по RX18 ещё до RX20** (замерено на `9264f4a190`): «Ни одна карта не хранит данные или бактерии»
  из аудита текстов владельца (`fb4d53fcfb`) — 8 слов, панель Medical Database при влиянии 0 на границе = 29–30 слов при
  лимите 28. Чужой перевод не переписан; решение владельца: короче нота, иначе без суффикса «если победите · шаг» при 0.
- **Запись `city` в итогах печаталась «0»**: часть без `amount` и без `tile` попадала в общую ветку `signedAmount`. Часть
  тайла обязана объявлять `tile` (и `stack`) — иначе панель честно печатает ноль.
- **Бюджет слов панели голосования** делит слова на inline-строку и блок: слова «куда» (5) в inline-варианте не помещаются —
  рисунок стрелки на город-основание их заменяет; подпись «Ваш при влиянии N — и без победы» (6 слов).
- **Эталон квитанции — тайл победителя**: `city` — такой же тайл доски (`detectNewViewerTile`), заседание читает страницу
  награды до итогов; без этого ход шёл с доски прямо в итоги.

### Прогоны (2026-09-25)

| Что | Результат |
| --- | --- |
| `Skyscrapers.spec` (19) · `CityStack.spec` (26) · `ResolutionContract` · `ColonizationFunding` · `resolutionCounts` · `parliamentBand` · `parliamentResults` · глоссарий · `e2eDriverGuard` · `serverDerivedCacheGuard` · `glyphLiteralGuard` | зелёные |
| клиентские юниты: `SkyscrapersReadings` (10, новый) · `parliamentRewardBeat` · `parliamentAnnotations` · `voteInfoModel` · `winnerRewardModel` · `influenceYieldModel` · `MetalResearchReadings` · `chairmanQuestFlow` · `consoleResolutionPayout` · `parliamentSittingSeed` · `tilePlacementModel` · `consoleTilePlacement` | зелёные |
| `voteInfoBudget` | **красный по RX18** (унаследован от `fb4d53fcfb`, см. находки); Skyscrapers во всех 16 комбинациях в бюджете |
| `npm run lint` (eslint + i18n + vue-tsc) · `build:test` (оба дерева) · `make:json` · `make:css` · `build:server` · `build:client` | зелёные |
| e2e `console-parliament-skyscrapers` · standard-1080 · свой сервер из `build/` | 1 passed |

Кадры: `screenshots/parliament-skyscrapers/standard-1080` — `02-tier-dossier` (досье: «Стопка городов: 1 → 2 яруса» +1 ПО,
«Без бонуса размещения»), `04-tier-hanging` (ярус висит над стопкой, счётчик «×1»), `05-contact` (**главный кадр**: контакт,
«×2»), `06-stack-settled` (стопка ×2 на доске, обычный зум), `08-results` (итоги: часть города «×2», пропуск красного назван),
`09-score-two-tiers` (разбор счёта: «Город · ярус 1 из 2», «… 2 из 2», по 1 ПО), `10-cell-inspection` (осмотр клетки: «Стопка
городов», ряд на ярус); TV — `screenshots/skyscrapers-stack/tv4k-cell.png` (блок B).

## RX21 · Migration Funding (Марс вперёд) — счёт по полю как ВЕЛИЧИНА: два счёта над одной плиткой, объявление несёт меру (2026-09-25)

Близнец RX08 над другой половиной доски: «каждый игрок получает 2 M€ за каждый свой город на Марсе + влияние; каждый город
в стопке считается отдельно». Выплата в ЗАПАС штатной волной (путь RX13), счётный член по ПОЛЮ (путь RX08). Ново ровно
одно: счёт по полю здесь — ВЕЛИЧИНА (сумма ярусов), а счёт RX20 над теми же клетками — НАЗНАЧЕНИЯ (клетка раз); без стопок
они равны на любой доске. Полный документ — `docs/TURMOIL_REDUX_MIGRATION_FUNDING.md`, промт —
`docs/claude/prompts/resolution-rx21-migration-funding.md`.

- **Блок A (сервер + общий слой).** `BoardCountMeasure = 'cells' | 'tiers'` — член вида `board` (`spaceCities`, `marsCities` → `cells`;
  новый `marsCityTiers` → `tiers`): из объявления ВИДНО, суммирует ли счёт ярусы. `ResolutionCountModel.tiers` — столбец высот,
  выровненный со `spaces` (двойник `units` у карт), в записи/модели/чтении `countedTiers`; клетка в списке один раз.
  Арифметика стопки переехала в `common/boards/cityStack.ts` (серверный файл — типизированная дверь), ею считают и
  `countCities`, и `countSpacesToward` стенда. Сервер: число меры `tiers` — `countCities(player, 'onmars')`, клетки —
  `getCitiesOnMars`, высоты — `Board.tiersOf`; назначения RX20 по-прежнему `getAvailableSpacesForCityTier().length`. Карта:
  один шаг `megacredits` через `stock.add`, свой пропуск «No cities on Mars and no influence», лицо `2 [M€] / [город] + [влияние]`
  без искры и без потолка, задание — ключ RX20. Локаль (9 ключей + 5 подписей стенда), арт RX21.
- **Блок B (чтения).** `yieldCountPresentation('marsCityTiers')`; `countedCellEntries` / `countedCellLabel` («Ноктис ×2» — тот же
  «×n», что у карт и колоний, и счётчик доски); «Для вас»: все клетки именованы → список с весом, есть безымянные → «4 города
  на Марсе · 3 клетки · стопка из 2» (число и то, что делает его больше клеток), без стопки — число одно; глиф города на Марсе
  БЕЗ искры (`countedTileSpark` — искра только у `spaceCity`; глиф `marsCities` RX20 потерял лишнюю искру тем же ходом).
- **Блок C (показ).** Ничего нового: волна чипов M€ в рельсу, тик на посадке — путь RX13.
- **Блок D (стенд, e2e, доки).** Сценарий семейства `counted-board` объявляет `counts` (id счёта) и показывается только закону с
  этим счётом (клетки RX08 с производством — не про RX21); синтетическая клетка знает `stackHeight`, ряд рисует «×N» на тайле
  (`.con-rxpg__cell-stack`), число ряда — `countSpacesToward`, имена клеток стенда — по Tharsis (`getSpecialCellInfo(id, THARSIS)`);
  10 сценариев RX21 + живой `parliament-migration-vote`. Чеклист (§1 строка о двух родах + раздел RX21), `game-logic.md` 4i,
  спек §4.1 «СДАНА RX21», RX08-док §6 — сноска о поправке к рецепту.

### Находки

- **Рецепт RX08 («следующий board-счёт — одно слово `marsCity` и одна ветка») оказался ловушкой**: слово завёл RX20 для
  счёта НАЗНАЧЕНИЙ, и повторное использование `marsCities` для величины было бы молчаливой ошибкой до первой стопки. Решение:
  мера в объявлении, свой id, спек «THE DIVERGENCE» (3 против 4 на одном столе, те же клетки, у назначений нет высот).
- **Имена клеток Марса зависят от доски**: `getSpecialCellInfo(id)` без имени доски знает только зарезервированные области
  (Ноктис и горы — записи Tharsis). Осмотр не несёт имя доски (`resolutionAnnotations` — 5 вызовов, у меню-хоста доски нет в
  контексте зума), поэтому на Марсе «Для вас» печатает число + клетки + стопки; путь для именованных клеток готов
  (`countedCellLabel`), стенд именует по Tharsis. Плумбинг имени доски через `ConsoleZoomParliament` — отдельный шаг, если
  владелец захочет «Ноктис ×2» в живом осмотре.
- **`build:test` красный по чужому спеку** `tests/parliament/MiningIncentives.spec.ts` (параллельная сессия RX22, незавершён);
  `voteInfoBudget` красный по RX18 (унаследовано, решение владельца). Мои деревья чисты. `console-parliament-skyscrapers.spec.ts`
  нёс неиспользуемую константу `GREENERY` (TS6133 в e2e-дереве) — снята.
- **Чужая сессия в том же клоне** (RX22: `MiningIncentives.ts`, `RX22.webp`, импорт в каталоге, staged-индекс с моими файлами):
  коммиты — только явными путями, каталог — частичным стейджингом без чужого импорта.

### Прогоны (2026-09-25)

| Что | Результат |
| --- | --- |
| `MigrationFunding.spec` (18) · `resolutionCounts` (30, +1) · `ResolutionContract` · `ColonizationFunding` · `Skyscrapers` · `CityStack` (26) | зелёные |
| клиентские юниты: `influenceYieldModel` (+4) · `parliamentAnnotations` (+1) · `PremiumCountGlyph` (+1) · `SkyscrapersReadings` · `ConsoleInfluenceYield` | зелёные |
| `voteInfoBudget` | **красный по RX18** (унаследован, решение владельца); RX21 во всех 16 комбинациях в бюджете |
| `npm run lint` (eslint + i18n + vue-tsc) · `make:json` · `make:cards` · `build:server` · `build:client` · `make:css` | зелёные |
| `build:test` | mocha-дерево и e2e-дерево — мои файлы чисты; красный ТОЛЬКО по чужому `tests/parliament/MiningIncentives.spec.ts` (RX22, параллельная сессия) |
| e2e `console-parliament-migration` · standard-1080 · свой сервер из `build/` | 1 passed (51 s) |

Кадры: `screenshots/parliament-migration/standard-1080` — `02-vote-reading` (**панель голосования**: формула «2 [M€] / [город] +
[влияние]», чтение «[город] 4 + [влияние] 3 → +14», без потолка и надбавки), `03-fullscreen` (**разбор числа со стопкой**:
правило «стопка из двух даёт 2», «Учтены сейчас: 4 города на Марсе · 3 клетки · стопка из 2»), `05-reward-wave` (чип «+14» в
полёте), `06-after-wave` (рельса 62 → 76), `07-results`, `08-stand-stack` (**стенд «Полигона»**, сценарий «Стопка из 2: четыре
города на трёх клетках» — тайл Ноктиса с «×2», у синего число 4, у красного 1; сценарии RX08 под RX21 не показаны).

**RX22 Mining Incentives (Индустриалисты) — СДАНА 2026-09-25.** Самая простая форма семейства: две обычные
производственные части, ни одной новой механики, ни одной правки в общем слое (тронуты только файл карты,
своя строка каталога, два ключа локали, арт и два своих спека). Плоский титан каждому = форма RX15
(`base: 1`, ставка 0 за влияние), сталь по влиянию = форма RX05. Три закона, которые карта НАСЛЕДУЕТ, а не
переписывает: ① влияние 0 пропускает ТОЛЬКО стальную часть (названный пропуск), титан выдаётся всё равно;
② реакцию партии пишет движок — и на этой карте ни одна не срабатывает вовсе (единственная
производственная реакция — Зелёные на растения/тепло; спек перебирает все шесть партий через
`productionReactionOf` и требует отсутствия записи `reaction`); ③ горизонт — оба шага впервые платят в
следующем поколении (серверный паритет на два поколения), но ПОМЕТКА в чтении молчит по своему же
закону (`productionHorizonOn` печатает её только рядом с чем-то оплаченным СЕГОДНЯ — платой или
запасной частью; здесь обе части в ОДНОМ горизонте, как у RX02 / RX04 / RX08). Второй формулировки не
заведено: клиентский спек пинует ОБА конца предиката. **Если владелец захочет, чтобы
производственная карта называла горизонт ВСЕГДА** — это правка ОДНОГО общего предиката и сразу
четырёх карт, не этой. Нового e2e нет (правило бюджета); юниты 18 + 7, гард контракта зелёный без
правок; кадр снят на стенде (`?resolutionsPlayground`, standard-1080). Документ карты —
`docs/TURMOIL_REDUX_MINING_INCENTIVES.md`. Замечено рядом (не правилось, общий файл в чужой работе):
подпись сценария семейства `influence` на стенде — «Влияние 3 — несколько животных»: единица RX01
протекла в подпись ДЛЯ ВСЕГО семейства (RX05 — тепло, RX19 — металлы, RX22 — производство).

**RX23 Mohole Contest (Зелёные) — СДАНА 2026-09-25.** Первая карта, чья часть победителя — ПРЯМОЙ ШАГ
глобального параметра без тайла (третий вид `winnerReward`: `{kind: 'parameter', parameter: 'temperature',
steps: 2}`), и первая, где такой шаг НАГРАЖДАЕТСЯ: общий исполнитель `winnerParameterStep(id, reward)`
(`resolutions/WinnerParameterStep.ts`, ключ шага = id параметра) не знает слова `unrewarded`, и гейт движка
в фазе `PARLIAMENT` платит победителю +1 РТ за сделанный шаг, бонусы −24/−20 °C (Зелёные отвечают и на
производство, и на РТ — записи `reaction` пишет драйвер), хуки карт и океан на 0 °C. Запись —
`globalParameter` + `part: 'winner'` с фактическими шагами, до/после и **измеренным `tr`**; гард контракта
требует шаг под выведенным ключом, запрещает `unrewarded` у записи победителя и `tr` меньше шагов. Комната —
общая `parameterRoom`, у которой появились `heatProductionBonus` / `oceanBonus` для подъёма температуры;
`winnerParameterRoom` принимает тайл | шаг и несёт `steps` / `applied`.

**Океан на 0 °C внутри заседания — три правки в общем слое.** ① Драйвер дренирует хвост МУТИРУЮЩЕГО
шага сразу после него (`stepEffects` → `drainDeferred` после каждого шага места/мира/победителя): до этого
отложенная работа шла после обхода всех мест, и «фаза ждёт океан» работало только у спрашивающих шагов
(RX01/RX03 — их хвост дренировал `continue()` ответа); первый прогон спека это и показал (p2 получил тепло
до океана p1). ② Модель публикует хвост как ask того же шага: при пустом `effects.pending` и промпте у места
из окна реакций `scan` (не гейт) — `pending = {player, key: <шаг>, input}`; другие места читают честное
ожидание, само место — шаг заседания. ③ Клиент: `sittingTailPlacementOf` (`space` без источника-резолюции +
`pending` сервера на зрителя с `input: 'space'`) → `rewardStep = 'placement'`, `position.tail = true` —
страница держится (`owed-step`), крошка «РАЗМЕЩЕНИЕ», хвост стоит за той же дверью, что тайл, но дверь
открывает ОБХОД, когда страница тиха, — не нажатие (первый прогон e2e: допущенный сразу хвост забирал экран
до волны тепла, секция размонтировалась и волна сбрасывалась неигранной). Планетарный такт обобщён с RX12: `isPlanetRecord` — `globalParameter` мировая без места или с
местом и `part: 'winner'`; `runWorldMoveBeat(…, {boardBusy})` не возвращает стек, пока у зрителя стоит
`space`-промпт, — один рейс к полю на глайд маркера с РТ-чипом и океан; `mayYieldForWorld` пускает хвост,
резолюционный тайл по-прежнему ждёт нажатия. Чтения: `winnerRewardModel` для шага (диапазон в °C, РТ,
«производство тепла +1», «на 0 °C следует океан», «температура на максимуме — без шага», глагол
«повышает»), блок победителя с иконкой параметра, итоги — часть строки победителя (`parameter` + `tr`),
строка ПЛАНЕТА только для мировых записей; стенд — пять сценариев температуры с `parameter`, сценарии
кислорода RX03 помечены. Локаль: 10 серверных + 19 клиентских ключей. Документ —
`docs/TURMOIL_REDUX_MOHOLE_CONTEST.md`; чеклист — раздел RX23 + строка в §1; `game-logic.md` 4j; спек §4.1
«СДАНА RX23».

### Находки RX23

- **`rulingParty()` в свежей Redux-партии — Зелёные** (стартовое правило), не Марс вперёд: спек, ждавший
  «Марс вперёд до принятия», был неправ; реакция Зелёных на РТ шага пишется и до, и после принятия одинаково.
- **Фаза производства платит доход ДО заседания**: M€ победителя после заседания = доход (РТ + пр. M€) + реакции;
  ассерт «2 M€ × 2 РТ» считать от дохода, не от нуля.
- **У бота свой ход по температуре** в `testAutomaGame` до заседания — запись читать относительно
  (`after − before === 4`), не абсолютно.
- Имя триггера производственной реакции — `production-gain` (не `production-raise`).
- **Скрытый дефект RX12, вскрытый свидетелем волны**: `enterServerStep` сажал обход на страницу НАГРАДА при
  ВХОДЕ в шаг, если «мировой ход должен» (`worldMoveOwed`), — ПРИНЯТИЕ пропускалось, принятая карта не
...
## RX25 · Plant Ban (Красные) — уровневой член ПОЛУЧИЛ НАПРАВЛЕНИЕ (2026-09-25)

**СДАНА.** Зеркало RX16: там игрока доводят ДО уровня добором, здесь срезают ДО уровня потерей.
Главное в карте — не карта: второго механизма не появилось. Док: `docs/TURMOIL_REDUX_PLANT_BAN.md`.

- **Блок A (`fc71e31bd2`).** `InfluenceScaledEffect.upTo` → **`level`**, `InfluenceLevelTerm` несёт
  `direction: 'up' | 'down'`; `topUpAmount` → **ОДНА `levelAmount`** (max(0, цель − уровень) вверх,
  max(0, уровень − цель) вниз), величина БЕЗ ЗНАКА; `levelAfter` применяет направление ровно один
  раз, `levelTakesAway` — предикат для чтений. Семейство стенда `up-to` → `level`.
  `declaredLevyResources` → **`declaredStockReads`** (один список запасов, которые вообще читают резолюции).
  Карта (`reds/PlantBan.ts`): норма 2 + влияние, потеря `stock.add(PLANTS, −N, {from: {resolution}})`,
  запись `stock` с отрицательной величиной и `owed`. Гард контракта дополнен проверкой `total.after`
  через `levelAfter` и знака у среза.
- **Блок B (`08a29107a2`).** `levelPresentation` теперь отвечает словом перед целью («до» / печатное «макс.»),
  фразой уровня («в руке N» / «у вас N»), спокойным нулём («добор не нужен» / «терять нечего») и нотой
  панели. Предупреждение голосования (`levelLossNoteOf`) — только тому, кому есть что терять. Строка
  итогов берёт спокойную фразу у ДЕКЛАРАЦИИ. Стенд: `levelDirection` у сценариев + края среза.
  Фикстуры `parliament-ban-vote` / `-assembly`.
- **Блок C (`06b10475c7`).** Подача не потребовала ни строки своей хореографии — отток RX15 уже был общим
  по ресурсу. Починены два общих закона (ниже) + e2e `console-parliament-ban`.

### Находки RX25

- **Лента называла потерю наградой**: «ВАША НАГРАДА · ЭТА ВЫПЛАТА» над чипом «−3 растения» (видно на
  первом же кадре прогона). Кикер и слово состояния выводятся из декларации (`bandRewardTakes`):
  «Что вы теряете» + «Эта потеря» → «Отнято».
- **Итоги печатали нетто «−3 = −3»**: `netOfParts` требует теперь, чтобы закон в той же валюте ещё и платил
  (документированное намерение функции было именно таким, но код этого не проверял).
...
- **Лицо по умолчанию повторяет иконку**: `b.plants(2)` нарисовал ДВА листа — «отнимут два растения»,
  ровно то, чего лицо говорить не должно. `{digit: true}`; та же правка у сноски задания `delegates`.
- **Гард бюджета панели поймал 0 чтений** у RX25: его синтетическое место везло только руку — теперь и
  запас выше любой нормы (худший случай, а не пустая модель).
- **Защиту ресурсов проверяем СПЕКОМ, а не вызовом**: `isProtectedFrom` требует атакующего ИГРОКА, а
  `Stock.add` гейтит хуки атаки по `isFromPlayer` — оба факта закреплены отдельными тестами, чтобы
  следующая правка не добавила вызов «на всякий случай».

### Прогоны (2026-09-25)

| Что | Итог |
| --- | --- |
| `tests/parliament/PlantBan.spec.ts` | 14 passing |
| `ResolutionContract` (весь каталог + dev) · `JointResearch` · `IndustrialistBudget` | 226 passing |
| `npm run test:server` | 12818 passing, 7 фейлов — **те же 7 на HEAD~3** (проверено откатом: `CentralPowerGrid` список счётов от RX20/21, `Cloner` по `stackHeight`, `SoilStudies` ×3, `ColonialRepresentation`, `VenusAllies`) |
| `npm run test:client` | 5971 passing, 27 фейлов — **те же 27 на HEAD~3** (проверено полным прогоном по откаченному дереву) |
| клиентские точечно: `plantBanReadings` · `levelYieldModel` · `voteInfoBudget` · `resolutionPremiumFace` | 11 + 11 passing; два красных — RX18 (слова) и RX20 (часть победителя), оба до RX25 |
| гарды: глоссарий · `rewardAddress` · `parliamentBand` · `e2eFixturesLoad` · `e2eDriverGuard` | зелёные |
| `vue-tsc` (по `src/`) · eslint по правленым · `build:test` (обе ступени) · `make:cards` · `make:json` | зелёные |
| e2e `console-parliament-ban` | 1 passed (35 с) |

Кадры: `screenshots/parliament-ban/standard-1080/` — `02-vote-warning` (норма · запас · −3 · предупреждение),
`04-plants-leave` (чип в воздухе, счётчик уже 4), `05-results` («−3» частью и «ТЕРЯТЬ НЕЧЕГО» у второго места).
