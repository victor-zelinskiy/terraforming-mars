...
| 9 | **круглая тег-иконка, зелёная** | Advance another track → Bio (см. TODO ниже) |
...
- **TODO(component-photo) — узкий, не блокирует:** поз. 9 — круглая «advance another track»
  иконка (нотация подтверждена легендой RB-A p.8: круглые тег-иконки = advance track этого
  тега). Прочтение: зелёный однотонный круг с каплевидным силуэтом = **Microbe** → Bio-трек.
  Альтернатива (менее вероятная по цвету): Jovian → Energy-трек. Константа
  `VENUS_CELL9_TARGET_TRACK` в `VenusMarsBot.ts` — флипнуть при получении фото.
