...
- `handCardModel.ts` — flat entry building (`buildHandEntries`, NO type grouping unlike played-cards), filter (`filterHandEntries`), sort (`sortHandEntries` — availability/cost/type/tag/name), and the type/tag chip data.
- `HandCardsOverlay.vue` — root: header (player chip + total + "Можно разыграть: N"), filters, adaptive grid (`grid-template-columns: repeat(auto-fill, calc(300px * var(--hand-card-zoom)))`, centered via `justify-content: center` so few cards don't hug the left), vertical scroll (expected here — custom thin scrollbar), single-click fullscreen via the shared `CardZoomModal` with a РАЗЫГРАТЬ in its `#actions` slot. Emits `@play(cardName)` / `@close`.
- `HandCardsFilters.vue` — availability segmented control (single-select), type chips (toggle-to-hide), tag chips (positive narrowing, tag icon + count), custom glass sort dropdown (no native `<select>`).
