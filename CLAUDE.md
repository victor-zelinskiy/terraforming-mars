...
**Source of truth for "playable now" is the server, reasons are client-derived.** The enabled state of РАЗЫГРАТЬ is gated STRICTLY by membership in the server's `Play project card` action list — `PlayerHome.findPlayProjectCardAction` walks `waitingFor` for the `projectCard`-type option titled `'Play project card'` (Player.ts action menu, `SelectCardToPlay.ts` title), and `playableProjectCardNames` is the set of names in its `cards` array. So the button can never offer an illegal play. The *reason* a card is greyed is derived client-side in `handCards/cardPlayability.ts` (`computeHandCardPlayState`): it checks the card's `requirements` (`getCard(name).requirements`) against live game globals + player tags/production, then affordability (`calculatedCost` + `additionalProjectCosts.redsCost` vs M€ + tag-applicable steel/titanium/heat), returning a STRUCTURED `UnplayableReason` (turn / megacredits / param / tag / production / generic). Reasons are display-only and degrade to a generic "Сейчас нельзя разыграть" for bespoke blockers we can't introspect — never blocking the (server-gated) button. No server changes were needed.

Framework (`src/client/components/handCards/`):
- `cardPlayability.ts` — the playability logic (`UnplayableReason`, `deriveIntrinsicReason`, `computeHandCardPlayState`).
...
- `HandCardsOverlay.vue` — root: header (player chip + total + "Можно разыграть: N"), filters, adaptive grid (`grid-template-columns: repeat(auto-fill, calc(300px * var(--hand-card-zoom)))`, centered via `justify-content: center` so few cards don't hug the left), vertical scroll (expected here — custom thin scrollbar), single-click fullscreen via the shared `CardZoomModal` with a РАЗЫГРАТЬ in its `#actions` slot. Emits `@play(cardName)` / `@close`. **Reflow is phased exit → reflow → enter** to stop tightening cards from sliding over still-fading ones: a `flush: 'pre'` watcher on `sorted` sets `reflowDelay` (true iff the upcoming patch REMOVES a card) BEFORE the grid re-renders, toggling a `--exiting` class on the `<transition-group>` in the SAME patch. The class delays the `-move` (reflow) and `-enter` transitions in `hand_cards.less` until the ~200ms `-leave` (exit) finishes; a pure re-sort or widening filter (no removals) leaves it false so reflow stays immediate. No timer — `reflowDelay` is recomputed on every change. Leaving cards are pinned in place by `@before-leave` (`onLeaveCapture` writes absolute left/top/width/height measured against the relative grid) so they shrink-fade where they sat. The initial open uses transition-group `appear` (`-appear-*`, nth-child stagger) — distinct from `-enter-*`, so a re-entering card never double-animates.
...
- `HandCardItem.vue` — one slot: `<Card>` (zero the legacy `.card-container` margin per "Centering UI") + the play affordance. `HandCardReasonPopover.vue` renders the reason (with a tag/resource icon for tag/production reasons). `HandCardsEmptyState.vue` for none/filtered.
