...
- `MODAL_INPUT_TYPES` = `payment, option, player, amount, resource, resources, productionToLose, and, projectCard` — these can only ever be a mandatory sub-decision, so every instance routes to `MandatoryInputModal`. (`projectCard` here is a card-driven "play a card / play a standard project" sub-prompt — EccentricSponsor, EcologyExperts, EstablishedMethods — NOT the action menu, whose `projectCard` is a NESTED option inside the top-level `or`. It currently renders via the legacy `SelectProjectCardToPlay.vue` form inside the factory fallback — visible + functional, premium hand/SP-overlay routing is a TODO.)
