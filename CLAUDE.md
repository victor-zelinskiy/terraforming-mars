...
- `MODAL_INPUT_TYPES` = `payment, option, player, amount, resource, resources, productionToLose, and, projectCard` — these can only ever be a mandatory sub-decision, so every instance routes to `MandatoryInputModal`. EXCEPTION for `projectCard`: `WaitingFor.useModalForCurrentInput` suppresses the modal when `handPlayPrompt(view)` matches (a "play a card FROM HAND" prompt — EccentricSponsor / EcologyExperts via `PlayProjectCard`, candidates all in hand) — those go to the КАРТЫ В РУКЕ overlay's PLAY mode instead (see below). A `projectCard` whose candidates are NOT in hand (EstablishedMethods' "play a standard project" via `SelectStandardProjectToPlay`) stays on the modal route and renders via the legacy `SelectProjectCardToPlay.vue` form inside the factory fallback (visible + functional; premium Standard-Projects-overlay routing is a TODO). The action menu's own `projectCard` is a NESTED option inside the top-level `or`, never top-level, so it's unaffected.
