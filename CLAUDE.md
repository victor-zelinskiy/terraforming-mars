...
The **desktop UI (`PlayerHome.vue` + its overlay stack) is FROZEN** (2026-07-15). Don't develop it further; the new desktop UI will one day be built *from* the console one, never the other way round. A desktop-only cosmetic bug: skip it, say so, move on. Nothing is deleted — desktop keeps working as a fallback.
