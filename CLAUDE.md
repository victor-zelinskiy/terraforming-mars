...
**`SelectAmount` and `SelectPlayer` carry the same icon vocabulary.** `SelectAmountModel` has optional `icon?` / `unit?`; the server `SelectAmount` constructor takes an optional 6th `options?: {icon?, unit?}` (backward-compatible — omit for a bare stepper). `ModernAmountSelector` renders the icon + unit beside the value via the same `iconClassFor`. `SelectPlayerModel` has optional `icon?` / `amount?` describing the (constant) effect applied to whichever player is picked (e.g. "−4 M€"); the server `SelectPlayer` constructor takes an optional 4th `options?: {icon?, amount?}`, and `ModernPlayerPicker` renders a `−<amount> <icon>` action badge above the player list.

Cards wired so far: `RemoveAnyPlants` (the flagship — shared by Asteroid / BigAsteroid / GiantIceAsteroid / DeimosDown / Comet / SmallAsteroid / ImpactorSwarm, one deferred → ~8 cards), `StealResources` (shared steal deferred, e.g. AirRaid) + `HiredRaiders` (steel/M€ steal) + `Sabotage` (titanium/steel/M€ removal — all player-target `SelectOption`s with a `Remove`/`Steal` verb buttonLabel since metadata-bearing player options surface the buttonLabel as their action text), `Atmoscoop` (temperature / Venus global-parameter), `Insulation` (heat-production `SelectAmount` with the heat icon), `OlympusConference` (add / remove a science card-resource), `Flooding` + `CometForVenus` (`SelectPlayer` remove-M€ with the action badge). To enrich a new option pattern: add/extend an `optionMetadata.ts` factory, call `.withMetadata(...)` where the `SelectOption` is built (or pass `SelectAmount` / `SelectPlayer`'s `options`), and only touch `optionIcons.ts` if a genuinely new icon-key family appears. **Player-target `SelectOption`s need an explicit verb buttonLabel** (the default `'Confirm'` would render as the action text) — `RemoveAnyPlants` uses `'Remove plants'`, the steal cards `'Steal'`, `Sabotage` `'Remove'`. The dev playground (`?modalPlayground`, `ModalInputPlayground.vue`) renders every modern input with mock data and tags each scenario `metadata: complete` / `fallback (text only)` so it's obvious which patterns are still on the text fallback — use it as a quality gate.
