...
The **desktop UI is BEING DELETED in waves** (policy change 2026-08-22; frozen since 2026-07-15). The future desktop UI will be built *from* the console one — the frozen code is dead weight (boot parse/eval, memory, tests, sync friction), not a fallback. **Wave 1 is done**: the console shell is UNCONDITIONAL (`consoleModeState.enabled` always true; `?console=0`, stored opt-outs, the settings shell switch and the hold-Menu toggle are gone), and App.vue no longer references `PlayerHome` or the desktop-only overlays. Files stay on disk until later waves delete them by import graph. ⚠️ **Four legacy pieces are LOAD-BEARING for the console and may be deleted only after replacement**: the `WaitingFor.vue` transport (polling + all submissions — extract a headless module first), legacy `Card.vue` (renders CEO faces until their premium pass; standard projects/actions moved to the premium face in wave 2), `MandatoryInputModal` (the `projectCard` degenerate-prompt fallback), and the entire SHARED layer below. A desktop-only bug: skip it — that code is scheduled for deletion.
