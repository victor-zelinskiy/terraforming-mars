...
The **desktop UI is BEING DELETED in waves** (policy change 2026-08-22; frozen since 2026-07-15). The future desktop UI will be built *from* the console one — the frozen code is dead weight (boot parse/eval, memory, tests, sync friction), not a fallback. **Waves 1–2 are done**: wave 1 made the console shell UNCONDITIONAL (`consoleModeState.enabled` always true; `?console=0`, stored opt-outs, the settings shell switch and the hold-Menu toggle are gone) and cut `PlayerHome` + the desktop overlays out of App.vue; wave 2 made `WaitingFor.vue` **HEADLESS** (poll chain + submit funnel + cinematic holds + the `SelectSpace` board binder — zero prompt rendering), deleted the whole radio/modal input stack (`PlayerInputFactory`, the leaf renderers, `MandatoryInputModal`, `ModalInputHost`, `PlayerHome`, the spectator feature), moved standard projects/actions onto the premium face, and gave the degenerate `projectCard` prompt a native console fallback in `ConsoleTaskHost` (pick → pay). **Waves 3–4 are done too**: wave 3 deleted the unreachable desktop subgraph (desktop journal/overlays, the TopBar chain, initialDraft, the desktop main menu + desktop create screens — their dead App branches cut, `/new-game` now lands in the console creator) and wave 4 moved CEOs onto the premium face (`ceo` theme + prose rule zone) and DELETED the legacy `Card.vue` renderer family (the render-DSL `CardRenderData` tree STAYS — shared by journal popovers / MarsBot faces). **The transport rework retired the last load-bearing legacy file**: `WaitingFor.vue` is DELETED — the poll chain / submit funnel / cinematic gates / view apply live in the **`console/transport/gameTransport.ts` MODULE** (`submitInput` / `submitBatch` / `cancelPlacement` — no `$refs` indirection), and `ConsoleBoardBinder.vue` owns the transport lifecycle + mounts `ConsoleBoardInput` (the console-native board cell-binder; the upstream `SelectSpace`/`GoToMap` are deleted). The WebSocket channel is the PRIMARY update signal (server + client default-ON; `?realtime=0` kill-switch), the poll is the bounded fallback (20 s while WS healthy, 1 s otherwise). Plus the entire SHARED layer below. A desktop-only bug: skip it — that code is scheduled for deletion.
...
10. **The game transport is a MODULE, and it renders NOTHING.** `console/transport/gameTransport.ts` owns the poll chain, the submit funnel (`submitInput` / `submitBatch` / `cancelPlacement`), the cinematic holds (`transportHolds`) and the view apply; `ConsoleBoardBinder.vue` owns its lifecycle and mounts only the `SelectSpace` board cell-binder. Every prompt renders through the console surfaces — never add UI to the transport, never submit around it.
...
13. **The Parliament speaks ONE glossary and stands still while the player points.** Every parliament surface names a concept by `docs/claude/parliament-glossary.md` (a RESOLUTION «принимается / принята», a PLAYER is the «победитель голосования», X is «ОСМОТРЕТЬ», the browse root is «ОБЗОР», the office is «председательство») — guard `tests/console/parliamentGlossary.spec.ts`; a d-pad walk over the overview moves nothing but the cursor and every party tile reserves one state-row height (`console-parliament-stability.spec.ts`); the crumb row stands on the shared workspace chassis (`console-parliament-chassis-parity.spec.ts`); a one-line status rail may cut a member only by an ellipsis in its OWN box, never by an ancestor, and a member yields width in a STRICT order (`expectRailHonest` / `expectDrawRailHierarchy` — `flex-basis: 0` + `min-width` + `max-width: max-content`, never a `flex-shrink` weight).
