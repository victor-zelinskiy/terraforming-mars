...
- The `reason` is an **English i18n key** describing the rule failure (the client can't always know the rule) — e.g. `'No plants to remove'`, `'Production already at minimum'`, `'Resources are protected'`, `'No Venus tag'`, `'No resources on this card'`. Add the ru value to `ru/ui.json`.
...
**Current coverage / next frontier:** metadata + disabled candidates are done for player-target prompts (`SelectPlayer` + the OrOptions attack/steal deferreds), global-parameter, `SelectAmount`, card-resource options, and **`SelectCard` (`RemoveResourcesFromCard`) with the Available/All/Unavailable filter**. Not yet enriched (same model shape applies when you do): `SelectColony` (untradeable colonies), maxed global parameters, and the deliberately-skipped blue-card `action()` / production-phase prompts.
