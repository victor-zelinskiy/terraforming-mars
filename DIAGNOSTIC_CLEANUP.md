...
**НЕ удалять (доп.):** warm-up top-layer `<dialog>` в AppBootLoader (`boot-loader__warm-dialog`) — это ФИКС Graphite first-allocation гонки (пустой первый fullscreen), часть warm-up фичи.

### Zoom first-open диагностика (клиент; временная — снять после ПОДТВЕРЖДЕНИЯ фикса 2026-07-14 на целевой машине; сам фикс описан в ZOOM_BUG_HANDOFF.md)
- `src/client/console/consoleZoomMotion.ts` — `zlog()` + его вызовы в `playZoomOpenFlight` (`[TM-DIAG zoom]`) + `zdump()` (решающий визуальный дамп; временно `export` — снять export вместе с ним).
- `src/client/components/console/ConsoleShell.vue` — `console.warn('[TM-DIAG zoom] …')` в watcher'е `consoleCardZoom.card` (tryOpen) и в `runZoomOpen` (open-строка + setTimeout-вызов `zdump`); временная probe-ручка `tm_zoom_vanilla` (форс ванильного открытия без полёта) в `runZoomOpen`. (Сам bounded-retry `tryOpen`, `runZoomOpen`/прокси-полёт и token-fence — ОСТАВИТЬ, это фикс.)
- `src/client/components/card/CardZoomModal.vue` — `console.warn('[TM-DIAG zoom] …')` в `show()` (showModal done / dialog ref missing) и в `fitCardToViewport` (card element NOT FOUND). (Сам `showRetries`-guard и `measureLanding()` — ОСТАВИТЬ, это фикс.)
