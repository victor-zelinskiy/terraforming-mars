...
### Zoom first-open диагностика (клиент; временная — снять после закрытия бага)
- `src/client/console/consoleZoomMotion.ts` — `zlog()` + все его вызовы в `playZoomOpen` (`[TM-DIAG zoom]`) + `zdump()` (решающий визуальный дамп) и его setTimeout-вызов в `playZoomOpen`. Ручку `tm_zoom_kick` + сам kick — решить по итогам (если kick не нужен после настоящего фикса — снять).
- `src/client/components/console/ConsoleShell.vue` — `console.warn('[TM-DIAG zoom] tryOpen …')` в watcher'е `consoleCardZoom.card`. (Сам bounded-retry `tryOpen` — ОСТАВИТЬ, это фикс.)
- `src/client/components/card/CardZoomModal.vue` — `console.warn('[TM-DIAG zoom] …')` в `show()` (showModal done / dialog ref missing) и в `fitCardToViewport` (card element NOT FOUND). (Сам `showRetries`-guard — ОСТАВИТЬ, это фикс.)
