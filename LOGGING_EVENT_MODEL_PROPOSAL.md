...
**Premium polish-pass журнала (группировка child rows + компактная шапка):**
- **Умная группировка `buildEventChildren` (одна строка = один значимый вклад источника, НЕ один raw event).**
  Дельты с одинаковым ключом `bucket + source(+benefit) + recipient + target` СЛИВАЮТСЯ в одну строку
  `source → impact · impact`; порядок — first-occurrence (хронологическая история действия). `JournalChildVM` += `bucket`
  (`JournalChildBucket`: payment/discount/placement/spaceBonus/oceanBonus/copied/effect/colony/globalParameter/production/
  card/system). Ключевые инварианты (защищены тестами `journalEventChild.spec.ts`): **оплата = один bundle** (`Оплата → M€ −13 · Титан −5`);
  **производство одной карты = одна строка** (`Метан с Титана → 🌿+2 prod · 🔥+2 prod`); **разные ПОЛУЧАТЕЛИ НЕ
  сливаются** (бонус Nastya vs Pets Victor — отдельные строки, recipient берётся из `e.player`, не из отфильтрованного);
  **разные БАКЕТЫ НЕ сливаются** (Бонус клетки ≠ Бонус океанов); tile-placed — всегда своя строка (ключ по id); copied —
  по copiedCard; источник НИКОГДА не теряется. `sourceDiscriminator` добавляет colony `#benefit` в ключ (награда трейда ≠
  бонус-владельцу). e2e подтверждено реальным Methane From Titan.
- **Премиальная подача (`JournalChildRow.vue` + journal.less):** строка — устойчивый FLEX-WRAP `[lead source(+recipient)] ·
  [arrow] · [impacts]` (lead `min-width:74px` → короткие метки примерно выровнены; контент НИКОГДА не сжимается в
  фиксированный трек). **КРИТ-ФИКС копирования (Проверка проекта/VIRON):** раньше строка была жёстким grid
  `minmax(80px,max-content)`, и широкий результат копирования («скопировано» + копируемая карта) сидел в lead-колонке →
  она схлопывалась до 80px, а `word-break:break-word` (на `__child-body`) ломал слово «скопировано» ПОБУКВЕННО вертикально,
  чипы вылезали за край. Фикс: (1) row → flex-wrap (не grid); (2) «скопировано»+копируемая карта перенесены в зону
  **impacts** (`source → copied <card> · chips`), а не в lead; (3) `__copied { white-space: nowrap }`. Теперь
  копирование рендерится чисто и переносится на узкой панели. Чипы
  тонированы по виду — **gain** (мятный) / **stock loss** (ЯВНО красный, hue≈2°) / **saved/discount** (золотой) /
  **production** (АНТАРНО-коричневый chip hue≈33° + иконка в коричневой ПЛИТКЕ — сигнатура production). Красный (stock) и
  коричневый (production) разнесены по hue + плитка иконки, чтобы не сливались (правка по фидбэку «loss и production почти
  не отличаются»); `--prod` применяется ДО `--neg`, поэтому атака на производство (−1 энергии-произв.) остаётся коричневой.
  `__src` тонируется по бакету. Стрелка тихая. Без локальных collapse — режим Подробно/Сводка прежний (считает СГРУППИРОВАННЫЕ вклады).
- **Компактная шапка-toolbar:** было 3 строки (заголовок / gen+filter / режим), стало 2 — переключатель **Подробно/Сводка**
  переехал в строку заголовка (использует пустовавшее место справа от `ЖУРНАЛ`, `titlewrap { margin-right:auto }`), gen+filter
  остались отдельной компактной строкой. Уменьшены высоты (mode-btn 28→23, genselect-trigger 28→26), паддинги, gap; список
  получил больше вертикали. Поведение и доступность сохранены.
