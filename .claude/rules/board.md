...
- **Not every "pick a cell" places a tile.** `SelectSpace.placementEffect` (`'tile'` default / `'bonus-only'` / `'marker'`) is the server's declaration, and `ctx.placesTile` / `ctx.grantsPlacementBonus` are what hooks read. Mars Nomads MOVES a camp — cell bonus yes, tile no, so no Ares adjacency, VP, milestone count or tile trigger (the card's own ruling: *"Mining Guild and Philares cannot take advantage of it"*); Land Claim / an Arcadian marker grant nothing at all. **A trigger that means "when a tile is placed" must gate on `ctx.placesTile`, never on live board state** — `game.nomadSpace` still points at the OLD cell during a preview. A new marker-style prompt that forgets to declare itself promises effects the commit suppresses.
