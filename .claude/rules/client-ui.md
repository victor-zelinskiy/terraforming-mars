...
⚠️⚠️ **And the LAN half is not a client problem at all: mDNS FINDS a host, a SOCKET says it is still there.** `bonjour-service`'s Browser emits `up` ONCE per service and never re-announces one it already holds, so a «last seen» fed from it freezes at discovery — the TTL sweep reading it deleted EVERY host 45 s after finding it, permanently, and only an app restart brought it back. That was the actual bug behind «партий по локальной сети не видно»; reworking the client did nothing for it. `lanDiscovery.ts` now re-reads presence from `browser.services` and decides liveness with a raced TCP connect — all of it in one sockets-free engine (`HostRegistry`) whose four invariants are: presence is ADDITIVE (an empty browser list removes nothing — `rebuildLinks()` on a Wi-Fi/VPN change makes it empty), only a socket removes, a hidden host keeps its own record and retries, one row per machine. The LAN section speaks in all three states (unreachable + REASON · answered-but-no-seat-with-your-name · stale) and «＋ Добавить хост по адресу» is the way in when multicast is blocked. The list is sorted **strictly by creation time, newest first**, and every row says how long ago it was created — the age is what makes the order legible, so the client sorts rather than inheriting a server's order, and ONE reactive clock (`lobbyState.nowMs`, re-armed at the freshest row's cadence, alive only while the screen is) moves every label in the same tick. **«Мои кампании» obeys the same rule on BOTH tabs** (`sortCampaignsNewestFirst`; the row's age is the CREATION age, and the screen hands its rows to that one clock via `setLobbyAgeRows`) — its sort once led with a priority band (action required → launch-ready), and since a mission's turn flips the row's state on every move, a fresh campaign sat on top one visit and at the bottom the next. What needs the player is marked ON the row, never by where it stands. ⚠️ Timers here pair `window.setTimeout` with `window.clearTimeout`: in a browser the bare global is identical, but under jsdom they are different implementations and a bare clear silently leaks the timer into later specs. Every trigger funnels into `refreshLobby()` — adding a trigger must never mean adding a loading path. Server side: `models/lobbyIndex.ts` (a per-game record + a revision; `IGameLoader.peek()` exists so a bulk sweep never `touch`es the cache). Full contract: `docs/claude/my-games-lobby.md`.
