...
A stage reached from inside a workspace is done only when ALL FOUR hold: (1) **embedded**, not a new surface; (2) the breadcrumb is **continuous** — `ДЕЙСТВИЯ КАРТ › <ЭТАП> · <карта>`, workspace name and card name never restart, and the embedded surface **hands its stage name up** (`setWorkspaceOutcomePhase`) instead of drawing its own kicker; (3) the transition is the **same phrase one level deeper** — RELEASE in place → UNFOLD from the old surface's rect → REVEAL from inside (`consoleActionOutcomeMotion.ts`); a `v-if` swap is a blink and is never acceptable; (4) **B = one logical level**, carried card survives. Reference: the blue-action flow.
