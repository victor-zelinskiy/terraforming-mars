...
A decision surface stands between the two bars **and right of the always-visible player rail** (resources stay context, and they are the landing zone of resource flights). Two things, always **both**: `.con-ws-band()` in LESS (the modal band + `left: var(--con-ws-left)`) and the **`con-ws` marker class** on the surface root — `.con-root:has(.con-ws)` drops `.con-main`'s stacking trap, lifts the rail to z11520, rings it, and hides the ДОП.РЕСУРСЫ satellite, holding automatically through leave transitions (no JS "closing" flag). Mixin without marker → the rail stays under the shade; marker without mixin → the rail sits over a full-width panel. Panel widths inside the band are `min(Xrem, 100%)` — a `vw` cap overruns the narrowed band on the Deck. Seam tokens (`--con-pad-x`, `--con-rail-w`, `--con-main-gap`, `--con-ws-left`) live on `.con-root`; profiles override the TOKENS there. Cinematics (reveal / ceremony / mandatory announce / fullscreen zoom / system menu / start scene / endgame) deliberately stay full-bleed. Full contract + family list: `docs/claude/console/workspace-band.md`.
