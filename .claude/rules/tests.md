...
- ⚠️ **A LAYOUT GUARD CALIBRATED ON WINDOWS HAS NO MARGIN ON THE LINUX RUNNER.** Chromium lays HUD-sized text out with SUBPIXEL glyph advances on a dev box and with advances ROUNDED UP TO WHOLE PIXELS on CI (FreeType, below the size at which subpixel positioning turns on): «ДОСТИЖЕНИЯ» inks 98.0 px at 13.2 px locally and ~103.5 px on the runner (+0.55 px per glyph), while the SAME string at 34 px costs nothing extra — which is why the strategy rail shipped an ellipsis only the FHD shard ever saw, at every retry, reproducible nowhere. So a text-fit assertion must measure HEADROOM, not «does it fit here», against the worst case of that rounding (1 px per glyph), and it must measure the COLUMN rather than the text's own box: a `flex: 0 1 auto` title that fits reports its own ink and says nothing about the room it has (measure a clone of the real element at the real width, with a string no line can hold). Reference: `tests/e2e/console-strategy-rail.spec.ts` § `headFits`.
