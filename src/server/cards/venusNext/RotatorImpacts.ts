...
export class RotatorImpacts extends Card implements IActionCard {
  constructor() {
    super({
      ...
      metadata: {
        cardNumber: '243',
        infoText: [
          {kind: 'action-short', text: 'Add an asteroid to this card', tokens: ['titanium']},
          {kind: 'action-short', text: 'Raise Venus one step', tokens: ['venus']},
        ],
  ...
  private addResource(player: IPlayer) {
    player.game.defer(new SelectPaymentDeferred(player, 6, {canUseTitanium: true, title: TITLES.payForCardAction(this.name), cause: cardSource(this)}));
