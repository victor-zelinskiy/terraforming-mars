...
export class SoilEnrichment extends Card implements IProjectCard {
  ...
  public override play(player: IPlayer) {
    ...
    // Per the fork's no-autoselect principle, ALWAYS ask which card the microbe is
    // taken from — even when only one card holds a microbe.
    return new SelectCard('Select card to remove 1 microbe from', 'Select', cards)
      .andThen(([card]) => {
  ...
  // the live follow-up. Per the no-autoselect principle the picker shows whenever
  // at least one card holds a microbe (even a single candidate).
