...
export class Game implements IGame, Logger {
    ...
    if (this.players.every((p) => p.isMarsBot || initialSelectionDoneOf(p))) {
      for (const somePlayer of this.playersInGenerationOrder) {
        ...
        if (somePlayer.dealtCorporationCards.length === 0 && !campaignHuman && somePlayer.pickedCorporationCard !== undefined) {
          ...
          // until the player explicitly presses it on the start screen —
  ...
  /**
   * The explicit "play your chosen corporation" prompt (startGamePrompt kind
   * `corporationPlay`). `pickedCorporationCard` is already serialized, so the
  ...
  public /* for testing */ takeNextFinalGreeneryAction(): void {
    for (const player of this.playersInGenerationOrder) {
      ...
      if (this.isSoloMode() && !this.isSoloModeWin()) {
        this.log('Final greenery phase is skipped since you did not complete the win condition.', (b) => b.announcement());
        continue;
      }
