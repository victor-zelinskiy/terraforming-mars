<template>
  <!--
    ...
    printed face available:
     - premium-face types (project / prelude / corporation / CEO since
       desktop-removal wave 4) render the REAL PremiumCard in name-only
       static mode at the THUMB quality tier (`lightweight` — no
       texture/glow/hover work), inert by construction;
     - the remaining out-of-scope types keep the legacy lite face via
       ConsoleCardFaceLite's own fallback branch (wave-4 part 2 deletes it
       once nothing routes there).
...
export default defineComponent({
  ...
  props: {
     ...
     *  fallback face (out-of-scope types) has no peek mode and stays full. */
    peek: {
      type: Boolean,
      required: false,
      default: false,
