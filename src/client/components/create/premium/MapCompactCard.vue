<template>
  <div class="map-compact" :class="{'map-compact--conflict': mapConflicted}" @mouseenter="focusInfo" @focusin="focusInfo">
    ...
    <div class="map-compact__info">
      ...
      <span v-if="mapConflicted" class="map-compact__conflict"><span class="map-compact__conflict-icon" aria-hidden="true">!</span><span v-i18n>MarsBot plays on the Tharsis map only for now</span></span>
...
import {createGameState, setInfoFocus, openMapPicker, stateAutomaConflictKeys} from './createGameState';
...
export default defineComponent({
  ...
  computed: {
    ...
    mapConflicted(): boolean {
      return stateAutomaConflictKeys().has('board');
    },
  },
  methods: {
    focusInfo(): void {
