<template>
  <div class="eg-tab eg-cards">
    <!-- Top scoring cards across the table -->
    <section v-if="topCards.length > 0" class="eg-cards__top">
      ...
      <div class="eg-cards__top-list">
        <div v-for="(item, i) in topCards" :key="i" class="eg-topcard" :style="{'--eg-pc': hex(item.color)}">
          <span class="eg-topcard__rank">{{ i + 1 }}</span>
          <span class="eg-topcard__owner-dot" :class="'player_bg_color_' + item.color" :title="item.owner"></span>
    ...
    <section class="eg-cards__players" :class="{'eg-cards__players--duel': model.mode === 'duel'}">
      <div v-for="p in model.players" :key="p.color" class="eg-cards__col"
        ...
        <div v-else class="eg-cards__rows">
          <div v-for="(c, i) in p.topCards" :key="'t' + i" class="eg-cardrow">
            <span class="eg-cardrow__kind" :class="'vp-accent--' + kindAccent(c.kind)" :title="$t(kindLabel(c.kind))"></span>
