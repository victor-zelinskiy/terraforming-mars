 ...
 * preludes + corporations + CEOs; scope gate = premiumCardTheme.isPremiumFaceType).
 * Mirrors the legacy <Card> host contract: `card` (CardModel), `actionUsed`,
 * `robotCard`, `cubeColor`, `lightweight`; click opens the shared fullscreen
 * viewer behind the same preference. `name`-only mode renders the pristine
 * printed face for inert proxies (console cinematics).
 */
