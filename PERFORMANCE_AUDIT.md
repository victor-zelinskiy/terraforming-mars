...
## Iteration 2 — Electron GPU tuning + webp + trace verification (2026-07-02)

### Trace #2 (post-cull) CONFIRMS the backdrop cull worked
`Trace-20260702T143052.json` (234 MB, ~264 s), same scenario class, vs trace #1:
| primitive | trace #1 | trace #2 | Δ |
|---|---|---|---|
| **Layerize** | 7991 ms | **4196 ms** | **−48%** |
| long-tasks ≥50 ms (sum) | 13766 ms | 11552 ms | −16% |
| UpdateLayoutTree | 8140 ms | 8269 ms | ~= (now the clear #1) |
| FunctionCall (JS) | 867 ms | 909 ms | ~= |

The compositor cost dropped ~3.8 s. `playerHome:mount`/`metricValue:mount` = 0 during play again (no-remount holds). The new #1 single task is a **494 ms input→JS** overlay open (card-mount burst) — JS/style-recalc, NOT compositor. The dominant remaining cost is `UpdateLayoutTree` (style recalc from mounting ~50 cards/overlay) — inherent, hard to cut safely without virtualization.

### Electron GPU / performance tuning — SHIPPED (`electron/perf.ts` + `main.ts` + `protocol.ts`)
The trace's remaining cost is rendering/compositing, so the highest-value desktop lever is pushing that to the GPU. `applyPerformanceSwitches(app)` (before `ready`, desktop-only) appends: **`ignore-gpu-blocklist`, `enable-gpu-rasterization`, `enable-zero-copy`, `enable-accelerated-2d-canvas`, `force_high_performance_gpu`** (GPU), and **`disable-background-timer-throttling` / `disable-renderer-backgrounding` / `disable-backgrounding-occluded-windows`** (never throttle the fullscreen game). Two aggressive flags are env-gated OFF by default: `TM_ELECTRON_UNCAP_FPS=1`→`disable-frame-rate-limit`, `TM_ELECTRON_FORCE_GPU=1`→`disable-software-rasterizer` (removes the CPU fallback). `backgroundColor` + `webPreferences.backgroundThrottling:false` added. `logGpuStatus(app)` prints the resolved `getGPUFeatureStatus()` on launch so HW accel is verifiable. **`protocol.ts` cache headers:** stable art/fonts now serve `Cache-Control: immutable` (packaged) / `max-age=3600` (dev) so the frequent in-app game-boundary reloads keep decoded textures in Chromium's memory cache instead of re-reading+re-decoding from disk; HTML/CSS/JS never cached. Guarded by `tests/electron/perf.spec.ts` (6) — 33 electron specs green; `electron:build:main` (tsc) clean. **NOTE:** these switches affect the ELECTRON client only; the browser build is untouched. The GPU flags target Paint/Commit/Layerize/GPUTask (compositor), NOT `UpdateLayoutTree` (main-thread style recalc).

### WebP (BND-3) — SHIPPED for the big-3 runtime textures
`scripts/convert-webp.cjs` (sharp 0.35.3, dev dep) converted the heavy RUNTIME textures at high quality, **keeping dimensions** (sprite `background-position` offsets stay valid): `board/mars.png` **4.64→0.32 MB** (q90 planet), `resources/card.png` **2.94→0.36 MB** (q90), `board_icons_ares.png` **2.73→1.98 MB** (lossless icon sprite) — **−7.5 MB**. All **20 refs** updated `.png→.webp` (12 `.less` + `DeltaProjectBoard.vue`) via a scripted path-scoped replace (`board/mars`, `resources/card`, `board_icons_ares` — distinct from the small `tags/mars.png`), old PNGs deleted, grep-verified zero stale refs. `protocol.ts`/`electron-builder.yml`/browser all already serve webp. `make:css` + `build:client` + `build:desktop` clean. **Honesty:** webp cuts download / installer / decode / source-memory, NOT the GPU raster of the DECODED bitmap (dimensions unchanged for max-zoom board quality) — so it helps load/installer/memory, not the `GPUTask` raster directly. The many small tag/resource sprite sheets are a contained follow-up (more refs each).

### Still the frontier (needs virtualization / deeper work or a visual call)
