...
// `electron:desktop` / `electron:run` are hosted by node_modules/electron/dist/electron.exe, whose
// embedded icon is the stock Electron atom: without this every dev window lies about the app.
// __dirname is build/electron at runtime, so the repo root is two levels up. Windows wants the
// multi-size .ico, Linux a PNG (both generated by scripts/make-app-icon.cjs).
const DEV_WINDOW_ICON = path.join(
  __dirname, '..', '..', 'electron', 'build-resources',
  process.platform === 'win32' ? 'icon.ico' : 'linuxIcon.png');
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    // Packaged inherits the branded exe icon; unpacked would otherwise show Electron's atom.
