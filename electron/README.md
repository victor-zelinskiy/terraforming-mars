...
| `TM_ELECTRON_SWITCHES` | — | Semicolon-separated raw Chromium switches (`key` or `key=value`), appended LAST so they override same-key defaults. E.g. `skia-graphite-dawn-backend=d3d11` (roll the default D3D12 Graphite backend back to D3D11), `disable-direct-composition`, `use-angle=d3d9`, `js-flags=--max-old-space-size=1024`, `ozone-platform=wayland;use-angle=vulkan` (a GL/display key on Linux also skips the default `--disable-gpu`). Replaces the retired per-knob env vars. |
