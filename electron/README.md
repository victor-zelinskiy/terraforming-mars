...
| `TM_ELECTRON_AFFINITY` | `auto` (win32) | CPU-affinity pinning for the main + renderer processes. `auto` (default) auto-detects an Intel HYBRID CPU (P-cores + E-cores) and pins to the **P-cores** — keeps the layout-bound renderer main thread on the fast cores instead of the weak E-cores (the "same code, worse than the console" gap). No-op on a uniform CPU / non-hybrid. `off`/`normal` = leave the OS scheduler. An explicit `0xfff` / `4095` mask forces that affinity (unusual topology). Windows-only. Reports itself as a `[TM affinity]` line in the renderer console (target mask, the WHY, and the per-pid READ-BACK) on every page load, but only re-runs the powershell shell-out when the pinned pid set actually changes — a pinned process stays pinned, and a powershell cold start costs seconds while the CPU is saturated. See `perf.ts` `pCoreAffinityMask`. |
