...
// wrapper) are Linux-only and have no Windows analogue — Velopack's Setup.exe places the
// .exe and self-updates in place, so no wrapper is needed.
//
// Invoked from the renderer via the `desktop:addToSteam` IPC — the in-app "Add to Steam"
// button + the first-run opt-in prompt (PremiumMainMenu / ConsoleMainMenu). Replaces the
// removed NSIS finish-page checkbox (Velopack's Setup.exe has no installer UI).
//
...
/** The bundled Steam art files (shared with the Steam Deck installer). `icon` is the SQUARE
 *  premium icon for the shortcut's `icon` field (list / "now playing"); the others are the wide
 *  library grid art. Generated by scripts/make-app-icon.cjs. */
const ART_FILES = {
 ...
 * both the quoted- and unquoted-Exe appid variants (mirrors the Deck installer). Also writes the
 * SQUARE premium icon and returns its path for the shortcut's small `icon` field (Steam wants a
 * square image there — the wide header read as blank/cropped, the "missing icon" complaint).
 * Returns '' when the icon asset is unavailable (caller falls back to the exe icon).
 */
function writeGridArt(gridDir: string, artIds: ReadonlySet<number>): string {
  ...
  const put = (buf: Buffer | undefined, ...names: Array<string>): void => {
    ...
    for (const name of names) {
      fs.writeFileSync(path.join(gridDir, name), buf);
    }
  };
  ...
  // The small SQUARE icon (list / friends "now playing"). Written once; the shortcut points at it.
  const iconBuf = readArt(ART_FILES.icon);
  if (iconBuf !== undefined) {
    const iconDest = path.join(gridDir, 'terraforming-mars-icon.png');
    fs.writeFileSync(iconDest, iconBuf);
    return iconDest;
  }
  return '';
}
...
function writeProfile(userDir: string, opts: {
  ...
  try {
    ...
    const iconDest = writeGridArt(path.join(cfg, 'grid'), opts.artIds);
    const icon = iconDest !== '' ? iconDest : opts.exePath;
...
/**
 * Whether our Non-Steam shortcut is ALREADY registered in any logged-in Steam profile on this
 * machine (matched by the quoted Exe or our deterministic appid). Windows-only, read-only,
 * best-effort — never throws. Used to hide the first-run "Add to Steam" prompt + the in-menu
 * button once the shortcut exists (a manual removal in Steam re-reveals them).
 */
export function isAddedToSteam(): boolean {
  ...
  const exeQuoted = `"${app.getPath('exe')}"`;
  const appId = appId32(exeQuoted + APP_NAME);
  for (const userDir of steamUserDirs(steamPath)) {
    try {
      const scPath = path.join(userDir, 'config', 'shortcuts.vdf');
      if (!fs.existsSync(scPath) || fs.statSync(scPath).size === 0) {
        continue;
      }
      const root = parseShortcuts(fs.readFileSync(scPath));
      if (shortcutExists(root.shortcuts as VdfMap, exeQuoted, appId)) {
        return true;
      }
    } catch {
      // a malformed / locked profile must not break detection — check the rest.
    }
  }
  return false;
}
