...
describe('sittingBeats — the sitting director\'s pure list', () => {
  ...
  it('every fact the summary carries adds its beat, in the SERVER\'s order', () => {
    ...
    // The enactment's three beats play on ONE page; the renewal and the lobby on the RENEWAL page («Обновление»); the closing card on the RESULTS page.
    expect(beats.map((b) => b.stage)).deep.eq(['verdict', 'enact', 'enact', 'enact', 'reward', 'reward', 'renewal', 'renewal', 'results']);
  ...
  it('THE JOURNAL is the renewal: a summary whose journal holds events plays the tact (a save without a journal still names its refresh in the flat lists)', () => {
    const journalled = sittingBeats(summary({renewal: [
      {kind: 'leave', instance: 'RDX_B#0', resolution: 'RDX_B', party: PartyName.REDS, slot: 1, returned: []},
      {kind: 'reshuffle', size: 2},
      {kind: 'deal', instance: 'RDX_B#0', resolution: 'RDX_B', party: PartyName.REDS, slot: 0, source: 'reshuffled'},
      {kind: 'lobby', player: BLUE},
    ]}), BLUE, 'live');
    expect(journalled.map((b) => b.kind)).deep.eq(['verdict', 'enact', 'renewal', 'lobby', 'closing']);
    // A card that left and was dealt straight back is NOT an exception: the tact plays, nothing says «stays».
    expect(sittingBeatsOfStage(journalled, 'renewal').map((b) => b.kind)).deep.eq(['renewal', 'lobby']);
  });
  ...
  it('RESUME compacts the stages before the current one; REVIEW compacts everything; LIVE nothing', () => {
    ...
    expect(sittingBeats(s, BLUE, 'resume', 'results').map((b) => `${b.kind}:${b.compact}`)).deep.eq(['verdict:true', 'enact:true', 'renewal:true', 'closing:false']);
    expect(sittingBeats(s, BLUE, 'resume', 'renewal').map((b) => `${b.kind}:${b.compact}`), 'a restore INTO the renewal plays its tact in full').deep.eq(['verdict:true', 'enact:true', 'renewal:false', 'closing:false']);
  ...
  it('the beats of one stage are the page\'s own', () => {
    ...
    // «Обновление»: the RENEWAL page plays the tact and the lobby; the RESULTS page only reveals its card.
    expect(sittingBeatsOfStage(beats, 'renewal').map((b) => b.kind)).deep.eq(['renewal', 'lobby']);
    expect(sittingBeatsOfStage(beats, 'results').map((b) => b.kind)).deep.eq(['closing']);
