...
async function crumb(page: Page): Promise<{subject: string, stage: string}> {
  return page.evaluate(() => {
    ...
    return {subject: text('.con-wshead__subject'), stage: text('.con-wshead__step')};
  });
}
...
test('card-door Pluto trade: B parks the deep chain, the wheel opens a FRESH list, A resumes the exact discard', async ({page, request}) => {
  ...
  await shoot(page, '02-discard-deep');
  ...
  for (let cycle = 0; cycle < 2; cycle++) {
    ...
    if (cycle === 0) {
      await shoot(page, '04-fresh-look');
    }
  ...
  await shoot(page, '05-resumed-discard');
