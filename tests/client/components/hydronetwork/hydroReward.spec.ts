...
describe('buildRewardView', () => {
  it('pos 3: +2 M€ production as current → new', () => {
    ...
    expect(v.lines[0]).deep.include({production: true, before: 5, after: 7, delta: 2});
  });
  ...
  it('pos 6: plants = current + plant-tag count (the computed amount)', () => {
    ...
    expect(v.lines[0]).deep.include({before: 4, after: 7, delta: 3, noteKey: 'Plant tags', noteValue: 3});
  });

  it('pos 8: +1 Jovian tag', () => {
    const v = buildRewardView({stage: stage(8), snapshot: snapshot({jovianTags: 1}), rewardChoice: undefined});
    expect(v.lines[0]).deep.include({special: 'jovian-tag', before: 1, after: 2, delta: 1});
  });
  ...
  it('pos 1: a choice must be made before a delta shows', () => {
    ...
    expect(steel.lines[0]).deep.include({before: 6, after: 8, delta: 2});
    const plants = buildRewardView({stage: stage(1), snapshot: snapshot({plants: 4}), rewardChoice: 1});
    expect(plants.lines[0]).deep.include({before: 4, after: 6, delta: 2});
  });
  ...
  it('pos 9: animals delta only once a target card is picked', () => {
    ...
    expect(after.lines[0]).deep.include({special: 'animals', before: 1, after: 3, delta: 2, cardName: 'Birds'});
  });
