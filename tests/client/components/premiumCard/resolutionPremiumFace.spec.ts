 ...
 * and on every SHIPPED resolution. The face prints the mechanics as graphics
 * and the chairman quest as a GRAPHIC zone with the reward marks (never a
 * sentence).
 */
...
describe('PremiumCard — the resolution face template', () => {
  for (const [id, label] of [['RDX_DEV_IMMEDIATE', 'immediate'], ['RDX_DEV_PASSIVE', 'passive'], ['RDX_DEV_ACTION', 'action']] as const) {
    it(`a ${label} resolution prints its mechanics as graphics and the quest as a graphic zone`, () => {
