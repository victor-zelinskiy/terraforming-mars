...
describe('rewardAddress — the delivery of a record', () => {
  it('resolves the payload from the record (resource, card, amount, parameter, party) and whether it is the viewer\'s', () => {
    ...
    expect(animals.payload).deep.eq({resource: 'Animal', card: CardName.FISH, cards: [{card: CardName.FISH, amount: 2, resource: 'Animal'}], amount: 2});
    ...
    expect(spread.payload).deep.eq({resource: 'Floater', cards: [{card: CardName.DIRIGIBLES, amount: 2, resource: 'Floater'}, {card: CardName.FLOATING_HABS, amount: 1, resource: 'Floater'}], amount: 3});
