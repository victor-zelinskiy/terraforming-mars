...
describe('Stock', () => {
  ...
  it('add logging', () => {
    ...
    expect(formatMessage(logEntry)).eq('blue gained 12 megacredits');
  });
  ...
  it('add logging from player', () => {
    ...
    expect(formatMessage(logEntry)).eq('blue lost 5 megacredits because of red');
  });
  ...
  it('add logging from global event', () => {
    ...
    const log = game.gameLog;
    const logEntry = log[log.length - 1];
    expect(formatMessage(logEntry)).eq('blue gained 12 megacredits because of Asteroid Mining');
  });
